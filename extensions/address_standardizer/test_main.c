...
static int
standardize_command_line(STANDARDIZER * std,
			 char *input_str,
			 int option);
...
parse_csv(char *buf, int *seq, char *word, char *stdword, int *token)
{
	...
	if ((next_str = convert_field(num_str, buf)) == NULL)
		return 0;
 ...
 *
 * 1 2 11 28 -1 10 10 11 13 -1 0 16 1 2 11 28 12 -1 10 10 11 13 12 -1 0 17 1
 * 2 11 28 29 -1 10 10 11 13 13 -1 0 16 1 2 11 28 29 12 -1 10 10 11 13 13 12
 * -1 0 17 -1
 */
...
main(int argc, char *argv[])
{
	 ...
	 * rules_free(rules); lex_free(gaz); lex_free(lex);
	 */
