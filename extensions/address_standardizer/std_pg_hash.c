...
StdCacheDelete(void *ptr)
{
	...
	/*
	 * lookup the hash entry in the global hash table so we can free it
	 */
...
DeleteNextSlotFromStdCache(StdPortalCache * STDCache)
{
	...
	if (STDCache->StdCache[STDCache->NextSlot].std != NULL) {
		...
		/*
		 * zero out the entries and free the memory context We will
		 * get a callback to free the std object.
		 */
...
AddToStdPortalCache(StdPortalCache * STDCache, char *lextab, char *gaztab, char *rultab)
{
	...
	/*
	 * change memory contexts so the pstrdup are allocated in the context
	 * of this cache item. They will be freed when the cache item is
	 * deleted.
	 */
