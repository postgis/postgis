 ...
 *
 * Stphen Woodbridge woodbri@swoodbridge.com woodbr@imaptools.com
 *
 *
 * TODO: * add recognition of country before or after postalcode * have clean
 * trailing punctionation return a code if a comma was removed if comma and
 * no state then there is probably no city
 *
 */
...
#include "parseaddress-regex.h"
	 ...
	 *
	 * pn1? n1 pn2? n2? sn2? where pn1 is a prefix character n1  is a
	 * number pn2 is a prefix character n2  is a number sn2 is a suffix
	 * character
	 *
	 ...
	 * belongs to which component. Futher since the the street and the
	 * city are both just a string of arbitrary words, it is difficult if
	 * not impossible to determine if an give word belongs to sone side
	 * or the other.
	 *
	 * So for the best results users should include a comma after the
	 * street.
	 *
	 * The approach will be as follows: 1. look for a comma and assume
	 * this is the separator 2. if we can find a state specific regex try
	 * that 3. else loop through an array of possible regex patterns 4.
	 * fail and assume there is not city
	 */
	...
	/*
	 * -- 2010-12-11 : per Nancy R. we are using @ to indicate an
	 * intersection ampersand is used in both street names and landmarks
	 * so it is highly ambiguous --
	 */
