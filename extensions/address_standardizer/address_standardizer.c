 ...
 * rultab should not change once the reference has been standardized and the
 * same tables must be used for a geocode request as were used on the
 * reference set or the matching will get degregated.
 *
 * select * from standardize_address( lextab text,  -- name of table of view
 * gaztab text,  -- name of table or view rultab text,  -- name of table of
 * view micro text,   -- '123 main st' macro text);  -- 'boston ma 01002'
 *
 * If you want to standardize a whole table then call it like:
 *
 * insert into stdaddr (...) select (std).* from ( select
 * standardize_address( 'lextab', 'gaztab', 'rultab', micro, marco) as std
 * from table_to_standardize) as foo;
 *
 * The structure of the lextab and gaztab tables of views must be:
 *
 * seq int4 word text stdword text token int4
 *
...
standardize_address1(PG_FUNCTION_ARGS)
{
	...
	k = 1;
	if (paddr->city)
		k += strlen(paddr->city) + 1;
	if (paddr->st)
		k += strlen(paddr->st) + 1;
	if (paddr->zip)
		k += strlen(paddr->zip) + 1;
	if (paddr->cc)
		k += strlen(paddr->cc) + 1;
