...
--
-- Processed by the build system: /Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3 is replaced with the
...
CREATE OR REPLACE FUNCTION h9_version()
    RETURNS text
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_version'
...
CREATE OR REPLACE FUNCTION h9_encode(geometry)
    RETURNS uuid
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_encode'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 50;

-- h9_diag(geometry) → text
--   Diagnostic: returns BRAW (pre-warp) and BARY (post-warp) (cx, cy) the
--   C++ encoder feeds into descent. Temporary aid for verifying frame
--   parity with Python's b_oct.encode pipeline. Drop once parity is
--   confirmed.
CREATE OR REPLACE FUNCTION h9_diag(geometry)
    RETURNS text
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_diag'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 50;
...
CREATE OR REPLACE FUNCTION h9_decode(uuid)
    RETURNS geometry
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_decode'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 10;
...
CREATE OR REPLACE FUNCTION h9_bin(uuid, integer)
    RETURNS uuid
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_bin'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 5;
...
CREATE OR REPLACE FUNCTION h9_label(uuid, integer)
    RETURNS text
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_label'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 5;
...
CREATE OR REPLACE FUNCTION h9_label_key(uuid, integer)
    RETURNS text
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_label_key'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 5;
...
CREATE OR REPLACE FUNCTION h9_cell(uuid, integer, integer DEFAULT 0)
    RETURNS geometry
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_cell'
    LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE
    COST 200;

-- h9_grid(geometry, integer, integer DEFAULT 0) → TABLE(hex9 uuid, geom geometry, centroid geometry)
--   Set-returning function: yields (bin_uuid, polygon) for every H9 cell at
--   the given layer (1..29) whose geographic centre falls within `bounds`.
--   The UUID column matches h9_bin(h9_encode(pt), layer) for any point in
--   that cell.
--
...
--
--   The third output column `centroid` is the cell's geographic centre as a
--   POINT (SRID 4326), computed by the BFS in the cell's own frame — useful
--   for labelling, clustering, or as a join key without re-decoding the UUID.
--
--   Example:
...
--     SELECT hex9, ST_AsText(geom)
--     FROM h9_grid(ST_MakeEnvelope(-10, 35, 30, 60, 4326), 2, 3);
--
-- Availability: Hex9 1.1.0 (arg order changed; centroid column added; densify added)
CREATE OR REPLACE FUNCTION h9_grid(geometry, integer, integer DEFAULT 0)
    ...
    AS '/Applications/Postgres.app/Contents/Versions/18/lib/postgresql/postgis_hex9-3', 'h9_grid'
