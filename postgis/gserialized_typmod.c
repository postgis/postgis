...
Datum
postgis_typmod_out(PG_FUNCTION_ARGS)
{
	...
	/*
	 * No SRID or type or dimensionality? Then no typmod at all. Return
	 * empty string.
	 */
...
postgis_valid_typmod(GSERIALIZED * gser, int32_t typmod)
{
	...
	/*
	 * #3031: If a user is handing us a MULTIPOINT EMPTY but trying to
	 * fit it into a POINT geometry column, there's a strong chance the
	 * reason she has a MULTIPOINT EMPTY because we gave it to her during
	 * data dump, converting the internal POINT EMPTY into a EWKB
	 * MULTIPOINT EMPTY (because EWKB doesn't have a clean way to
	 * represent POINT EMPTY). In such a case, it makes sense to turn the
	 * MULTIPOINT EMPTY back into a point EMPTY, rather than throwing an
	 * error.
	 */
...
static uint32 gserialized_typmod_in(ArrayType * arr, int is_geography){
	...
	deconstruct_array(arr,
			  CSTRINGOID, -2, false, 'c',	/* hardwire cstring
							 * representation
							 * details */
			  &elem_values, NULL, &n);
...
Datum
geography_typmod_in(PG_FUNCTION_ARGS)
{
...
Datum
geometry_enforce_typmod(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *arg = PG_GETARG_GSERIALIZED_P(0);
	int32		typmod = PG_GETARG_INT32(1);
	/* We don't need to have different behavior based on explicitness. */
...
/*
 * * postgis_typmod_type(uint32) returns cstring * Used for geometry_columns
 * and other views on system tables
 */
...
/*
 * * postgis_typmod_dims(uint32) returns int * Used for geometry_columns and
 * other views on system tables
 */
PG_FUNCTION_INFO_V1(postgis_typmod_dims);
Datum
postgis_typmod_dims(PG_FUNCTION_ARGS)
{
	int32		typmod = PG_GETARG_INT32(0);
	int32		dims = 2;
	if (typmod < 0)
...
/*
 * * postgis_typmod_srid(uint32) returns int * Used for geometry_columns and
 * other views on system tables
 */
