 ...
 * operator class, we need to overload the original
 ...
 *
 * The previous limitation might be lifted, but we also eliminate some
 * overhead by doing it this way, namely calling different functions through
 * the FunctionCallInvoke machinery for each heap tuple.
 */

Datum
gidx_brin_inclusion_add_value(BrinDesc * bdesc, BrinValues * column, Datum
			      newval, bool isnull, int max_dims);

/*
 * As for the GiST case, geographies are converted into GIDX before they are
 * added to the other index keys
 */
...
gidx_brin_inclusion_add_value(__attribute__((__unused__)) BrinDesc * bdesc,
	...
	/*
	 * No need for further processing if the block range is already
	 * initialized and is marked as containing unmergeable values.
	 */
	...
	if (gserialized_datum_get_gidx_p(newval, gidx_geom) == LW_FAILURE) {
		/*
		 * Empty entries have to be supported in the opclass: test
		 * the passed new value for emptiness; if it returns true, we
		 * need to set the "contains empty" flag in the element
		 * (unless already set).
		 */
		if (is_gserialized_from_datum_empty(newval)) {
			...
			/*
			 * in case the entry is not empty and it is not
			 * possible to retrieve a box, raise an error
			 */
	...
	if (column->bv_allnulls) {
		/*
		 * It's not safe to summarize geometries of different number
		 * of dimensions in the same range.  We therefore fix the
		 * number of dimension for this range by storing the bounding
		 * box of the first geometry found as is, being careful not
		 * to store more dimension than defined in the opclass.
		 */
		if (dims_geom > max_dims) {
			/*
			 * Diminush the varsize to only store the maximum
			 * number of dimensions allowed by the opclass
			 */
	...
	/*
	 * Mark the datum as unmergeable if its number of dimension is not
	 * the same as the one stored in the key of the current range
	 */
	...
	/*
	 * Check if the stored bounding box already contains the geometry's
	 * one
	 */
	...
	/*
	 * Otherwise, we need to enlarge the stored GIDX to make it contains
	 * the current geometry.  As we store a GIDX with a fixed number of
	 * dimensions, we just need adjust min and max
	 */
