...
/*
 * distance from -126 49  to -126 49.011096139863 in
 * 'SPHEROID["GRS_1980",6378137,298.257222101]' is 1234.000
 */
...
/*
 * Use the WKT definition of an ellipsoid ie. SPHEROID["name",A,rf] or
 * SPHEROID("name",A,rf) SPHEROID["GRS_1980",6378137,298.257222101] wkt says
 * you can use "(" or "["
 */
PG_FUNCTION_INFO_V1(ellipsoid_in);
Datum
ellipsoid_in(PG_FUNCTION_ARGS)
{
...
Datum
ellipsoid_out(PG_FUNCTION_ARGS)
{
...
/*
 * support function for distance calc code is taken from David Skea
 * Geographic Data BC, Province of British Columbia, Canada. Thanks to GDBC
 * and David Skea for allowing this to be put in PostGIS.
 */
double
		 ...
		 double lat2, double long2, SPHEROID * sphere)
{
	...
	if (result != result) {	/* NaN check (x==x for all x except NaN by
				 * IEEE definition) */
...
/*
 * Given 2 lat/longs and ellipse, find the distance note original r = 1st,
 * s=2nd location
 */
double
...
{
	/*
	 * Code is taken from David Skea Geographic Data BC, Province of
	 * British Columbia, Canada. Thanks to GDBC and David Skea for
	 * allowing this to be put in PostGIS.
	 */
	...
	do {
		...
		/*
		 * Patch from Patrica Tozer to handle minor mathematical
		 * stability problem
		 */
		...
		/*
		 * (old code?) tsm = acos(cosSigma - (2.0 * sinU1 *
		 * sinU2)/(cos(azimuthEQ)*cos(azimuthEQ)));
		 */
...
/*
 * Find the "length of a geometry" length2d_spheroid(point, sphere) = 0
 * length2d_spheroid(line, sphere) = length of line
 * length2d_spheroid(polygon, sphere) = 0 -- could make sense to return
 * sum(ring perimeter) uses ellipsoidal math to find the distance x's are
 * longitude, and y's are latitude - both in decimal degrees
 */
PG_FUNCTION_INFO_V1(LWGEOM_length2d_ellipsoid);
Datum
LWGEOM_length2d_ellipsoid(PG_FUNCTION_ARGS)
{
 ...
 *
 * length2d_spheroid(point, sphere) = 0 length2d_spheroid(line, sphere) =
 * length of line length2d_spheroid(polygon, sphere) = 0 -- could make sense
 * to return sum(ring perimeter) uses ellipsoidal math to find the distance
...
/*
 * For some lat/long points, the above method doesn't calculate the distance
 * very well. Typically this is for two lat/long points that are very very
 * close together (<10cm). This gets worse closer to the equator.
 *
 * This method works very well for very close together points, not so well if
 * they're far away (>1km).
 *
 * METHOD: We create two circles (with Radius R and Radius S) and use these
 * to calculate the distance.
 *
 * The first (R) is basically a (north-south) line of longitude. Its radius
 * is approximated by looking at the ellipse. Near the equator R = 'a'
 * (earth's major axis) near the pole R = 'b' (earth's minor axis).
 *
 * The second (S) is basically a (east-west) line of lattitude. Its radius
 * runs from 'a' (major axis) at the equator, and near 0 at the poles.
 *
 *
 * North pole *
 *
 ...
 * *
 *
 * South pole (side view of earth)
 *
 * Angle A is lat1 R is the distance from the centre of the earth to the
 * lat1/long1 point on the surface of the Earth. S is the
 * circle-of-lattitude.  Its calculated from the right triangle defined by
 * the angle (90-A), and the hypothenus R.
 *
 *
 *
 * Once R and S have been calculated, the actual distance between the two
 * points can be calculated.
 *
 * We dissolve the vector from lat1,long1 to lat2,long2 into its X and Y
 * components (called DeltaX,DeltaY). The actual distance that these
 * angle-based measurements represent is taken from the two circles we just
 * calculated; R (for deltaY) and S (for deltaX).
 *
 * (if deltaX is 1 degrees, then that distance represents 1/360 of a circle
 * of radius S.)
 *
 *
 * Parts taken from PROJ4 - geodetic_to_geocentric() (for calculating Rn)
 *
 * remember that lat1/long1/lat2/long2 are comming in a *RADIANS* not
 * degrees.
 *
...
Datum
geometry_distance_spheroid(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_distance_ellipsoid(PG_FUNCTION_ARGS)
{
