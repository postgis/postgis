...
/*
 * Does the object correctly model a trajectory ? Must be a LINESTRINGM with
 * growing measures
 */
Datum		ST_IsValidTrajectory(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(ST_IsValidTrajectory);
Datum
ST_IsValidTrajectory(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *gs0 = PG_GETARG_GSERIALIZED_P(0);
	/*
	 * All checks already performed by liblwgeom, not worth checking
...
/*
 * Return the distance between two trajectories at their closest point of
 * approach.
 */
Datum		ST_DistanceCPA(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(ST_DistanceCPA);
Datum
ST_DistanceCPA(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *gs0 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *gs1 = PG_GETARG_GSERIALIZED_P(1);
	/*
...
/*
 * Return true if the distance between two trajectories at their closest
 * point of approach is within the given max.
 */
Datum		ST_CPAWithin(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(ST_CPAWithin);
Datum
ST_CPAWithin(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *gs0 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *gs1 = PG_GETARG_GSERIALIZED_P(1);
