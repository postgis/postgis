...
/*
 * ST_DumpPoints for postgis. By Nathan Wagner, copyright disclaimed, this
 * entire file is in the public domain
 */
...
/*
 * 32 is the max depth for st_dump, so it seems reasonable to use the same
 * here
 */
#define MAXDEPTH 32
struct dumpstate {
	...
	int		pathlen;	/* polygon rings and such need extra
					 * path info */
	struct dumpnode	stack[MAXDEPTH];
	Datum		path[34];	/* two more than max depth, for ring
					 * and point */
...
LWGEOM_dumppoints(PG_FUNCTION_ARGS)
{
	...
	HeapTuple	tuple;
	Datum		pathpt[2];	/* used to construct the composite
					 * return value */
	bool		isnull[2] = {0, 0};	/* needed to say neither
						 * value is null */
	...
	while (1) {
		...
		if (!lwgeom_is_collection(lwgeom)) {
			...
			/*
			 * TODO use a union?  would save a tiny amount of
			 * stack space. probably not worth the bother
			 */
			LWLINE	       *line;
			LWCIRCSTRING   *circ;
			LWPOLY	       *poly;
			...
			/*
			 * net result of switch should be to set lwpoint to
			 * the next point to return, or leave at NULL if
			 * there are no more points in the geometry
			 */
			switch (lwgeom->type) {
			case TRIANGLETYPE:
			...
			case POLYGONTYPE:
				...
				if (state->ring == poly->nrings) {
					...
					/*
					 * TODO should be able to directly
					 * get the point into the point array
					 * of a fixed lwpoint
					 */
					/*
					 * can't get the point directly from
					 * the ptarray because it might be
					 * aligned wrong, so at least one
					 * memcpy seems unavoidable It might
					 * be possible to pass it directly to
					 * gserialized depending how that
					 * works, it might effectively be
					 * gserialized though a brief look at
					 * the code indicates not
					 */
			...
			/*
			 * At this point, lwpoint is either NULL, in which
			 * case we need to pop the geometry stack and get the
			 * next geometry, if amy, or lwpoint is set and we
			 * construct a record type with the integer array of
			 * geometry indexes and the point number, and the
			 * actual point geometry itself
			 */

			if (!lwpoint) {
				/*
				 * no point, so pop the geom and look for
				 * more
				 */
				if (--state->stacklen == 0)
					SRF_RETURN_DONE(funcctx);
		...
		if (node->idx < lwcoll->ngeoms) {
			...
			/*
			 * loop back to beginning, which will then check
			 * whatever node we just pushed
			 */
			continue;
		}
