...
is_gml_namespace(xmlNodePtr xnode, bool is_strict)
{
	...
	/*
	 * If no namespace is available we could return true anyway (because
	 * we work only on GML fragment, we don't want to 'oblige' to add
	 * namespace on the geometry root node)
	 */
	if (ns == NULL) {
		return !is_strict;
	}
	...
	/*
	 * Handle namespaces: - http://www.opengis.net/gml      (GML 3.1.1
	 * and priors) - http://www.opengis.net/gml/3.2  (GML 3.2.1)
	 */
	for (p = ns; *p; p++) {
		if ((*p)->href == NULL || (*p)->prefix == NULL ||
		    xnode->ns == NULL || xnode->ns->prefix == NULL)
			continue;
...
gml_is_srid_planar(int srid)
{
	...
	if (err < 0)
		lwpgerror("gml_is_srid_planar: error executing query %d", err);
...
parse_gml_srs(xmlNodePtr xnode, gmlSrs * srs)
{
	...
	if (!srsname) {
		...
		/*
		 * Severals	srsName formats are available... cf WFS 1.1.0
		 * -> 9.2 (p36) cf ISO 19142:2009 -> 7.9.2.4.4 (p34) cf RFC
		 * 5165 <http://tools.ietf.org/html/rfc5165> cf CITE WFS-1.1
		 * (GetFeature-tc17.2)
		 */

		/*
		 * SRS pattern like:   	EPSG:4326 urn:EPSG:geographicCRS:4326
...
parse_gml_double(char *d, bool space_before, bool space_after)
{
	...
	/*
	 * Double pattern [-|\+]?[0-9]+(\.)?([0-9]+)?([Ee](\+|-)?[0-9]+)? We
	 * could also meet spaces before and/or after this pattern upon
	 * parameters
	 */
...
static POINTARRAY * parse_gml_coordinates(xmlNodePtr xnode, bool *hasz){
	...
	/*
	 * Default GML coordinates pattern: 	x1,y1 x2,y2 x1,y1,z1 x2,y2,z2
	 *
	...
	for (q = p, gml_dims = 0, digit = false; *p; p++) {
		...
		if (*p == cs) {
			*p = '\0';
			...
			/*
			 * Need to put standard decimal separator to atof
			 * handle
			 */
		} else if (*p == dec && dec != '.')
			*p = '.';
...
static POINTARRAY * parse_gml_pos(xmlNodePtr xnode, bool *hasz){
	...
	/*
	 * gml:pos pattern: 	x1 y1 x1 y1 z1
	 */
...
static POINTARRAY * parse_gml_poslist(xmlNodePtr xnode, bool *hasz){
	...
	/*
	 * gml:posList pattern: 	x1 y1 x2 y2 x1 y1 z1 x2 y2 z2
	 */
...
static LWGEOM * parse_gml_curve(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (xa = xa->children, lss = 0; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "LineStringSegment"))
			continue;
	...
	/*
	 * "The curve segments are connected to one another, with the end
	 * point of each segment except the last being the start point of the
	 * next segment"  from  ISO 19107:2003 -> 6.3.16.1 (p43)
	 *
...
static LWGEOM * parse_gml_polygon(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (xa = xnode->children; xa != NULL; xa = xa->next) {
		...
		for (xb = xa->children; xb != NULL; xb = xb->next) {
			...
			if (!is_gml_namespace(xb, false))
				continue;
			if (strcmp((char *)xb->name, "LinearRing"))
				continue;
	...
	/*
	 * Found an <exterior> or <outerBoundaryIs> but no rings?!? We're
	 * outa here!
	 */
	if (!ppa)
		gml_lwpgerror("invalid GML representation", 43);
...
static LWGEOM * parse_gml_triangle(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	/*
	 * GML SF is resticted to planar interpolation NOTA: I know Triangle
	 * is not part of SF, but we have to be consistent with other
	 * surfaces
	 */
	...
	for (xa = xnode->children; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "exterior"))
			continue;
...
static LWGEOM * parse_gml_patch(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (ring = 1, xa = xnode->children; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "interior"))
			continue;
...
static LWGEOM * parse_gml_surface(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (patch = 0, xa = xa->children; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "PolygonPatch"))
			continue;
		patch++;

		/*
		 * SQL/MM define ST_CurvePolygon as a single patch only, cf
		 * ISO 13249-3:2009 -> 4.2.9 (p27)
		 */
		if (patch > 1)
...
static LWGEOM * parse_gml_mline(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (xa = xnode->children; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "lineStringMember"))
			continue;
		if (xa->children != NULL)
...
static LWGEOM * parse_gml_mpoly(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (xa = xnode->children; xa != NULL; xa = xa->next) {
		...
		if (!is_gml_namespace(xa, false))
			continue;
		if (strcmp((char *)xa->name, "polygonMember"))
			continue;
		if (xa->children != NULL)
...
static LWGEOM * parse_gml_coll(xmlNodePtr xnode, bool *hasz, int *root_srid){
	...
	for (xa = xnode->children; xa != NULL; xa = xa->next) {
		 ...
		 * polygonMember are parts of geometryMember substitution
		 * group
		 */
...
static LWGEOM * lwgeom_from_gml(const char *xml){
	 ...
	 *
	 * So we deal with 3D in all structures allocation, and flag hasz to
	 * false if we met once a missing Z dimension In this case, we force
	 * recursive 2D.
	 */
	if (!hasz) {
