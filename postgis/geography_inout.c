...
Datum
geography_out(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	char	       *hexwkb;
...
Datum
geography_as_gml(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	char	       *gml;
	...
	if (option & 1)
		lwopts |= LW_GML_IS_DEGREE;
	if (option & 2)
		lwopts &= ~LW_GML_IS_DIMS;
	if (option & 8) {
		elog(ERROR,
...
Datum
geography_as_kml(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g = NULL;
	LWGEOM	       *lwgeom = NULL;
	char	       *kml;
...
Datum
geography_as_svg(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g = NULL;
	LWGEOM	       *lwgeom = NULL;
	char	       *svg;
...
Datum
geography_as_geojson(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	char	       *geojson;
	...
	/*
	 * Retrieve output option 0 = without option (default) 1 = bbox 2 =
	 * short crs 4 = long crs
	 */
	if (PG_NARGS() > 3 && !PG_ARGISNULL(3))
		option = PG_GETARG_INT32(3);
	...
	if (option & 2 || option & 4) {
		...
		if (option & 2)
			srs = getSRSbySRID(SRID_DEFAULT, true);
		if (option & 4)
			srs = getSRSbySRID(SRID_DEFAULT, false);
...
Datum
geography_from_text(PG_FUNCTION_ARGS)
{
	LWGEOM_PARSER_RESULT lwg_parser_result;
	GSERIALIZED    *g_ser = NULL;
...
Datum
geography_from_binary(PG_FUNCTION_ARGS)
{
...
Datum
geography_from_geometry(PG_FUNCTION_ARGS)
{
...
Datum
geometry_from_geography(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	...
	/*
	 * We want "geometry" to think all our "geography" has an SRID, and
	 * the implied SRID is the default, so we fill that in if our SRID is
	 * actually unknown.
	 */
	if ((int)lwgeom->srid <= 0)
		lwgeom->srid = SRID_DEFAULT;
...
Datum
geography_recv(PG_FUNCTION_ARGS)
{
...
Datum
geography_send(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	size_t		size_result;
