 ...
 * Proceedings of the ACM SIGMOD Conference, pp 47-57, June 1984. * [2] C.-H.
 * Ang and T. C. Tan. New linear node splitting algorithm for *     R-Trees.
 * Advances in Spatial Databases - 5th International Symposium, *     1997 *
 * [3] N. Beckmann, H.-P. Kriegel, R. Schneider, B. Seeger. The R*tree: an *
 * efficient and robust access method for points and rectangles. *
 * Proceedings of the ACM SIGMOD Conference. June 1990. * [4] A. Korotkov, "A
 * new double sorting-based node splitting algorithm for R-tree", *
...
/*
 * * When is a node split not so good? If more than 90% of the entries * end
 * up in one of the children.
 */
#define LIMIT_RATIO 0.1

/*
 * * 0 == don't use it * 1 == use it
 */
...
/*
 * Enlarge b_union to contain b_new. If b_new contains more dimensions than
 * b_union, expand b_union to contain those dimensions.
 */
void
...
/*
 * Convert a double-based GBOX into a float-based BOX2DF, ensuring the float
 * box is larger than the double box
 */
static inline int
...
gserialized_datum_get_box2df_p(Datum gsdatum, BOX2DF * box2df)
{
	 ...
	 * enough to take serious advantage of PG_DETOAST_DATUM_SLICE will
	 * have * already been compressed, which means the entire object will
	 * be * fetched and decompressed before a slice is taken, thus
	 * removing * any efficiencies gained from slicing. We need to move
	 * to * "storage = external" and implement our own geometry
	 * compressor * before we can take advantage of sliced retrieval.
	 */
...
gserialized_datum_predicate_2d(Datum gs1, Datum gs2, box2df_predicate predicate)
{
	...
	if (gserialized_datum_get_box2df_p(gs1, &b1) == LW_SUCCESS)
		br1 = &b1;
	if (gserialized_datum_get_box2df_p(gs2, &b2) == LW_SUCCESS)
		br2 = &b2;
...
Datum
gserialized_contains_box2df_geom_2d(PG_FUNCTION_ARGS)
{
	POSTGIS_DEBUG(3, "entered function");
...
Datum
gserialized_contains_box2df_box2df_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_within_box2df_geom_2d(PG_FUNCTION_ARGS)
{
	POSTGIS_DEBUG(3, "entered function");
...
Datum
gserialized_within_box2df_box2df_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_overlaps_box2df_geom_2d(PG_FUNCTION_ARGS)
{
	POSTGIS_DEBUG(3, "entered function");
...
Datum
gserialized_overlaps_box2df_box2df_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_distance_centroid_2d(PG_FUNCTION_ARGS)
{
	BOX2DF		b1, b2;
...
Datum
gserialized_distance_box_2d(PG_FUNCTION_ARGS)
{
	BOX2DF		b1, b2;
	Datum		gs1 = PG_GETARG_DATUM(0);
	Datum		gs2 = PG_GETARG_DATUM(1);
...
Datum
gserialized_same_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_within_2d(PG_FUNCTION_ARGS)
{
	POSTGIS_DEBUG(3, "entered function");
...
Datum
gserialized_contains_2d(PG_FUNCTION_ARGS)
{
	POSTGIS_DEBUG(3, "entered function");
...
Datum
gserialized_overlaps_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_left_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_right_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_above_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_below_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_overleft_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_overright_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_overabove_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_overbelow_2d(PG_FUNCTION_ARGS)
{
 ...
 * bounding box. If the geography already has the box embedded in it * we
 * pull that out and hand it back.
 */
PG_FUNCTION_INFO_V1(gserialized_gist_compress_2d);
Datum
gserialized_gist_compress_2d(PG_FUNCTION_ARGS)
{
	...
	/*
	 * * Not a leaf key? There's nothing to do. * Return the input
	 * unchanged.
	 */
	if (!entry_in->leafkey) {
	...
	/*
	 * Is the bounding box valid (non-empty, non-infinite)? If not,
...
/*
 * * GiST support function. * Decompress an entry. Unused for geography, so
 * we return.
 */
PG_FUNCTION_INFO_V1(gserialized_gist_decompress_2d);
Datum
gserialized_gist_decompress_2d(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gist_consistent_2d(PG_FUNCTION_ARGS)
{
	...
	/*
	 * We set recheck to false to avoid repeatedly pulling every
	 * "possibly matched" geometry out during index scans. For cases when
	 * the geometries are large, rechecking can make things twice as
	 * slow.
	 */
	*recheck = false;
...
/*
 * * GiST support function. Take in a query and an entry and return the
 ...
 * with the smallest "distance" values first, so the results must be
 * consistent * with the operator's semantics. For a leaf index entry the
 * result just * represents the distance to the index entry; for an internal
 * tree node, the * result must be the smallest distance that any child entry
 * could have. *
 *
 * * Strategy 13 = true distance tests <-> * Strategy 14 = box-based distance
 * tests <#>
 */
PG_FUNCTION_INFO_V1(gserialized_gist_distance_2d);
Datum
gserialized_gist_distance_2d(PG_FUNCTION_ARGS)
{
	GISTENTRY      *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	BOX2DF		query_box;
	BOX2DF	       *entry_box;
	...
	/* True distance test (formerly centroid distance) */
		...
		/*
		 * the minimum possible distance, which is the
		 * box2df_distance
		 */
...
/*
 * * Function to pack floats of different realms * This function serves to
 * pack bit flags inside float type * Resulted value represent can be from
 * four different "realms" * Every value from realm 3 is greater than any
 * value from realms 2, 1 and 0. * Every value from realm 2 is less than
 * every value from realm 3 and greater * than any value from realm 1 and 0,
 * and so on. Values from the same realm * loose two bits of precision. This
 * technique is possible due to floating * point numbers specification
 * according to IEEE 754: exponent bits are highest * (excluding sign bits,
 * but here penalty is always positive). If float a is * greater than float
 * b, integer A with same bit representation as a is greater * than integer B
 * with same bits as b.
 */
static float
...
/*
 * * GiST support function. Calculate the "penalty" cost of adding this entry
 * into an existing entry. * Calculate the change in volume of the old entry
 * once the new entry is added.
 */
PG_FUNCTION_INFO_V1(gserialized_gist_penalty_2d);
Datum
gserialized_gist_penalty_2d(PG_FUNCTION_ARGS)
{
...
/*
 * * GiST support function. Merge all the boxes in a page into one master
 * box.
 */
PG_FUNCTION_INFO_V1(gserialized_gist_union_2d);
Datum
gserialized_gist_union_2d(PG_FUNCTION_ARGS)
{
	GistEntryVector *entryvec = (GistEntryVector *) PG_GETARG_POINTER(0);
	int	       *sizep = (int *)PG_GETARG_POINTER(1);	/* Size of the return
								 * value */
	int		numranges, i;
...
Datum
gserialized_gist_same_2d(PG_FUNCTION_ARGS)
{
...
/*
 * Trivial split: half of entries will be placed on one page and another half
 * - to another
 */
static void
 ...
 */
typedef struct {
	int		entriesCount;	/* total number of entries being
					 * split */
	BOX2DF		boundingBox;	/* minimum bounding box across all
					 * entries */
		     ...
		     float leftUpper, int maxLeftCount)
{
	...
	/*
	 * Calculate entries distribution ratio assuming most uniform
	...
	if (ratio > LIMIT_RATIO) {
		...
		/*
		 * The ratio is acceptable, so compare current split with
		 * previously selected one. Between splits of one dimension
		 * we search for minimal overlap (allowing negative values)
		 * and minimal ration (between same overlaps. We switch
		 * dimension if find less overlap (non-negative) or less
		 * range with same overlap.
		 */
		if (dimNum == 0)
		...
		if (context->first)
			...
			/*
			 * Within the same dimension, choose the new split if
			 * it has a smaller overlap, or same overlap but
			 * better ratio.
			 */
			...
			/*
			 * Across dimensions, choose the new split if it has
			 * a smaller *non-negative* overlap, or same
			 * *non-negative* overlap but bigger range. This
			 * condition differs from the one described in the
			 * article. On the datasets where leaf MBRs don't
			 * overlap themselves, non-overlapping splits (i.e.
			 * splits which have zero *non-negative* overlap) are
			 * frequently possible. In this case splits tends to
			 * be along one dimension, because most distant
			 * non-overlapping splits (i.e. having lowest
			 * negative overlap) appears to be in the same
			 * dimension as in the previous split. Therefore MBRs
			 * appear to be very prolonged along another
			 * dimension, which leads to bad search performance.
			 * Using range as the second split criteria makes
			 * MBRs more quadratic. Using *non-negative* overlap
			 * instead of overlap as the first split criteria
			 * gives to range criteria a chance to matter,
			 * because non-overlapping splits are equivalent in
			 * this criteria.
			 */
 ...
 *
 * 1) Entries which should be placed to the left group 2) Entries which
 * should be placed to the right group 3) "Common entries" which can be
 * placed to any of groups without affecting of overlap along selected axis.
 *
...
Datum
gserialized_gist_picksplit_2d(PG_FUNCTION_ARGS)
{
	GistEntryVector *entryvec = (GistEntryVector *) PG_GETARG_POINTER(0);
	...
	for (dim = 0; dim < 2; dim++) {
		...
		/*
		 * Make two arrays of intervals: one sorted by lower bound
		 * and another sorted by upper bound.
		 */
		...
		/*
		 * Iterate over lower bound of right group, finding smallest
		 * possible upper bound of left group.
		 */
		i1 = 0;
		i2 = 0;
		...
		while (true) {
			...
			/*
			 * Find count of intervals which anyway should be
			 * placed to the left group.
			 */
		 ...
		 * possible lower bound of right group.
		 */
		i1 = nentries - 1;
		i2 = nentries - 1;
	 ...
	 *
	 * While considering the splits, we already determined that there
	 * will be enough entries in both groups to reach the desired ratio,
	 * but we did not memorize which entries go to which group. So
	 * determine that now.
	 */
	...
	/*
	 * Allocate an array for "common entries" - entries which can be
	 * placed to either group without affecting overlap along selected
	 * axis.
	 */
...
#define PLACE_RIGHT(box, off)					\
	...
	/*
	 * Distribute entries which can be distributed unambiguously, and
	...
	for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i)) {
		...
		if (upper <= context.leftUpper || isnan(upper)) {
			...
			if (lower >= context.rightLower || isnan(lower)) {
				/*
				 * Fits also to the right group, so "common
				 * entry"
				 */
				commonEntries[commonEntriesCount++].index = i;
			} else {
				/*
				 * Doesn't fit to the right group, so join to
				 * the left group
				 */
				PLACE_LEFT(box, i);
			}
			...
			/*
			 * Each entry should fit on either left or right
			 * group. Since this entry didn't fit on the left
			 * group, it better fit in the right group.
			 */
			Assert(lower >= context.rightLower);

			/*
			 * Doesn't fit to the left group, so join to the
			 * right group
			 */
			PLACE_RIGHT(box, i);
		}
	...
	if (commonEntriesCount > 0) {
		/*
		 * Calculate minimum number of entries that must be placed in
		 * both groups, to reach LIMIT_RATIO.
		 */
		int		m = ceil(LIMIT_RATIO * (double)nentries);

		/*
		 * Calculate delta between penalties of join "common entries"
		 * to different groups.
		 */
		...
		/*
		 * Sort "common entries" by calculated deltas in order to
		 * distribute the most ambiguous entries first.
		 */
		...
		for (i = 0; i < commonEntriesCount; i++) {
			...
			/*
			 * Check if we have to place this entry in either
			 * group to achieve LIMIT_RATIO.
			 */
			if (v->spl_nleft + (commonEntriesCount - i) <= m)
				...
				/*
				 * Otherwise select the group by minimal
				 * penalty
				 */
...
/*
 * * The BOX32DF key must be defined as a PostgreSQL type, even though it is
 * only * ever used internally. These no-op stubs are used to bind the type.
 */
PG_FUNCTION_INFO_V1(box2df_in);
Datum
box2df_in(PG_FUNCTION_ARGS)
{
