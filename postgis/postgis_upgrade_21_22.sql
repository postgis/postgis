...
--
-- UPGRADE SCRIPT FROM PostGIS 2.0 TO PostGIS 2.2
--
...
CREATE OR REPLACE FUNCTION GeomFromEWKB(bytea)
	RETURNS geometry
	AS '$libdir/postgis-2.2','LWGEOMFromWKB'
	LANGUAGE 'c' IMMUTABLE STRICT;
...
CREATE OR REPLACE FUNCTION postgis_scripts_installed() RETURNS text
	AS $$ SELECT '2.2.0dev'::text || ' r' || 12069::text AS version $$
...
CREATE OR REPLACE FUNCTION postgis_scripts_build_date() RETURNS text
	AS 'SELECT ''2013-10-30 17:43:00''::text AS version'
	LANGUAGE 'sql' IMMUTABLE;
...
CREATE OR REPLACE FUNCTION pgis_twkb_accum_finalfn(internal)
  RETURNS bytea
	AS '$libdir/postgis-2.2'
  LANGUAGE c ;
DROP AGGREGATE IF EXISTS st_astwkb_agg(geometry,int);
CREATE AGGREGATE st_astwkb_agg(geometry,int) (
  SFUNC=pgis_twkb_accum_transfn,
  STYPE=internal,
  FINALFUNC=pgis_twkb_accum_finalfn
);
DROP AGGREGATE IF EXISTS st_astwkb_agg(geometry,int,int8);
