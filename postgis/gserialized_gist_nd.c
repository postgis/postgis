...
/*
 * An "unknown" GIDX is used to represent the bounds of an EMPTY geometry or
 * other-wise unindexable geometry (like one with NaN or Inf bounds)
 */
inline bool
...
/*
 * Calculate the edge of the union of the boxes. Avoids creating an
 * intermediate box.
 */
static float
 ...
 *
 * * Box(A) Overlaps Box(B) IFF for every dimension d: *   min(A,d) <=
 * max(B,d) && max(A,d) => min(B,d) *
 *
 * * Any missing dimension is assumed by convention to * overlap whatever
 * finite range available on the * other operand. See *
...
gidx_overlaps(GIDX * a, GIDX * b)
{
	...
	/*
	 * compare only up to dimensions of (b), missing dimensions always
	 * overlap
	 */
	for (i = 0; i < ndims_b; i++) {
...
gidx_contains(GIDX * a, GIDX * b)
{
	...
	if (dims_a < dims_b) {
		/*
		 * * If (b) is of higher dimensionality than (a) it can only
		 * be contained * if those higher dimensions are zeroes.
		 */
	...
	/*
	 * Excess dimensions of (a), don't matter, it just has to contain (b)
	 * in (b)'s dimensions
	 */
...
gserialized_datum_predicate(Datum gs1, Datum gs2, gidx_predicate predicate)
{
	...
	/*
	 * Must be able to build box for each arguement (ie, not empty
	 * geometry) and predicate function to return true.
	 */
...
gserialized_datum_predicate_gidx_geom(GIDX * gidx1, Datum gs2, gidx_predicate predicate)
{
	...
	/*
	 * Must be able to build box for gs2 arguement (ie, not empty
	 * geometry) and predicate function to return true.
	 */
...
gidx_distance_leaf_centroid(const GIDX * a, const GIDX * b)
{
	...
	for (i = 0; i < ndims; ++i) {
		...
		sum += d * d;
		/*
		 * POSTGIS_DEBUGF(3, " centroid of A for dimension %d is %g",
		 * i, ca); POSTGIS_DEBUGF(3, " centroid of B for dimension %d
		 * is %g", i, cb); POSTGIS_DEBUGF(3, " distance on dimension
		 * %d is %g, squared as %g, grows sum to %g", i, d, d*d,
		 * sum);
		 */
	}
	return sqrt(sum);
...
/*
 * Do centroid to centroid n-d distance if you don't have re-check available
 * (PgSQL 9.5+), do "real" n-d distance if you do
 */
PG_FUNCTION_INFO_V1(gserialized_distance_nd);
Datum
gserialized_distance_nd(PG_FUNCTION_ARGS)
{
...
/*
 * * '~' and operator function. Based on two serialized return true if * the
 * first is contained by the second.
 */
PG_FUNCTION_INFO_V1(gserialized_within);
Datum
gserialized_within(PG_FUNCTION_ARGS)
{
...
#if POSTGIS_PGSQL_VERSION > 94
/*
 * * '~' and operator function. Based on a GIDX and a serialized return true
 * if * the first is contained by the second.
 */
PG_FUNCTION_INFO_V1(gserialized_gidx_geom_within);
Datum
gserialized_gidx_geom_within(PG_FUNCTION_ARGS)
{
...
/*
 * * '~' and operator function. Based on two GIDX return true if * the first
 * is contained by the second.
 */
PG_FUNCTION_INFO_V1(gserialized_gidx_gidx_within);
Datum
gserialized_gidx_gidx_within(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_contains(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gidx_geom_contains(PG_FUNCTION_ARGS)
{
	GIDX	       *gidx = (GIDX *) PG_GETARG_POINTER(0);
...
Datum
gserialized_gidx_gidx_contains(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gidx_geom_same(PG_FUNCTION_ARGS)
{
	GIDX	       *gidx = (GIDX *) PG_GETARG_POINTER(0);
...
Datum
gserialized_gidx_gidx_same(PG_FUNCTION_ARGS)
{
...
/*
 * * '&&' operator function. Based on two serialized return true if * they
 * overlap and false otherwise.
 */
PG_FUNCTION_INFO_V1(gserialized_overlaps);
Datum
gserialized_overlaps(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gidx_geog_overlaps(PG_FUNCTION_ARGS)
{
	GIDX	       *gidx = (GIDX *) PG_GETARG_POINTER(0);
...
Datum
gserialized_gidx_geom_overlaps(PG_FUNCTION_ARGS)
{
	GIDX	       *gidx = (GIDX *) PG_GETARG_POINTER(0);
...
Datum
gserialized_gidx_gidx_overlaps(PG_FUNCTION_ARGS)
{
...
gserialized_gist_compress(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Is the bounding box valid (non-empty, non-infinite) ? If not, use
	 * the "unknown" GIDX.
	 */
	...
	/*
	 * Check all the dimensions for finite values. If not, use the
	 * "unknown" GIDX as a key
	 */
...
Datum
gserialized_gist_decompress(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gist_consistent(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gist_penalty(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Calculate the size difference of the boxes (volume difference in
	 * this case).
	 */
...
Datum
gserialized_gist_union(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gist_same(PG_FUNCTION_ARGS)
{
...
Datum
gserialized_gist_geog_distance(PG_FUNCTION_ARGS)
{
	GISTENTRY      *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	...
	/*
	 * We are using '13' as the gist geography distance <-> strategy
	 * number
	 */
	if (strategy != 13) {
 ...
 * could have. *
 *
 * * Strategy 13 is centroid-based distance tests <<->> * Strategy 20 is cpa
 * based distance tests |=|
 */
PG_FUNCTION_INFO_V1(gserialized_gist_distance);
Datum
gserialized_gist_distance(PG_FUNCTION_ARGS)
{
	GISTENTRY      *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	StrategyNumber	strategy = (StrategyNumber) PG_GETARG_UINT16(2);
	char		query_box_mem[GIDX_MAX_SIZE];
...
/*
 * * Where the picksplit algorithm cannot find any basis for splitting one
 * way * or another, we simply split the overflowing node in half.
 */
static void
...
static void
gserialized_gist_picksplit_constructsplit(GIST_SPLITVEC * v, OffsetNumber * list1, int nlist1, GIDX * *union1, OffsetNumber * list2, int nlist2, GIDX * *union2)
{
 ...
 * Uses linear algorithm from Ang & Tan [2], dividing node extent into * four
 * quadrants, and splitting along the axis that most evenly distributes *
 * entries between the new nodes. * TODO: Re-evaluate this in light of R*Tree
 * picksplit approaches.
 */
PG_FUNCTION_INFO_V1(gserialized_gist_picksplit);
Datum
gserialized_gist_picksplit(PG_FUNCTION_ARGS)
{
	...
	/*
	 * * First calculate the bounding box and maximum number of
	 * dimensions in this page.
	 */
	...
	/*
	 * Calculate the containing box (box_pageunion) for the whole page we
	 * are going to split.
	 */
	...
	/*
	 * Every box in the page is the same! So, we split and just put half
	 * the boxes in each child.
	 */
	...
	/*
	 * * Assign each entry in the node to the volume partitions it
	 * belongs to, * such as "above the x/y plane, left of the y/z plane,
	 * below the x/z plane". * Each entry thereby ends up in three of the
	 * six partitions.
	 */
	...
	/*
	 * * "Bad disposition", too many entries fell into one octant of the
	 * space, so no matter which * plane we choose to split on, we're
	 * going to end up with a mostly full node. Where the * data is
	 * pretty homogeneous (lots of duplicates) entries that are
	 * equidistant from the * sides of the page union box can
	 * occasionally all end up in one place, leading * to this condition.
	 */
	if (gserialized_gist_picksplit_badratios(pos, ndims_pageunion) == true) {
		/*
		 * * Instead we split on center points and see if we do
		 * better. * First calculate the average center point for
		 * each axis.
		 */
		...
		for (d = 0; d < ndims_pageunion; d++) {
			...
			pos[BELOW(d)] = pos[ABOVE(d)] = 0;	/* Re-initialize our
								 * counters. */
		...
		/*
		 * Do we have a good disposition now? If not, screw it, just
		 * cut the node in half.
		 */
		if (gserialized_gist_picksplit_badratios(pos, ndims_pageunion) == true) {
	 ...
	 * apportioned entries * against the same number of total entries,
	 * the axis that has the smallest maximum * number of entries in its
	 * regions is the most evenly distributed. * TODO: what if the
	 * distributions are equal in two or more axes?
	 */
	for (d = 0; d < ndims_pageunion; d++) {
...
/*
 * * The GIDX key must be defined as a PostgreSQL type, even though it is
