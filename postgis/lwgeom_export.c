...
/*
 * Retrieve an SRS from a given SRID Require valid spatial_ref_sys table
 * entry
 *
 * Could return SRS as short one (i.e EPSG:4326) or as long one: (i.e
...
/*
 * Retrieve an SRID from a given SRS Require valid spatial_ref_sys table
 * entry
 *
 */
int
...
Datum
LWGEOM_asGeoJson_old(PG_FUNCTION_ARGS)
{
	switch (PG_NARGS()) {
	case 2:
...
Datum
LWGEOM_asEncodedPolyline(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *lwgeom;
