 ...
 *
 * Prints error message only if it was not for interruption, in which case we
 * let PostgreSQL deal with the error.
 */
...
/*
 * utility function that checks a LWPOINT and a GSERIALIZED poly against a
 * cache.  Serialized poly may be a multipart.
 */
static int
...
{
	/*
	 * * For GEOS >= 3.3, use the new UnaryUnion functionality to merge
	 * the * terminal collection from the ST_Union aggregate
	 */
	ArrayType      *array;
...
#pragma GCC diagnostic pop
	...
	/*
	 * * Collect the non-empty inputs and stuff them into a GEOS
	 * collection
	 */
	geoms_size = nelems;
	...
	/*
	 * * We need to convert the array of GSERIALIZED into a GEOS
	 * collection. * First make an array of GEOS geometries.
	 */
#if POSTGIS_PGSQL_VERSION >= 95
	iterator = array_create_iterator(array, 0, NULL);
#else
	...
	/* No real geometries in our array, any empties? */
		...
		/*
		 * If it was only empties, we'll return the largest type
		 * number
		 */
		if (empty_type > 0) {
...
Datum
geos_geomunion(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	GSERIALIZED    *result;
...
convexhull(PG_FUNCTION_ARGS)
{
	...
	if (gserialized_get_gbox_p(geom1, &bbox)) {
		/*
		 * Force the box to have the same dimensionality as the
		 * lwgeom
		 */
	...
	if (gserialized_is_empty(geom1)) {
		lwg = lwpoly_as_lwgeom(lwpoly_construct_empty(
						gserialized_get_srid(geom1),
							      0, 0));
		//buffer wouldn 't give back z or m anyway
		...
		for (param = params;; param = NULL) {
			...
			*val = '\0';
			...
			if (!strcmp(key, "endcap")) {
				/*
				 * Supported end cap styles: "round", "flat",
				 * "square"
				 */
				if (!strcmp(val, "round")) {
...
/*
 * Generate a field of random points within the area of a polygon or
 * multipolygon. Throws an error for other geometry types.
 */
...
ST_OffsetCurve(PG_FUNCTION_ARGS)
{
	...
	/*
	 * For distance == 0, just return the input. Note that due to a bug,
	 * GEOS 3.3.0 would return EMPTY. See
	 * http://trac.osgeo.org/geos/ticket/454
	 */
	if (size == 0)
...
Datum
geos_intersection(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	GSERIALIZED    *result;
...
Datum
geos_difference(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	GSERIALIZED    *result;
...
centroid(PG_FUNCTION_ARGS)
{
	...
	if (type == CIRCSTRINGTYPE || type == COMPOUNDTYPE) {
		...
		PG_FREE_IF_COPY(geom, 0);	/* free memory, we already
						 * have a lwgeom geometry
						 * copy */
...
isvalid(PG_FUNCTION_ARGS)
{
	...
	if (!g1) {
		/*
		 * should we drop the following notice now that we have
		 * ST_isValidReason ?
		 */
...
/*
 * * IsValidReason is only available in the GEOS * C API > version 3.0
 */
...
/*
 * * IsValidDetail is only available in the GEOS * C API >= version 3.3
 */
...
isvaliddetail(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = NULL;
	const		GEOSGeometry *g1 = NULL;
	char	       *values[3];	/* valid bool, reason text, location
					 * geometry */
	...
	/*
	 * Build a tuple description for a valid_detail tuple
	 */
...
overlaps(PG_FUNCTION_ARGS)
{
	...
	/*
	 * short-circuit 1: if geom2 bounding box does not overlap geom1
	 * bounding box we can prematurely return FALSE.
	 */
	...
	if (is_poly(geom1) && is_point(geom2)) {
		...
		if (gserialized_get_type(gpoint) == POINTTYPE) {
			...
			for (i = 0; i < mpoint->ngeoms; i++) {
				/*
				 * We need to find at least one point that's
				 * completely inside the polygons (pip_result
				 * == 1).  As long as we have one point
				 * that's completely inside, we can have as
				 * many as we want on the boundary itself.
				 * (pip_result == 0)
				 */
...
Datum
containsproperly(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	bool		result;
...
covers(PG_FUNCTION_ARGS)
{
	...
	/*
	 * short-circuit 2: if geom2 is a point and geom1 is a polygon call
...
coveredby(PG_FUNCTION_ARGS)
{
	...
	/*
	 * short-circuit 2: if geom1 is a point and geom2 is a polygon call
	 * the point-in-polygon function.
	 */
...
Datum
geos_intersects(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	int		result;
	...
	/*
	 * short-circuit 2: if the geoms are a point and a polygon, call the
	 * point_outside_polygon function.
	 */
...
disjoint(PG_FUNCTION_ARGS)
{
	...
	/*
	 * short-circuit 1: if geom2 bounding box does not overlap geom1
	 * bounding box we can prematurely return TRUE.
	 */
	if (gserialized_get_gbox_p(geom1, &box1) &&
	    gserialized_get_gbox_p(geom2, &box2)) {
		if (gbox_overlaps_2d(&box1, &box2) == LW_FALSE) {
...
Datum
relate_pattern(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom1;
	GSERIALIZED    *geom2;
	char	       *patt;
	...
	/*
	 * * Need to make sure 't' and 'f' are upper-case before handing to
	 * GEOS
	 */
...
ST_Equals(PG_FUNCTION_ARGS)
{
	...
	/*
	 * short-circuit: If geom1 and geom2 do not have the same bounding
	 * box we can return FALSE.
	 */
	if (gserialized_get_gbox_p(geom1, &box1) &&
	    gserialized_get_gbox_p(geom2, &box2)) {
	...
	/*
	 * short-circuit: if geom1 and geom2 are binary-equivalent, we can
	 * return TRUE.  This is much faster than doing the comparison using
	 * GEOS.
	 */
...
/*
 * ARRAY2LWGEOM: Converts the non-null elements of a Postgres array into a
 * LWGEOM* array
 */
LWGEOM	      **
...
/*
 * ARRAY2GEOS: Converts the non-null elements of a Postgres array into a
 * GEOSGeometry* array
 */
GEOSGeometry  **
...
clusterintersecting_garray(PG_FUNCTION_ARGS)
{
	...
	pfree(geos_inputs);	/* don't need to destroy items because
				 * GeometryCollections have taken ownership */
...
cluster_within_distance_garray(PG_FUNCTION_ARGS)
{
	...
	pfree(lw_inputs);	/* don't need to destroy items because
				 * GeometryCollections have taken ownership */
 ...
 * Actually a wrapper around GEOSpolygonize, transforming the resulting
 * collection into a valid polygon Geometry.
 */
...
/*
 * Take the vertices of a geometry and builds Delaunay triangles around them.
 */
 ...
 * ST_Split
 *
 * Split polygon by line, line by line, line by point. Returns at most
 * components as a collection. First element of the collection is always the
 * part which remains after the cut, while the second element is the part
 * which has been cut out. We arbitrarely take the part on the *right* of cut
 * lines as the part which has been cut out. For a line cut by a point the
 * part which remains is the one from start of the line to the cut point.
 *
 *
 * Author: Sandro Santilli <strk@kbt.io>
 *
 * Work done for Faunalia (http://www.faunalia.it) with fundings from Regione
 * Toscana - Sistema Informativo per il Governo del Territorio e
 * dell'Ambiente (RT-SIGTA).
 *
