...
typedef struct {
	uint32_t	cluster_id;
	char		is_null;	/* NULL may result from a NULL
					 * geometry input, or it may be used
					 * by algorithms such as DBSCAN that
					 * do not assign all inputs to a
					 * cluster. */
...
read_lwgeom_from_partition(WindowObject win_obj, uint32_t i, bool *is_null){
	...
	if (*is_null) {
		/*
		 * So that the indexes in our clustering input array can
		 * match our partition positions, toss an empty point into
		 * the clustering inputs, as a pass-through. NOTE: this will
		 * cause gaps in the output cluster id sequence.
		 */
...
ST_ClusterDBSCAN(PG_FUNCTION_ARGS)
{
	...
	if (row == 0) {		/* beginning of the partition; do all of the
				 * work now */
		uint32_t	i;
		uint32_t       *result_ids;
		LWGEOM	      **geoms;
...
Datum
ST_ClusterKMeans(PG_FUNCTION_ARGS)
{
	...
	if (!context->isdone) {
		...
		/*
		 * Read all the geometries from the partition window into a
		 * list
		 */
