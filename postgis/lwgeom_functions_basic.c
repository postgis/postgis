...
Datum
LWGEOM_mem_size(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_summary(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	char	       *result;
	text	       *mytext;
...
Datum
postgis_version(PG_FUNCTION_ARGS)
{
...
Datum
postgis_liblwgeom_version(PG_FUNCTION_ARGS)
{
...
Datum
postgis_lib_version(PG_FUNCTION_ARGS)
{
...
Datum
postgis_svn_version(PG_FUNCTION_ARGS)
{
...
Datum
postgis_lib_build_date(PG_FUNCTION_ARGS)
{
...
Datum
postgis_scripts_released(PG_FUNCTION_ARGS)
{
	char		ver[64];
	text	       *result;
...
Datum
postgis_uses_stats(PG_FUNCTION_ARGS)
{
	PG_RETURN_BOOL(true);
}
...
Datum
postgis_autocache_bbox(PG_FUNCTION_ARGS)
{
...
Datum
postgis_libxml_version(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_npoints(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
...
Datum
LWGEOM_nrings(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
	int		nrings = 0;
...
Datum
LWGEOM_area_polygon(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
	double		area = 0.0;
...
Datum
LWGEOM_length2d_linestring(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
...
Datum
LWGEOM_perimeter_poly(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
...
Datum
LWGEOM_perimeter2d_poly(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
	double		perimeter = 0.0;
...
Datum
LWGEOM_force_2d(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_force_3dz(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *pg_geom_in = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *pg_geom_out;
	LWGEOM	       *lwg_in, *lwg_out;
...
Datum
LWGEOM_force_3dm(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *pg_geom_in = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *pg_geom_out;
	LWGEOM	       *lwg_in, *lwg_out;
...
Datum
LWGEOM_force_4d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *pg_geom_in = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *pg_geom_out;
	LWGEOM	       *lwg_in, *lwg_out;
...
Datum
LWGEOM_force_collection(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *result;
	LWGEOM	      **lwgeoms;
	...
	/*
	 * This funx is a no-op only if a bbox cache is already present in
	 * input. If bbox cache is not there we'll need to handle automatic
	 * bbox addition FOR_COMPLEX_GEOMS.
	 */
...
Datum
LWGEOM_force_multi(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *result;
	LWGEOM	       *lwgeom;
...
Datum
LWGEOM_force_curve(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *result;
	LWGEOM	       *lwgeom;
...
Datum
LWGEOM_force_sfs(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *result;
	LWGEOM	       *lwgeom;
...
Datum
LWGEOM_closestpoint(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
...
Datum
LWGEOM_shortestline2d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_longestline2d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_mindistance2d(PG_FUNCTION_ARGS)
{
	double		mindist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
	...
	/*
	 * if called with empty geometries the ingoing mindistance is
	 * untouched, and makes us return NULL
	 */
	if (mindist < FLT_MAX)
...
LWGEOM_dwithin(PG_FUNCTION_ARGS)
{
	 ...
	 * underlying functions should be FLT_MAX which causes false as
	 * answer
	 */
...
Datum
LWGEOM_maxdistance2d_linestring(PG_FUNCTION_ARGS)
{
	double		maxdist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_closestpoint3d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_shortestline3d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_longestline3d(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_mindistance3d(PG_FUNCTION_ARGS)
{
	double		mindist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_dwithin3d(PG_FUNCTION_ARGS)
{
	double		mindist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_dfullywithin3d(PG_FUNCTION_ARGS)
{
	double		maxdist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_maxdistance3d(PG_FUNCTION_ARGS)
{
	double		maxdist;
	GSERIALIZED    *geom1 = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *geom2 = PG_GETARG_GSERIALIZED_P(1);
...
Datum
LWGEOM_inside_circle_point(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
...
Datum
LWGEOM_collect(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_collect_garray(PG_FUNCTION_ARGS)
{
	ArrayType      *array;
	int		nelems;
	/* GSERIALIZED **geoms; */
...
Datum
LWGEOM_line_from_mpoint(PG_FUNCTION_ARGS)
{
...
LWGEOM_makeline_garray(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Deserialize all point geometries in array into the geoms pointers
	 * array. Count actual number of points.
	 */
...
Datum
LWGEOM_makeline(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_makepoly(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *pglwg1;
...
Datum
LWGEOM_expand(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
...
Datum
LWGEOM_envelope(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
	...
	/*
	 * Alter envelope type so that a valid geometry is always returned
	 * depending upon the size of the geometry. The code makes the
	 * following assumptions: - If the bounding box is a single point
	 * then return a POINT geometry - If the bounding box represents
	 * either a horizontal or vertical line, return a LINESTRING geometry
...
Datum
LWGEOM_isempty(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(geom);
...
Datum
LWGEOM_segmentize2d(PG_FUNCTION_ARGS)
{
	...
	if (dist <= 0) {
		...
		/*
		 * Note that we'll end out of memory anyway for other small
		 * distances
		 */
	...
	//lwgeom_free(outlwgeom);	/* TODO fix lwgeom_clone /
					 * ptarray_clone_deep for consistent
					 * semantics */
	lwgeom_free(inlwgeom);
...
Datum
LWGEOM_reverse(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *lwgeom;
...
Datum
LWGEOM_force_clockwise_poly(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_zmflag(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *in;
	int		ret = 0;
...
Datum
LWGEOM_hasBBOX(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *in = PG_GETARG_GSERIALIZED_P(0);
...
Datum
ST_MakeEnvelope(PG_FUNCTION_ARGS)
{
	LWPOLY	       *poly;
	GSERIALIZED    *result;
	double		x1, y1, x2, y2;
...
Datum
ST_IsCollection(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	int		type;
	size_t		size;
...
Datum
LWGEOM_makepoint(PG_FUNCTION_ARGS)
{
	double		x, y, z, m;
	LWPOINT	       *point;
	GSERIALIZED    *result;
...
Datum
LWGEOM_makepoint3dm(PG_FUNCTION_ARGS)
{
	double		x, y, m;
	LWPOINT	       *point;
	GSERIALIZED    *result;
...
Datum
LWGEOM_removepoint(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_setpoint_linestring(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *pglwg1, *pglwg2, *result;
	LWGEOM	       *lwg;
	LWLINE	       *line;
...
Datum
LWGEOM_azimuth(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWPOINT	       *lwpoint;
	POINT2D		p1, p2;
...
LWGEOM_angle(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < n_args; i++) {
		...
		if (!getPoint2d_p(lwpoint->point, 0, &points[i])) {
			/*
			 * // can't free serialized geom, it might be needed
			 * by lw for (j=0;j<n_args;j++)
			 * PG_FREE_IF_COPY(seri_geoms[j], j);
			 */
			lwpgerror("Error extracting point");
			PG_RETURN_NULL();
	...
	/*
	 * // can't free serialized geom, it might be needed by lw for
	 * (j=0;j<n_args;j++) PG_FREE_IF_COPY(seri_geoms[j], j);
	 */

	/*
	 * compute azimuth for the 2 pairs of points note that angle is not
	 * defined identically for 3 points or 4 points
	 */
	if (n_args == 3) {
	...
	result = az2 - az1;
	result += (result < 0) * 2 * M_PI;	/* we dont want negative
						 * angle */
	PG_RETURN_FLOAT8(result);
}
...
/*
 * optimistic_overlap(Polygon P1, Multipolygon MP2, double dist) returns true
 * if P1 overlaps MP2 method: bbox check - is separation < dist?  no - return
 * false (quick) yes  - return distance(P1,MP2) < dist
 */
PG_FUNCTION_INFO_V1(optimistic_overlap);
Datum
optimistic_overlap(PG_FUNCTION_ARGS)
{
	...
	/*
	 * compute distances should be a fast calc if they actually do
	 * intersect
	 */
...
Datum
LWGEOM_affine(PG_FUNCTION_ARGS)
{
...
Datum
ST_CollectionExtract(PG_FUNCTION_ARGS)
{
...
Datum
ST_CollectionHomogenize(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *input = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *output;
	LWGEOM	       *lwgeom = lwgeom_from_gserialized(input);
...
Datum
ST_RemoveRepeatedPoints(PG_FUNCTION_ARGS)
{
...
Datum
ST_SwapOrdinates(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *in;
	GSERIALIZED    *out;
	LWGEOM	       *lwgeom;
...
Datum
ST_BoundingDiagonal(PG_FUNCTION_ARGS)
{
...
ST_Scale(PG_FUNCTION_ARGS)
{
	...
	if (!FLAGS_GET_Z(lwpoint->flags))
		factors.z = 1;
	if (!FLAGS_GET_M(lwpoint->flags))
		factors.m = 1;
