...
Datum
LWGEOM_snaptogrid(PG_FUNCTION_ARGS)
{
	LWGEOM	       *in_lwgeom;
...
Datum
LWGEOM_snaptogrid_pointoff(PG_FUNCTION_ARGS)
{
 ...
 *
 * * Determines crossing direction of line2 relative to line1. * Only accepts
 * LINESTRING as parameters!
 */
PG_FUNCTION_INFO_V1(ST_LineCrossingDirection);
Datum
ST_LineCrossingDirection(PG_FUNCTION_ARGS)
{
	int		type1, type2, rv;
	LWLINE	       *l1 = NULL;
	LWLINE	       *l2 = NULL;
...
LWGEOM_line_substring(PG_FUNCTION_ARGS)
{
	...
	if (type == LINETYPE) {
		...
		LWGEOM	      **geoms = NULL;
		double		length = 0.0, sublength = 0.0, minprop = 0.0,
				maxprop = 0.0;
		...
		for (i = 0; i < iline->ngeoms; i++) {
			...
			/*
			 * This subline doesn't reach the lowest proportion
			 * requested or is beyond the highest proporton
			 */
		...
		/*
		 * If we got any points, we need to return a
		 * GEOMETRYCOLLECTION
		 */
		if (!homogeneous)
			type = COLLECTIONTYPE;
...
/*
 * returns: >0 for a point to the left of the segment, <0 for a point to the
 * right of the segment, 0 for a point on the segment
 */
static double
...
/*
 * This function doesn't test that the point falls on the line defined by the
 * two points.  It assumes that that has already been determined by having
 * determineSide return within the tolerance.  It simply checks that if the
 * point is on the line, it is within the endpoints.
 *
 * returns: 1 if the point is not outside the bounds of the segment 0 if it
 * is
 */
static int
...
/*
 * return -1 iff point is outside ring pts return 1 iff point is inside ring
 * pts return 0 iff point is on ring pts
 */
static int
...
/*
 * return 0 iff point outside polygon or on boundary return 1 iff point
 * inside polygon
 */
int
...
/*
 * return -1 if point outside polygon return 0 if point on boundary return 1
 * if point inside polygon
 *
 * Expected **root order is each exterior ring followed by its holes, eg.
 * EIIEIIEI
 */
int
point_in_multipolygon_rtree(RTREE_NODE * *root, int polyCount, int *ringCounts, LWPOINT * point)
{
	...
	for (p = 0; p < polyCount; p++) {
		...
		if (in_ring == -1) {	/* outside the exterior ring */
			...
			/*
			 * if we have a positive result, we can short-circuit
			 * and return it
			 */
			if (result != -1) {
				return result;
		...
		/*
		 * increment the index by the total number of rings in the
		 * sub-poly
		 */
		/*
		 * we do this here in case we short-cutted out of the poly
		 * before looking at all the rings
		 */
		i += ringCounts[p];
	}
...
/*
 * return -1 iff point outside polygon return 0 iff point on boundary return
 * 1 iff point inside polygon
 */
int
point_in_polygon(LWPOLY * polygon, LWPOINT * point)
{
	...
	for (i = 1; i < polygon->nrings; i++) {
		...
		if (in_ring == 1) {	/* inside a hole => outside the
					 * polygon */
...
/*
 * return -1 iff point outside multipolygon return 0 iff point on
 * multipolygon boundary return 1 iff point inside multipolygon
 */
int
point_in_multipolygon(LWMPOLY * mpolygon, LWPOINT * point)
{
	...
	for (j = 0; j < mpolygon->ngeoms; j++) {
		...
		for (i = 1; i < polygon->nrings; i++) {
			in_ring = point_in_ring(polygon->rings[i], &pt);
			if (in_ring == 1) {	/* inside a hole => outside
						 * the polygon */
				POSTGIS_DEBUGF(3, "point_in_polygon: within hole %d.", i);
				result = -1;
				break;
...
Datum
ST_MinimumBoundingRadius(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *input;
...
Datum
ST_MinimumBoundingCircle(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *input;
	LWBOUNDINGCIRCLE *mbc = NULL;
...
Datum
ST_GeometricMedian(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	GSERIALIZED    *result;
	LWGEOM	       *input;
	...
	if (compute_tolerance_from_box) {
		/*
		 * Compute a default tolerance based on the smallest
		 * dimension of the geometry's bounding box.
		 */
		...
		if (box) {
			...
			/*
			 * Apply a lower bound to the computed default
			 * tolerance to avoid a tolerance of zero in the case
			 * of collinear points.
			 */
...
Datum
ST_IsPolygonCW(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *input;
	bool		is_clockwise;
...
Datum
ST_IsPolygonCCW(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *input;
	bool		is_ccw;
