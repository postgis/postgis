...
/*
 * LWGEOM_in(cstring) format is '[SRID=#;]wkt|wkb' LWGEOM_in(
 * 'SRID=99;POINT(0 0)') LWGEOM_in( 'POINT(0 0)')            --> assumes
 * SRID=SRID_UNKNOWN LWGEOM_in(
 * 'SRID=99;0101000000000000000000F03F000000000000004') LWGEOM_in(
 * '0101000000000000000000F03F000000000000004') returns a GSERIALIZED object
 */
...
LWGEOM_in(PG_FUNCTION_ARGS)
{
	...
	if (str[0] == '0') {
		...
		/*
		 * TODO: 20101206: No parser checks! This is inline with
		 * current 1.5 behavior, but needs discussion
		 */
		lwgeom = lwgeom_from_wkb(wkb, hexsize / 2, LW_PARSER_CHECK_NONE);
		/*
		 * If we picked up an SRID at the head of the WKB set it
		 * manually
		 */
		if (srid)
...
/*
 * LWGEOM_to_latlon(GEOMETRY, text) NOTE: Geometry must be a point.  It is
 * assumed that the coordinates of the point are in a lat/lon projection, and
 * they will be normalized in the output to -90-90 and -180-180.
 *
 ...
 * degrees, "M" for minutes, "S" for seconds, and "C" for cardinal direction
 * (NSEW).  DMS tokens may be repeated to indicate desired width and
 * precision ("SSS.SSSS" means "  1.0023"). "M", "S", and "C" are optional.
 * If "C" is omitted, degrees are shown with a "-" sign if south or west.  If
 * "S" is omitted, minutes will be shown as decimal with as many digits of
 * precision as you specify.  If "M" is omitted, degrees are shown as decimal
 * with as many digits precision as you specify.
 *
 * If the format string is omitted (null or 0-length) a default format will
 * be used.
 *
 * returns text
 */
...
Datum
LWGEOM_to_latlon(PG_FUNCTION_ARGS)
{
	/* Get the parameters */
	...
	/*
	 * The input string supposedly will be in the database encoding, so
	 * convert to UTF-8.
	 */
...
/*
 * LWGEOM_out(lwgeom) --> cstring output is 'SRID=#;<wkb in hex form>' ie.
 * 'SRID=-99;0101000000000000000000F03F0000000000000040' WKB is machine
 * endian if SRID=-1, the 'SRID=-1;' will probably not be present.
 */
PG_FUNCTION_INFO_V1(LWGEOM_out);
Datum
LWGEOM_out(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_asHEXEWKB(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom;
	char	       *hexwkb;
...
/*
 * LWGEOM_to_text(lwgeom) --> text output is 'SRID=#;<wkb in hex form>' ie.
 * 'SRID=-99;0101000000000000000000F03F0000000000000040' WKB is machine
 * endian if SRID=-1, the 'SRID=-1;' will probably not be present.
 */
PG_FUNCTION_INFO_V1(LWGEOM_to_text);
Datum
LWGEOM_to_text(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom;
	char	       *hexwkb;
...
/*
 * LWGEOMFromEWKB(wkb,  [SRID] ) NOTE: wkb is in *binary* not hex form.
 *
 * NOTE: this function parses EWKB (extended form) which also contains SRID
 * info.
 */
PG_FUNCTION_INFO_V1(LWGEOMFromEWKB);
Datum
LWGEOMFromEWKB(PG_FUNCTION_ARGS)
{
...
/*
 * LWGEOMFromTWKB(wkb) NOTE: twkb is in *binary* not hex form.
 *
 */
PG_FUNCTION_INFO_V1(LWGEOMFromTWKB);
Datum
LWGEOMFromTWKB(PG_FUNCTION_ARGS)
{
...
/*
 * WKBFromLWGEOM(lwgeom) --> wkb this will have no 'SRID=#;'
 */
PG_FUNCTION_INFO_V1(WKBFromLWGEOM);
Datum
WKBFromLWGEOM(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	LWGEOM	       *lwgeom;
	uint8_t	       *wkb;
...
Datum
TWKBFromLWGEOM(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom;
	LWGEOM	       *lwgeom;
	uint8_t	       *twkb;
	...
	/*
	 * check for null input since we cannot have the sql-function as
	 * strict. That is because we use null as default for optional ID
	 */
	if (PG_ARGISNULL(0))
		PG_RETURN_NULL();
...
Datum
TWKBFromLWGEOMArray(PG_FUNCTION_ARGS)
{
	...
	while (array_iterate(iter_geoms, &val_geom, &null_geom) &&
		...
		/*
		 * If so, we can make this a homogeneous collection and save
		 * some space
		 */
...
Datum
LWGEOM_addBBOX(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
	GSERIALIZED    *result;
	LWGEOM	       *lwgeom;
...
Datum
LWGEOM_dropBBOX(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *geom = PG_GETARG_GSERIALIZED_P(0);
 ...
 * a 'text' input. First unwrap the text into a cstring, then call
 * geometry_in
 */
PG_FUNCTION_INFO_V1(parse_WKT_lwgeom);
Datum
parse_WKT_lwgeom(PG_FUNCTION_ARGS)
{
...
/*
 * This function must advance the StringInfo.cursor pointer and leave it at
 * the end of StringInfo.buf. If it fails to do so the backend will raise an
 * exception with message: ERROR:  incorrect binary data format in bind
 * parameter #
 *
 */
PG_FUNCTION_INFO_V1(LWGEOM_recv);
Datum
LWGEOM_recv(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_to_bytea(PG_FUNCTION_ARGS)
{
...
Datum
LWGEOM_from_bytea(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *result;
