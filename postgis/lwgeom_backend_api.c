...
lwgeom_init_backend()
{
	...
	/*
	 * Uh oh, this GUC name already exists. Ordinarily we could just go
	 * on
	 */
	/*
	 * our way, but the way the postgis.backend works is by using the
	 * "assign"
	 */
	/*
	 * callback to change which backend is in use by flipping a global
	 * variable
	 */
	/*
	 * over. This saves the overhead of looking up the engine every time,
	 * at
	 */
	/* the expense of the extra complexity. */
	if (postgis_guc_find_option(guc_name)) {
		/*
		 * In this narrow case the previously installed GUC is tied
		 * to the callback in
		 */
		/*
		 * the previously loaded library. Probably this is happening
		 * during an
		 */
	...
	DefineCustomStringVariable(guc_name,	/* name */
				   ...
				   0,	/* int flags */
				   NULL,	/* GucStringCheckHook
						 * check_hook */
				   lwgeom_backend_switch,	/* GucStringAssignHook
								 * assign_hook */
...
int
GetNumConfigOptions(void)returns num_guc_variables
...
/*
 * intersects3d through dwithin used by the 'geos' backend
 */
PG_FUNCTION_INFO_V1(intersects3d_dwithin);
Datum
intersects3d_dwithin(PG_FUNCTION_ARGS)
{
	double		mindist;
