...
/*
 * Specific tree types include all the generic slots and their own slots for
 * their trees. We put the implementation for the CircTreeGeomCache here
 * because we can't shove the PgSQL specific bits of the code (fcinfo) back
 * into liblwgeom, where most of the circtree logic lives.
 */
typedef struct {
	GeomCache	gcache;
	CIRC_NODE      *index;
...
CircTreePIP(const CIRC_NODE * tree1, const GSERIALIZED * g1, const POINT4D * in_point)
{
	...
	/*
	 * If the tree'ed argument is a polygon, do the P-i-P using the
	 * tree-based P-i-P
	 */
	if (tree1_type == POLYGONTYPE || tree1_type == MULTIPOLYGONTYPE) {
		...
		/*
		 * If the candidate isn't in the tree box, it's not in the
		 * tree area
		 */
		...
		/*
		 * The candidate point is in the box, so it *might* be inside
		 * the tree
		 */
		else {
...
geography_distance_cache_tolerance(FunctionCallInfoData * fcinfo, const GSERIALIZED * g1, const GSERIALIZED * g2, const SPHEROID * s, double tolerance, double *distance)
{
	...
	/*
	 * OK, we have an index at the ready! Use it for the one tree
	 * argument and
	 */
	...
	if (tree_cache && tree_cache->gcache.argnum && tree_cache->index) {
		...
		/*
		 * We need to dynamically build a tree for the uncached side
...
geography_dwithin_cache(FunctionCallInfoData * fcinfo, const GSERIALIZED * g1, const GSERIALIZED * g2, const SPHEROID * s, double tolerance, int *dwithin)
{
	double		distance;
	/*
	 * Ticket #2422, difference between sphere and spheroid distance can
	 * trip up the
	 */
	/*
	 * threshold shortcircuit (stopping a calculation before the spheroid
	 * distance is actually
	 */
	/*
	 * below the threshold. Lower in the code line, we actually reduce
	 * the threshold a little to
	 */
	/* avoid this. */
	/*
	 * Correct fix: propogate the spheroid information all the way to the
	 * bottom of the calculation
	 */
