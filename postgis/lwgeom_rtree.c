...
RTreeCreateLeafNode(POINTARRAY * pa, uint32_t startPoint) {
	...
	/*
	 * The given point array will be part of a geometry that will be
	 * freed independently of the index.	Since we may want to cache the
	 * index, we must create independent arrays.
	 */
...
RTreeBuilder(const LWGEOM * lwgeom, GeomCache * cache)
{
	...
	if (lwgeom->type == MULTIPOLYGONTYPE) {
		...
		/*
		 * * Load the array in geometry order, each outer ring
		 * followed by the inner rings * associated with that outer
		 * ring
		 */
		i = 0;
