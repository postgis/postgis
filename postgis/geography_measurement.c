...
Datum
geography_distance_knn(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
	...
	bool		use_spheroid = false;	/* must use sphere, can't get
						 * index to harmonize with
						 * spheroid */
	SPHEROID	s;
	...
	/*
	 * Something went wrong, negative return... should already be eloged,
	 * return NULL
	 */
	if (distance < 0.0) {
		PG_RETURN_NULL();
...
/*
 * * geography_distance_uncached(GSERIALIZED *g1, GSERIALIZED *g2, double
 * tolerance, boolean use_spheroid) * returns double distance in meters
 */
PG_FUNCTION_INFO_V1(geography_distance_uncached);
Datum
geography_distance_uncached(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
...
/*
 * * geography_distance(GSERIALIZED *g1, GSERIALIZED *g2, double tolerance,
 * boolean use_spheroid) * returns double distance in meters
 */
PG_FUNCTION_INFO_V1(geography_distance);
Datum
geography_distance(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g1 = NULL;
	GSERIALIZED    *g2 = NULL;
	double		distance;
	...
	if (LW_FAILURE == geography_distance_cache(fcinfo, g1, g2, &s, &distance)) {
		/*
		 * default to using tree-based distance calculation at all
		 * times
		 */
		/* in standard distance call. */
		geography_tree_distance(g1, g2, &s, FP_TOLERANCE, &distance);
		/*
		 * LWGEOM* lwgeom1 = lwgeom_from_gserialized(g1); LWGEOM*
		 * lwgeom2 = lwgeom_from_gserialized(g2); distance =
		 * lwgeom_distance_spheroid(lwgeom1, lwgeom2, &s, tolerance);
...
/*
 * * geography_dwithin(GSERIALIZED *g1, GSERIALIZED *g2, double tolerance,
 * boolean use_spheroid) * returns double distance in meters
 */
PG_FUNCTION_INFO_V1(geography_dwithin);
Datum
geography_dwithin(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g1 = NULL;
	GSERIALIZED    *g2 = NULL;
...
/*
 * * geography_distance_tree(GSERIALIZED *g1, GSERIALIZED *g2, double
 * tolerance, boolean use_spheroid) * returns double distance in meters
 */
PG_FUNCTION_INFO_V1(geography_distance_tree);
Datum
geography_distance_tree(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g1 = NULL;
	GSERIALIZED    *g2 = NULL;
	double		tolerance = 0.0;
...
/*
 * * geography_dwithin_uncached(GSERIALIZED *g1, GSERIALIZED *g2, double
 * tolerance, boolean use_spheroid) * returns double distance in meters
 */
PG_FUNCTION_INFO_V1(geography_dwithin_uncached);
Datum
geography_dwithin_uncached(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
 ...
 *
 * * warning, this tricky little function does not expand the * geometry at
 * all, just re-writes bounding box value to be * a bit bigger. only useful
 * when passing the result along to * an index operator (&&)
 */
PG_FUNCTION_INFO_V1(geography_expand);
Datum
geography_expand(PG_FUNCTION_ARGS)
{
	GSERIALIZED    *g = NULL;
...
/*
 * * geography_area(GSERIALIZED *g) * returns double area in meters square
 */
PG_FUNCTION_INFO_V1(geography_area);
Datum
geography_area(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	GBOX		gbox;
	...
	/*
	 * User requests spherical calculation, turn our spheroid into a
	 * sphere
	 */
	if (!use_spheroid)
		s.a = s.b = s.radius;
...
/*
 * * geography_perimeter(GSERIALIZED *g) * returns double perimeter in meters
 * for area features
 */
PG_FUNCTION_INFO_V1(geography_perimeter);
Datum
geography_perimeter(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	double		length;
...
/*
 * * geography_length(GSERIALIZED *g) * returns double length in meters
 */
PG_FUNCTION_INFO_V1(geography_length);
Datum
geography_length(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
	GSERIALIZED    *g = NULL;
	double		length;
...
/*
 * * geography_point_outside(GSERIALIZED *g) * returns point outside the
 * object
 */
PG_FUNCTION_INFO_V1(geography_point_outside);
Datum
geography_point_outside(PG_FUNCTION_ARGS)
{
	GBOX		gbox;
	GSERIALIZED    *g = NULL;
	GSERIALIZED    *g_out = NULL;
	...
	/*
	 * We need the bounding box to get an outside point for area
	 * algorithm
	 */
...
/*
 * * geography_covers(GSERIALIZED *g, GSERIALIZED *g) returns boolean * Only
 * works for (multi)points and (multi)polygons currently. * Attempts a simple
 * point-in-polygon test on the polygon and point. * Current algorithm does
 * not distinguish between points on edge * and points within.
 */
PG_FUNCTION_INFO_V1(geography_covers);
Datum
geography_covers(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
...
/*
 * * geography_bestsrid(GSERIALIZED *g, GSERIALIZED *g) returns int * Utility
 * function. Returns negative SRID numbers that match to the * numbers
 * handled in code by the transform(lwgeom, srid) function. * UTM, polar
 * stereographic and mercator as fallback. To be used * in wrapping existing
 * geometry functions in SQL to provide access * to them in the geography
 * module.
 */
PG_FUNCTION_INFO_V1(geography_bestsrid);
Datum
geography_bestsrid(PG_FUNCTION_ARGS)
{
	...
	/*
	 * If we have a unique second argument, fill in all the necessary
	 * variables.
	 */
	if (d1 != d2) {
	...
	/*
	 * * Can we fit these data into one UTM zone? * We will assume we can
	 * push things as * far as a half zone past a zone boundary. * Note
	 * we have no handling for the date line in here.
	 */
	if (xwidth < 6.0) {
	...
	/*
	 * * Can we fit into a custom LAEA area? (30 degrees high, variable
	 * width) * We will allow overlap into adjoining areas, but use a
	 * slightly narrower test (25) to try * and minimize the worst case. *
	...
	/*
	 * * Running out of options... fall-back to Mercator * and hope for
	 * the best.
	 */
...
/*
 * * geography_project(GSERIALIZED *g, distance, azimuth) * returns point of
 * projection given start point, * azimuth in radians (bearing) and distance
 * in meters
 */
PG_FUNCTION_INFO_V1(geography_project);
Datum
geography_project(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
...
/*
 * * geography_azimuth(GSERIALIZED *g1, GSERIALIZED *g2) * returns direction
 * between points (north = 0) * azimuth (bearing) and distance
 */
PG_FUNCTION_INFO_V1(geography_azimuth);
Datum
geography_azimuth(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
...
/*
 * * geography_segmentize(GSERIALIZED *g1, double max_seg_length) * returns
 * densified geometry with no segment longer than max
 */
PG_FUNCTION_INFO_V1(geography_segmentize);
Datum
geography_segmentize(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom1 = NULL;
	LWGEOM	       *lwgeom2 = NULL;
	GSERIALIZED    *g1 = NULL;
	...
	/*
	 * * Set the geodetic flag so subsequent * functions do the right
	 * thing.
	 */
