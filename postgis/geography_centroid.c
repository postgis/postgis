...
Datum
geography_centroid(PG_FUNCTION_ARGS)
{
	LWGEOM	       *lwgeom = NULL;
...
lonlat_to_cart(const double_t raw_lon, const double_t raw_lat)
{
	...
	//prepare coordinate for trigonometric
		functions from[-90, 90]->[0, pi]
			lat = (raw_lat + 90) / 180 * M_PI;

	//prepare coordinate for trigonometric
		functions from[-180, 180]->[-pi, pi]
...
LWPOINT	       *
cart_to_lwpoint(const double_t x_sum, const double_t y_sum, const double_t z_sum, const double_t weight_sum, const uint32_t srid)
{
...
geography_centroid_from_mpoly(const LWMPOLY * mpoly, bool use_spheroid, SPHEROID * s)
{
	...
	for (ip = 0; ip < mpoly->ngeoms; ip++) {
		...
		for (ir = 0; ir < poly->nrings; ir++) {
			...
			/*
			 * split into triangles (two points + reference
			 * point)
			 */
			for (i = 0; i < ring->npoints - 1; i++) {
				...
				/*
				 * Calculate the weight of the triangle. If
				 * counter clockwise, the weight is negative
				 * (e.g. for holes in polygons)
				 */
