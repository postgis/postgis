 ...
 * their trees. We put the implementation for the RectTreeGeomCache here
 * because we can't shove the PgSQL specific bits of the code (fcinfo) back
 * into liblwgeom/lwtree.c, where most of the rect_tree logic lives.
 */
typedef struct {
	GeomCache	gcache;
	RECT_NODE      *index;
...
Datum
ST_DistanceRectTree(PG_FUNCTION_ARGS)
{
...
Datum
ST_DistanceRectTreeCached(PG_FUNCTION_ARGS)
{
