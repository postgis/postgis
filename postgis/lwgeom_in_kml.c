...
/*
 * TODO: - OGC:LonLat84_5773 explicit support (rather than EPSG:4326) -
 * altitudeModeGroup relativeToGround Z Altitude computation upon Geoid
 */
...
is_kml_namespace(xmlNodePtr xnode, bool is_strict)
{
	 ...
	 * we work only on KML fragment, we don't want to 'oblige' to add
 ...
 */
static double
parse_kml_double(char *d, bool space_before, bool space_after)
{
	char	       *p;
	int		st;
	enum states {
...
static POINTARRAY * parse_kml_coordinates(xmlNodePtr xnode, bool *hasz){
	...
	for (found = false; xnode != NULL; xnode = xnode->next) {
		if (xnode->type != XML_ELEMENT_NODE)
			continue;
		if (!is_kml_namespace(xnode, false))
			continue;
		if (strcmp((char *)xnode->name, "coordinates"))
			continue;
	...
	/*
	 * KML coordinates pattern:     x1,y1 x2,y2 x1,y1,z1 x2,y2,z2
	 */
	...
	for (kml_dims = 0; *p; p++) {
		...
		//TODO:		destroy dpa, return NULL
					lwpgerror("invalid KML representation");	/* : %s",
											 * strerror(errno)); */
			}
			if (kml_dims == 1)
				pt.x = d;
				...
				lwpgerror("invalid KML representation");	/* (more than 3
										 * dimensions)"); */
		//TODO:		destroy dpa, return NULL
			}
			...
			if (*q && !isspace(*q) && *q != ',') {
				lwpgerror("invalid KML representation");	/* (invalid character %c
										 * follows ordinate
										 * value)", *q); */
			}
			...
			if (isdigit(*q) || *q == '+' || *q == '-' || *q == '.' || !*q) {
				if (kml_dims < 2)
					lwpgerror("invalid KML representation");	/* (not enough
											 * ordinates)"); */
				else if (kml_dims < 3)
					*hasz = false;
				if (!seen_kml_dims)
			...
			p = q - 1;	/* will be incrementedon next
					 * iteration */
			//lwpgnotice("after look-ahead *p:%c, kml_dims:%d", *p, kml_dims);
		} else if (*p != ',' && !isspace(*p)) {
			lwpgerror("invalid KML representation");	/* (unexpected character
									 * %c)", *p); */
		}
	}
