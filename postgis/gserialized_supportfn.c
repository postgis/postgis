...
Datum postgis_index_supportfn(PG_FUNCTION_ARGS)
{
	...
	if (IsA(rawreq, SupportRequestSimplify))
	{
		...
		if (needsSpatialIndex(clause->funcid, &idxfn) &&
		    idxfn.index == ST_ORDERINGEQUALS_IDX &&
		    list_length(clause->args) == 2)
		{
			...
			get_sort_group_operators(leftdatatype, false, true, false,
						 NULL, &eqoproid, NULL, &hashable);
