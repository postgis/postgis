...
struct LWT_BE_DATA_T {
	...
	/*
	 * This flag will need to be set to false at top-level function enter
	 * and set true whenever an callback changes the data in the
	 * database. It will be used by SPI_execute calls to make sure to see
	 * any data change occurring doring operations.
	 */
	bool		data_changed;
...
static void
cberror(const LWT_BE_DATA * be, const char *fmt,...)
...
addEdgeFields(StringInfo str, int fields, int fullEdgeData)
{
	...
	if (fields & LWT_COL_EDGE_NEXT_LEFT) {
		...
		if (fullEdgeData)
			appendStringInfoString(str, ", abs_next_left_edge");
		sep = ",";
	}
	if (fields & LWT_COL_EDGE_NEXT_RIGHT) {
		appendStringInfo(str, "%snext_right_edge", sep);
		if (fullEdgeData)
			appendStringInfoString(str, ", abs_next_right_edge");
		sep = ",";
	}
...
fillEdgeFields(LWT_ISO_EDGE * edge, HeapTuple row, TupleDesc rowdesc, int fields)
{
	...
	if (fields & LWT_COL_EDGE_GEOM) {
		...
		if (!isnull) {{
				...
				MemoryContextSwitchTo(oldcontext);	/* switch back */
				lwgeom_free(lwg);
				if (DatumGetPointer(dat) != (Pointer) geom)
					pfree(geom);	/* IF_COPY */
		}
		} else {
...
fillFaceFields(LWT_ISO_FACE * face, HeapTuple row, TupleDesc rowdesc, int fields)
{
	...
	if (fields & LWT_COL_FACE_MBR) {
		...
		if (!isnull) {
			/*
			 * NOTE: this is a geometry of which we want to take
			 * (and clone) the BBOX
			 */
			geom = (GSERIALIZED *) PG_DETOAST_DATUM(dat);
...
//TODO:		prepare and execute ?
		...
		if (edges[i].edge_id == -1)
			needsEdgeIdReturn = 1;
	}
	if (needsEdgeIdReturn)
		appendStringInfoString(sql, " RETURNING edge_id");
...
//TODO:		prepare and execute ?
		...
		if (faces[i].face_id == -1)
			needsFaceIdReturn = 1;
	}
	if (needsFaceIdReturn)
		appendStringInfoString(sql, " RETURNING face_id");
	   ...
	   LWT_ELEMID rem_edge, LWT_ELEMID face_left, LWT_ELEMID face_right)
{
	...
	if (face_left != face_right) {
		 ...
		 * the definition in case of healing, so we'd have to bail
		 * out
		 */
		initStringInfo(sql);
		     ...
		     LWT_ELEMID face1, LWT_ELEMID face2, LWT_ELEMID newface)
{
	...
	/*
	 * delete oldfaces (not equal to newface) from the set of primitives
	 * defining the TopoGeometries found before
	 */
		     ...
		     LWT_ELEMID edge1, LWT_ELEMID edge2, LWT_ELEMID newedge)
{
	...
	/*
	 * delete old edges (not equal to new edge) from the set of
	 * primitives defining the TopoGeometries found before
	 */
...
cb_getFaceContainingPoint(const LWT_BE_TOPOLOGY * topo, const LWPOINT * pt){
	...
	/*
	 * TODO: call GetFaceGeometry internally, avoiding the round-trip to
	 * sql
	 */
	appendStringInfo(sql,
...
_PG_init(void)
{
	...
	/*
	 * install PostgreSQL handlers for liblwgeom NOTE: they may be
	 * already in place!
	 */
	pg_install_lwgeom_handlers();

	/*
	 * Switch to the top memory context so that the backend interface is
	 * valid for the whole backend lifetime
	 */
	...
	/*
	 * Switch back to whatever memory context was in place at time of
	 * _PG_init enter. See
	 * http://www.postgresql.org/message-id/20150623114125.GD5835@localhost
	 */
...
Datum
ST_ModEdgeSplit(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	edge_id;
	...
	if (!topo) {
		/*
		 * should never reach this point, as lwerror would raise an
		 * exception
		 */
		SPI_finish();
		PG_RETURN_NULL();
...
Datum
ST_NewEdgesSplit(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	edge_id;
...
Datum
ST_AddIsoNode(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
...
Datum
ST_AddIsoEdge(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	edge_id;
...
Datum
ST_AddEdgeModFace(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
...
Datum
ST_AddEdgeNewFaces(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	startnode_id, endnode_id;
...
Datum
ST_GetFaceGeometry(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	face_id;
...
Datum
ST_GetFaceEdges(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	LWT_ELEMID	face_id;
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		if (!topo) {
			/*
			 * should never reach this point, as lwerror would
			 * raise an exception
			 */
			SPI_finish();
			PG_RETURN_NULL();
		...
		/*
		 * Build a tuple description for a getfaceedges_returntype
		 * tuple
		 */
...
Datum
ST_ChangeEdgeGeom(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char		buf[64];
	char	       *toponame;
...
Datum
ST_RemoveIsoNode(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char		buf[64];
	char	       *toponame;
	...
	/*
	 * TODO: check if any TopoGeometry exists including this point in its
	 * definition !
	 */
...
Datum
ST_RemIsoEdge(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char		buf[64];
	char	       *toponame;
...
Datum
ST_MoveIsoNode(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char		buf[64];
	char	       *toponame;
...
Datum
ST_RemEdgeModFace(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	int		ret;
...
Datum
ST_ModEdgeHeal(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	int		ret;
...
Datum
ST_NewEdgeHeal(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	int		ret;
...
Datum
GetNodeByPoint(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	double		tol;
...
Datum
GetEdgeByPoint(PG_FUNCTION_ARGS)
{
	text	       *toponame_text;
	char	       *toponame;
	double		tol;
