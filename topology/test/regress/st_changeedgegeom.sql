...
-- Now add an obstacle and try to change back (allowed for an isolated
-- universal-face edge; see #3508)
...
-- See https://trac.osgeo.org/postgis/ticket/3508
SELECT NULL FROM topology.CreateTopology('t3508');
SELECT '#3508.1', topology.ST_AddIsoNode('t3508', 0, 'POINT(0 0)');
SELECT '#3508.2', topology.ST_AddIsoNode('t3508', 0, 'POINT(10 0)');
SELECT '#3508.3', topology.ST_AddIsoNode('t3508', 0, 'POINT(5 2)');
SELECT '#3508.4', topology.ST_AddIsoEdge('t3508', 1, 2, 'LINESTRING(0 0, 10 0)');
SELECT '#3508.5', topology.ST_ChangeEdgeGeom('t3508', 1, 'LINESTRING(0 0, 5 5, 10 0)');
SELECT '#3508.6', 'unexpected invalidity', * FROM topology.ValidateTopology('t3508');
SELECT NULL FROM topology.DropTopology('t3508');

-- Free isolated universal-face nodes can be crossed by the changed isolated edge,
-- but nodes bound to other isolated edges must still be protected.
SELECT NULL FROM topology.CreateTopology('t3508b');
SELECT '#3508.7', topology.ST_AddIsoNode('t3508b', 0, 'POINT(0 0)');
SELECT '#3508.8', topology.ST_AddIsoNode('t3508b', 0, 'POINT(10 0)');
SELECT '#3508.9', topology.ST_AddIsoNode('t3508b', 0, 'POINT(5 5)');
SELECT '#3508.10', topology.ST_AddIsoNode('t3508b', 0, 'POINT(5 8)');
SELECT '#3508.11', topology.ST_AddIsoEdge('t3508b', 3, 4, 'LINESTRING(5 5, 5 8)');
SELECT '#3508.12', topology.ST_AddIsoEdge('t3508b', 1, 2, 'LINESTRING(0 0, 10 0)');
SELECT '#3508.13', topology.ST_ChangeEdgeGeom('t3508b', 2, 'LINESTRING(0 0, 5 5, 10 0)');
SELECT NULL FROM topology.DropTopology('t3508b');
