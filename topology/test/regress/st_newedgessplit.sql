...
select null from topology.createtopology('test_topo', 0, 0);
select null from topology.st_addisonode('test_topo', 0, st_geomfromtext('POINT(0 0)'));
select null from topology.st_addisonode('test_topo', 0, st_geomfromtext('POINT(10 10)'));
select null from topology.st_addisoedge('test_topo', 1, 2, st_geomfromtext('LINESTRING(0 0, 10 0, 10 10)'));
select null from topology.st_addedgenewfaces('test_topo', 2, 1, st_geomfromtext('LINESTRING(10 10, 0 10, 0 0)'));
select null from topology.st_newedgessplit('test_topo', 1, st_geomfromtext('POINT(5 0)'));
select 'E' || edge_id, next_left_edge, next_right_edge, left_face, right_face FROM test_topo.edge_data WHERE edge_id=2;
select 'E' || edge_id, next_left_edge, next_right_edge, left_face, right_face FROM test_topo.edge_data WHERE edge_id=3;
select 'E' || edge_id, next_left_edge, next_right_edge, left_face, right_face FROM test_topo.edge_data WHERE edge_id=4;
