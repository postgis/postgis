...
-- A Layer of type 1 (POINT) cannot contain a TopoGeometry of id 2069028923 and type 4 (COLL.)
SELECT topology.CreateTopoGeom( 'MiX', 4, 1, '{{12,2}}', 2069028923);
...
SELECT layer_id(tg), id(tg), type(tg) FROM (
 SELECT topology.CreateTopoGeom( 'MiX', 1, 1, '{{1,1}}', 2069028923) as tg
) foo;

-- Failure: Topogeom ID already used
SELECT topology.CreateTopoGeom( 'MiX', 1, 1, '{{1,1}}', 2069028923);

-- Invalid TopoGeometry type (out of range)
SELECT CreateTopoGeom( 'MiX', 5, 1, '{{1,1}}');
SELECT CreateTopoGeom( 'MiX', 0, 1, '{{1,1}}');
SELECT CreateTopoGeom( 'MiX', 5, 1, '{{1,1}}', 2069028924);
SELECT CreateTopoGeom( 'MiX', 0, 1, '{{1,1}}', 2069028924);
...
SELECT 'MM', ST_AsText(CreateTopoGeom( 'MiX', 4, 4, '{{1,3},{1,2},{1,1}}', 2069028924)); -- fine mix
...
SELECT 'GEOMETRYCOLLECTION EMPTY', ST_AsEWKT( CreateTopoGeom( 'MiX', 4, 4, '{{0,0}}', 2069028929 ) );

-- Test if sequence is set after user defined
SELECT id(tg) FROM (
  SELECT topology.CreateTopoGeom( 'MiX', 4, 4, '{{0,0}}') as tg
) as foo; -- should be 2069028930
