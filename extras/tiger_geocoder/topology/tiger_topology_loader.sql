...
DECLARE
	var_sql text;
	var_rgeom @extschema:postgis@.geometry;
	var_statefp text;
	var_rcnt bigint;
	var_result text := '';
	var_srid int := 4269;
...
BEGIN
	CASE region_type
		WHEN 'place' THEN
			SELECT the_geom , statefp FROM tiger.place INTO var_rgeom, var_statefp WHERE plcidfp = region_id;
		WHEN 'county' THEN
			SELECT the_geom, statefp FROM tiger.county INTO var_rgeom, var_statefp WHERE cntyidfp = region_id;
		ELSE
	...
	CREATE TEMPORARY TABLE tmp_edge
			AS
	WITH te AS
			(SELECT tlid,  @extschema:postgis@.ST_GeometryN(@extschema:postgis@.ST_SnapToGrid(@extschema:postgis@.ST_Transform(@extschema:postgis@.ST_LineMerge(the_geom),$3),$4),1) As geom, tnidf, tnidt, tfidl, tfidr , the_geom As orig_geom
								FROM tiger.edges
								WHERE statefp = $1 AND @extschema:postgis@.ST_Covers($2, the_geom)
									)
	...
	var_sql := 'INSERT INTO ' || quote_ident(toponame) || '.face(face_id, mbr)
					SELECT f.tfid, @extschema:postgis@.ST_Envelope(@extschema:postgis@.ST_Transform(f.the_geom,$3)) As mbr
						FROM tiger.faces AS f
							WHERE statefp = $1 AND
							(  tfid IN(SELECT left_face FROM tmp_edge)
								OR tfid IN(SELECT right_face FROM tmp_edge) OR @extschema:postgis@.ST_Covers($2, the_geom) )
   ...
   var_sql := 'INSERT INTO tmp_edge(edge_id, geom, start_node, end_node, left_face, right_face, next_left_edge, next_right_edge, orig_geom)
			WITH te AS
			(SELECT tlid,  @extschema:postgis@.ST_GeometryN(@extschema:postgis@.ST_SnapToGrid(@extschema:postgis@.ST_Transform(@extschema:postgis@.ST_LineMerge(the_geom),$2),$3),1) As geom, tnidf, tnidt, tfidl, tfidr, the_geom As orig_geom
								FROM tiger.edges
								WHERE statefp = $1 AND
	...
	var_sql := 'INSERT INTO ' || quote_ident(toponame) || '.node(node_id, geom)
				SELECT DISTINCT ON(tnid) tnid, geom
					FROM
					(
						SELECT start_node AS tnid, @extschema:postgis@.ST_StartPoint(e.geom) As geom
							FROM tmp_edge As e LEFT JOIN ' || quote_ident(toponame) || '.node AS n ON e.start_node = n.node_id
					UNION ALL
						SELECT end_node AS tnid, @extschema:postgis@.ST_EndPoint(e.geom) As geom
   ...
   -- start Mark which nodes are contained in faces
	var_sql := 'UPDATE ' || quote_ident(toponame) || '.node AS n
				SET containing_face = f.tfid
					...
					WHERE @extschema:postgis@.ST_ContainsProperly(f.the_geom, @extschema:postgis@.ST_Transform(n.geom,4269)) ';
	EXECUTE var_sql USING var_statefp, var_rgeom;
	GET DIAGNOSTICS var_rcnt = ROW_COUNT;
   ...
   var_sql := 'UPDATE tmp_edge SET geom = @extschema:postgis@.ST_SetPoint(@extschema:postgis@.ST_SetPoint(tmp_edge.geom, 0, s.geom), @extschema:postgis@.ST_NPoints(tmp_edge.geom) - 1,e.geom)
                ...
                WHERE s.node_id = tmp_edge.start_node AND e.node_id = tmp_edge.end_node AND
                    ( NOT @extschema:postgis@.ST_Equals(s.geom, @extschema:postgis@.ST_StartPoint(tmp_edge.geom) ) OR NOT @extschema:postgis@.ST_Equals(e.geom, @extschema:postgis@.ST_EndPoint(tmp_edge.geom) ) ) '  ;
    EXECUTE var_sql;
    GET DIAGNOSTICS var_rcnt = ROW_COUNT;
