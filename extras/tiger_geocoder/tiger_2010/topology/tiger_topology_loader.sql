...
BEGIN
	...
	END CASE;
	SELECT srid FROM topology.topology into var_srid
                WHERE name = toponame;
	var_sql := '
	CREATE TEMPORARY TABLE tmp_edge
   				AS 
	WITH te AS 
   			(SELECT tlid,  ST_GeometryN(ST_LineMerge(ST_Transform(the_geom,$3)),1) As geom, tnidf, tnidt, tfidl, tfidr , the_geom As orig_geom
									FROM tiger.edges 
   ...
   var_sql := 'INSERT INTO tmp_edge(edge_id, geom, start_node, end_node, left_face, right_face, next_left_edge, next_right_edge, orig_geom)	
   			WITH te AS 
   			(SELECT tlid,  ST_GeometryN(ST_LineMerge(ST_Transform(the_geom,$2)),1) As geom, tnidf, tnidt, tfidl, tfidr, the_geom As orig_geom 
									FROM tiger.edges 
									WHERE statefp = $1 AND
