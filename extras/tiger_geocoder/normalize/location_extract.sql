...
BEGIN
  ...
  FOR i IN 1..word_count LOOP
    ...
    stmt := ' SELECT'
         || '   1,'
         || '   name,'
         || '   tiger.@extschema:fuzzystrmatch@.levenshtein_ignore_case(' || quote_literal(tempString) || ',name) as rating,'
         || '   length(name) as len'
         || ' FROM tiger.place'
         ...
         || '   AND tiger.@extschema:fuzzystrmatch@.levenshtein_ignore_case(' || quote_literal(tempString) || ',name) <= 2 '
         || ' UNION ALL SELECT'
         || '   2,'
         || '   name,'
         || '   tiger.@extschema:fuzzystrmatch@.levenshtein_ignore_case(' || quote_literal(tempString) || ',name) as rating,'
