...
/*
 * This trigger will check for authorization before allowing UPDATE or DELETE
 * of specific rows. Rows are identified by the provided column.
 * Authorization info is extracted by the "authorization_table"
 *
 */
PG_FUNCTION_INFO_V1(check_authorization);
Datum
check_authorization(PG_FUNCTION_ARGS)
{
...
#define ERRMSGLEN 256
	...
	/*
	 * check to see if temp_lock_have_table table exists (it might not
	 * exist if they own no locks)
	 */
...
Datum
getTransactionID(PG_FUNCTION_ARGS)
{
