...
uint8_t *lwgeom_to_wkb_buf_direct(const LWGEOM *geom, uint8_t *buf, uint8_t variant);
...
static uint8_t* lwcollection_to_wkb_buf(const LWCOLLECTION *col, uint8_t *buf, uint8_t variant)
{
	...
	for ( i = 0; i < col->ngeoms; i++ )
	{
		buf = lwgeom_to_wkb_buf_direct(col->geoms[i], buf, variant | WKB_NO_SRID);
	}
...
/*
 * Public sizing must match the buffer advance of lwgeom_to_wkb_buf_direct().
 * Internal sizing counts binary WKB bytes; HEX output writes two ASCII bytes
 * per binary byte. The optional C string terminator is deliberately excluded
 * here because only owning string wrappers need it.
 */
size_t
lwgeom_to_wkb_size_full(const LWGEOM *geom, uint8_t variant)
{
...
uint8_t *
lwgeom_to_wkb_buf_direct(const LWGEOM *geom, uint8_t *buf, uint8_t variant)
{
...
lwgeom_to_wkb_write_buf(const LWGEOM *geom, uint8_t variant, uint8_t *buffer)
{
	...
	ptrdiff_t written_bytes = (lwgeom_to_wkb_buf_direct(geom, buffer, variant) - buffer);

	return written_bytes;
}

/*
 * The owning buffer API returns a C string for HEX output, so it allocates one
 * byte beyond the public WKB size for the terminator. Callers using
 * lwgeom_to_wkb_size_full() with lwgeom_to_wkb_buf_direct() should not allocate
 * that terminator.
 */
uint8_t *
lwgeom_to_wkb_buffer(const LWGEOM *geom, uint8_t variant)
{
	size_t b_size = lwgeom_to_wkb_size_full(geom, variant);
