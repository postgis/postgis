...
lwgeom_wkb_needs_srid(const LWGEOM * geom, uint8_t variant)
{
	...
	/*
	 * We can only add an SRID if the geometry has one, and the WKB form
	 * is extended
	 */
...
static uint32_t lwgeom_wkb_type(const LWGEOM * geom, uint8_t variant){
	...
	if (variant & WKB_EXTENDED) {
		...
		if (FLAGS_GET_M(geom->flags))
			wkb_type |= WKBMOFFSET;
		/*
		 * if ( geom->srid != SRID_UNKNOWN && ! (variant &
		 * WKB_NO_SRID) )
		 */
...
static uint8_t * integer_to_wkb_buf(const int ival, uint8_t * buf, uint8_t variant){
	...
	if (variant & WKB_HEX) {
		...
		/*
		 * If machine arch and requested arch match, don't flip byte
		 * order
		 */
		else {
...
static uint8_t * ptarray_to_wkb_buf(const POINTARRAY * pa, uint8_t * buf, uint8_t variant){
	...
	/*
	 * Bulk copy the coordinates when: dimensionality matches, output
	 * format
	 */
 ...
 * COMPOUNDCURVE, MULTISURFACE, CURVEPOLYGON, TIN, POLYHEDRALSURFACE
 */
...
lwgeom_to_wkb(const LWGEOM * geom, uint8_t variant, size_t * size_out)
{
	...
	/*
	 * The buffer pointer should now land at the end of the allocated
	 * buffer space. Let's check.
	 */
