...
typedef struct LWT_BE_CALLBACKS_T {
	 ...
	 *
	 * The walk starts from the side of an edge and stops when either the
	 * max number of visited edges OR the starting position is reached.
	 * The output list never includes a duplicated signed edge
	 * identifier.
	 *
	 * It is expected that the walk uses the "next_left" and "next_right"
	 * attributes of ISO edges to perform the walk (rather than
	 * recomputing the turns at each node).
	 *
	 * @param topo the topology to operate on @param edge walk start
	 * position and direction: abs value identifies the edge, sign
	 * expresses side (left if positive, right if negative) and direction
	 * (forward if positive, backward if negative). @param numedges
	 * output parameter, gets the number of edges visited @param limit
	 * max edges to return (to avoid an infinite loop in case of a
	 * corrupted topology). 0 is for unlimited. The function is expected
	 * to error out if the limit is hit.
	 *
	 * @return an array of signed edge identifiers (positive edges being
	 * walked in their direction, negative ones in opposite) or NULL on
	 * error (@see lastErroMessage)
	 */
 ...
 */
LWT_TOPOLOGY   *
lwt_CreateTopology(LWT_BE_IFACE * iface, const char *name,
 ...
 */
LWT_ELEMID     *
lwt_AddLine(LWT_TOPOLOGY * topo, LWLINE * line, double tol,
	    int *nedges);
 ...
 */
LWT_ELEMID     *
lwt_AddLineNoFace(LWT_TOPOLOGY * topo, LWLINE * line, double tol,
		  int *nedges);

/*
 * Determine and register all topology faces:
 *
 * - Determines which faces are generated by existing edges. - Creates face
 * records with correct mbr - Update edge left/right face attributes
 *
 * Precondition: - the topology edges are correctly linked - there are no
 * faces registered in the topology
 *
 * Postconditions: - all left/right face attributes of edges reference faces
 * with correct mbr.
 *
 * Notes: - does not attempt to assign isolated nodes to their containing
 * faces - does not remove existing face records - loads in memory all the
 * topology edges
 *
 * @param topo the topology to operate on
 *
 * @return 0 on success, -1 on error (librtgeom error handler will be invoked
 * with error message)
 */
 ...
 */
LWT_ELEMID     *
lwt_AddPolygon(LWT_TOPOLOGY * topo, LWPOLY * poly, double tol,
	       int *nfaces);
 ...
 */
int
lwt_MoveIsoNode(LWT_TOPOLOGY * topo,
 ...
 */
LWT_ELEMID
lwt_AddIsoEdge(LWT_TOPOLOGY * topo,
