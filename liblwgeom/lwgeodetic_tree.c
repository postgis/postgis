...
circ_center_spherical(const GEOGRAPHIC_POINT * c1, const GEOGRAPHIC_POINT * c2, double distance, double offset, GEOGRAPHIC_POINT * center)
{
	...
	/*
	 * Center of new circle is projection from start point, using offset
	 * distance
	 */
...
circ_node_internal_new(CIRC_NODE * *c, uint32_t num_nodes) {
	...
	for (i = 1; i < num_nodes; i++) {
		...
		/*
		 * Promote geometry types up the tree, getting more and more
		 * collected
		 */
		...
		/*
		 * If we can't add next feature to this collection cleanly,
		 * promote again to anonymous collection
		 */
		...
		if (FP_EQUALS(dist, 0)) {
			...
			/*
			 * Sometimes the sphere_direction function fails...
			 * this causes the center calculation
			 */
			/*
			 * to fail too. In that case, we're going to fall
			 * back ot a cartesian calculation, which
			 */
			/*
			 * is less exact, so we also have to pad the radius
			 * by (hack alert) an arbitrary amount
			 */
			/*
			 * which is hopefully always big enough to contain
			 * the input edges
			 */
...
circ_nodes_merge(CIRC_NODE * *nodes, int num_nodes){
	...
	/*
	 * TODO, roll geom_type *up* as tree is built, changing to collection
	 * types as simple types are merged TODO, change the distance
	 * algorithm to drive down to simple types first, test pip on
	 * poly/other cases, then test edges
	 */
...
circ_tree_contains_point(const CIRC_NODE * node, const POINT2D * pt, const POINT2D * pt_outside, int *on_boundary)
{
	...
	/*
	 * Construct a stabline edge from our "inside" to our known outside
	 * point
	 */
	...
	/*
	 * If the stabline doesn't cross within the radius of a node, there's
	 * no way it can cross.
	 */
	...
	if (FP_LTEQ(d, node->radius)) {
		...
		if (circ_node_is_leaf(node)) {
			...
			if (inter & PIR_INTERSECTS) {
				 ...
				 * crossings-at-a-vertex,
				 */
				/*
				 * always ignore crossings at "lower" ends of
				 * edges
				 */
		...
		/*
		 * Or, add up the crossing numbers of all children of this
		 * node.
		 */
		else {
			c = 0;
...
circ_tree_distance_tree_internal(const CIRC_NODE * n1, const CIRC_NODE * n2, double threshold, double *min_dist, double *max_dist, GEOGRAPHIC_POINT * closest1, GEOGRAPHIC_POINT * closest2)
{
	...
	/*
	 * circ_tree_print(n1, 0); circ_tree_print(n2, 0);
	 */
	...
	/*
	 * Polygon on one side, primitive type on the other. Check for
	 * point-in-polygon
	 */
	/* short circuit. */
	...
	if (circ_node_is_leaf(n1) && circ_node_is_leaf(n2)) {
		...
		d_min = FLT_MAX;
		/*
		 * Drive the recursion into the COLLECTION types first so we
		 * end up with
		 */
		/*
		 * pairings of primitive geometries that can be forced into
