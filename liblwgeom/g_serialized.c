...
{
	/*
	 * read GSERIALIZED size + max bbox according gbox_serialized_size (2
	 * + Z + M) + 1 int for type
	 */
...
gserialized_get_srid(const GSERIALIZED * s)
{
	...
	srid = srid | s->srid[2];
	/*
	 * Only the first 21 bits are set. Slide up and back to pull the
	 * negative bits down, if we need them.
	 */
	srid = (srid << 11) >> 11;

	/*
	 * 0 is our internal unknown value. We'll map back and forth here for
	 * now
	 */
	if (srid == 0)
		return SRID_UNKNOWN;
...
/*
 * Unfortunately including advanced instructions is something that only helps
 * a small sliver of users who can build their own knowing the target system
 * they will be running on. Packagers have to aim for the lowest common
 * demoninator. So this is dead code for the forseeable future.
 */
#define HAVE_PDEP 0
#if HAVE_PDEP
...
gbox_get_sortable_hash(const GBOX * g)
{
	...
	/*
	 * Since in theory the bitwise representation of an IEEE float is
	 * sortable (exponents come before mantissa, etc) we just copy the
	 * bits directly into an int and then interleave those ints.
	 */
	if (FLAGS_GET_GEODETIC(g->flags)) {
		 ...
		 * Since it's just a sortable bit representation we can omit
		 * division from (A+B)/2. All it should do is subtract 1 from
		 * exponent anyways.
		 */
		x.f = g->xmax + g->xmin;
		y.f = g->ymax + g->ymin;
...
/*
 * Populate a bounding box *without* allocating an LWGEOM. Useful for some
...
gserialized_peek_gbox_p(const GSERIALIZED * g, GBOX * gbox)
{
	...
	/* We can also do single-entry multi-points */
		...
		/*
		 * The check below is necessary because we can have a
		 * MULTIPOINT that contains a single, empty POINT (ngeoms =
		 * 1, npoints = 0)
		 */
		if (npoints != 1)
			return LW_FAILURE;
...
gserialized_from_lwgeom(LWGEOM * geom, size_t * size)
{
	...
	/*
	 * Set up the uint8_t buffer into which we are going to write the
	 * serialized geometry.
	 */
	...
	if (size)		/* Return the output size to the caller if
				 * necessary. */
		*size = return_size;
	...
	/*
	 * * We are aping PgSQL code here, PostGIS code should use * VARSIZE
	 * to set this for real.
	 */
...
static LWPOINT * lwpoint_from_gserialized_buffer(uint8_t * data_ptr, uint8_t g_flags, size_t * g_size) {
	...
	npoints = gserialized_get_uint32_t(data_ptr);	/* Zero => empty
							 * geometry */
...
static LWPOLY * lwpoly_from_gserialized_buffer(uint8_t * data_ptr, uint8_t g_flags, size_t * g_size) {
	...
	nrings = gserialized_get_uint32_t(data_ptr);	/* Zero => empty
							 * geometry */
	poly->nrings = nrings;
	...
	if (nrings > 0) {
		...
		ordinate_ptr += nrings * 4;	/* Move past all the npoints
						 * values. */
	...
	for (i = 0; i < nrings; i++) {
		...
		/*
		 * Make a point array for the ring, and move the ordinate
		 * pointer past the ring ordinates.
		 */
