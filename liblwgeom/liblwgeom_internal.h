...
struct json_object;
/*
 * Module-level PostGIS shims expose the historical downstream names, while the
 * liblwgeom routines below keep names that describe their in-tree role and
 * avoid turning the static archive into the exported ABI.
 */
LWGEOM *lwgeom_from_geojson_object(struct json_object *geojson, int *hasz);

size_t lwgeom_to_wkb_size_full(const LWGEOM *geom, uint8_t variant);
