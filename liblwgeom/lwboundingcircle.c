...
calculate_mbc(const POINT2D * *points, uint32_t max_n, SUPPORTING_POINTS * support, LWBOUNDINGCIRCLE * mbc)
{
	...
	if (num_supporting_points(support) == 3) {
		/*
		 * If we're entering the function with three supporting
		 * points already, our circle is already fully constrained -
		 * we couldn't add another supporting point if we needed to.
		 * So, there's no point in proceeding further.  Welzl (1991)
		 * provides a much better explanation of why this works.
		 */
		return LW_SUCCESS;
	}
	...
	for (i = 0; i < max_n; i++) {
		if (!point_inside_circle(points[i], mbc)) {
			/*
			 * We've run into a point that isn't inside our
			 * circle.  To fix this, we'll go back in time, and
			 * re-run the algorithm for each point we've seen so
			 * far, with the constraint that the current point
			 * must be on the boundary of the circle.  Then,
			 * we'll continue on in this loop with the modified
			 * circle that by definition includes the current
			 * point.
			 */
...
lwgeom_calculate_mbc(const LWGEOM * g)
{
	...
	/*
	 * Technically, a randomized algorithm would demand that we shuffle
	 * the input points before we call calculate_mbc().  However, we make
	 * the (perhaps poor) assumption that the order we happen to find the
	 * points is as good as random, or close enough.
	 */
