...
static void test_force_3dm(void)
{
	...
	geom3dm = lwgeom_force_3dm(geom);
	wkt_out = lwgeom_to_ewkt(geom3dm);
	CU_ASSERT_STRING_EQUAL("CIRCULARSTRINGM(-5 0 0,0 5 0,5 0 0,10 -5 0,15 0 0)", wkt_out);
	lwgeom_free(geom);
	lwgeom_free(geom3dm);
	lwfree(wkt_out);
}

static void test_force_3dm_with_default(void)
{
	LWGEOM *geom;
	LWGEOM *geom3dm;
	char *wkt_out;
	...
	geom = lwgeom_from_wkt("CIRCULARSTRING(-5 0 4,0 5 3,5 0 2,10 -5 1,15 0 0)", LW_PARSER_CHECK_NONE);
	geom3dm = lwgeom_force_3dm_with_default(geom, 1);
	wkt_out = lwgeom_to_ewkt(geom3dm);
...
static void test_force_3dz_with_default(void)
{
	LWGEOM *geom;
	LWGEOM *geom3dz;
	char *wkt_out;
	...
	geom = lwgeom_from_wkt("CIRCULARSTRING(-5 0,0 5,5 0,10 -5,15 0)", LW_PARSER_CHECK_NONE);
	geom3dz = lwgeom_force_3dz_with_default(geom, -99);
	wkt_out = lwgeom_to_ewkt(geom3dz);
...
static void test_force_4d(void)
{
	...
	geom = lwgeom_from_wkt("POINT(1 1)", LW_PARSER_CHECK_NONE);
	geom4d = lwgeom_force_4d(geom);
	wkt_out = lwgeom_to_ewkt(geom4d);
	CU_ASSERT_STRING_EQUAL("POINT(1 1 0 0)", wkt_out);
	lwgeom_free(geom);
	lwgeom_free(geom4d);
	lwfree(wkt_out);
}

static void test_force_4d_with_default(void)
{
	LWGEOM *geom;
	LWGEOM *geom4d;
	char *wkt_out;
	...
	geom4d = lwgeom_force_4d_with_default(geom, 3, 4);
	wkt_out = lwgeom_to_ewkt(geom4d);
...
void force_dims_suite_setup(void)
{
	...
	PG_ADD_TEST(suite, test_force_3dm_with_default);
	PG_ADD_TEST(suite, test_force_3dz);
	PG_ADD_TEST(suite, test_force_3dz_with_default);
	PG_ADD_TEST(suite, test_force_4d);
	PG_ADD_TEST(suite, test_force_4d_with_default);
}
