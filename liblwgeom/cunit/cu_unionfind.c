...
test_unionfind_ordered_by_cluster(void)
{
	...
	for (i = 0; i < uf.N; i++) {
		...
		if (!encountered_cluster[c]) {
			...
			/*
			 * If we've seen an element of this cluster before,
			 * then the current cluster must be the same as the
			 * previous cluster.
			 */
...
test_unionfind_path_compression(void)
{
	...
	/*
	 * Calling "find" on a leaf should attach all nodes between the root
	 * and the leaf directly to the root.
	 */
	uint32_t	root = UF_find(uf, 4);
	for (i = 0; i < uf->N; i++) {
		/*
		 * Verify that all cluster references have been updated to
