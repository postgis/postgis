...
cu_twkb(char *wkt, int8_t prec_xy, int8_t prec_z, int8_t prec_m, uint8_t variant)
{
	...
	uint8_t	       *twkb;
	if (!g)
		lwnotice("input wkt is invalid: %s", wkt);
...
test_twkb_out_point(void)
{
	...
	/*
	 * Since the third dimension is Z it shall get a precision of 1
	 * decimal (third argument)
	 */
	cu_twkb("POINTZ(1 1 1)", 0, 1, 2, 0);
	CU_ASSERT_STRING_EQUAL(s, "010845020214");

	/*
	 * Since the third dimension is M it shall get a precision of 2
	 * decimals (fourth argument)
	 */
...
test_twkb_out_idlist(void)
{
	...
	/*
	 * 04 06 multipoint, size/idlist 07 size 7 bytes 02 two geometries
	 * 0408 idlist (2, 4) 0202 first point @ 1,1 0101 second point offset
	 * -1,-1
	 */
	idlist[0] = 2;
	idlist[1] = 4;
	...
	/*
	 * 04 07 multipoint, bbox/size/idlist 0B size 11 bytes 00020002 bbox
	 * x(0,1), y(0,1) 02 two geometries 0408 idlist (2,4) 0202 first
	 * point @ 1,1 0101 seconds point offset -1,-1
	 */
	idlist[0] = 2;
	idlist[1] = 4;
	...
	/*
	 * 0704 geometrycollection, idlist 02 two geometries 0408 idlist
	 * (2,4) 01000202 first point (type, meta, x, y) 01000000 second
	 * point (type, meta, x, y)
	 */
	idlist[0] = 2;
	idlist[1] = 4;
	...
	/*
	 * 0706 geometrycollection, size/idlist 0D size, 13 bytes 02 two
	 * geometries 0408 idlist (2,4) 0102020202 first point (type, meta,
	 * size, x, y) 0102020000 second point (type, meta, size, x, y)
	 */
	idlist[0] = 2;
	idlist[1] = 4;
