...
stringbuffer_append(stringbuffer_t * s, const char *a)
{
	...
	int		alen0 = alen + 1;	/* Length including null
						 * terminator */
...
stringbuffer_avprintf(stringbuffer_t * s, const char *fmt, va_list ap)
{
	...
	/*
	 * Make a copy of the variadic arguments, in case we need to print
	 * twice
	 */
	/* Print to our buffer */
	va_copy(ap2, ap);
...
stringbuffer_trim_trailing_zeroes(stringbuffer_t * s)
{
	...
	/*
	 * Roll backwards again, with the decimal as stop point, trimming
	 * contiguous zeroes
	 */
	...
	/*
	 * If we stopped at the decimal, we want to null that out. It we
	 * stopped on a numeral, we want to preserve that, so push the
