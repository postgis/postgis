...
/*
 * ISO format uses both Z and M qualifiers. Extended format only uses an M
 * qualifier for 3DM variants, where it is not clear what the third dimension
 * represents. SFSQL format never has more than two dimensions, so no
 * qualifiers.
 */
static void
...
/*
 * Write an empty token out, padding with a space if necessary.
 */
static void
...
/*
 * Point array is a list of coordinates. Depending on output mode, we may
 * suppress some dimensions. ISO and Extended formats include all dimensions.
 * Standard OGC output only includes X/Y coordinates.
 */
static void
 ...
 * ISO: POINT ZM (0 0 0 0) Extended: POINT(0 0 0 0) OGC: POINT(0 0) A
 * three-dimensional m-point will have different outputs too. ISO: POINT M (0
 * 0 0) Extended: POINTM(0 0 0) OGC: POINT(0 0)
 */
static void
...
/*
 * Multi-points do not wrap their sub-members in parens, unlike other
 * multi-geometries. MULTPOINT(0 0, 1 1) instead of MULTIPOINT((0 0),(1 1))
 */
static void
lwmpoint_to_wkt_sb(const LWMPOINT * mpoint, stringbuffer_t * sb, int precision, uint8_t variant)
{
	...
	stringbuffer_append(sb, "(");
	variant = variant | WKT_IS_CHILD;	/* Inform the sub-geometries
						 * they are childre */
 ...
 * but not their linestring sub-geometries. COMPOUNDCURVE((0 0, 1 1),
 * CURVESTRING(1 1, 2 2, 3 3))
 */
static void
...
/*
 * Curve polygons provide type information for their curved rings but not
 * their linestring rings. CURVEPOLYGON((0 0, 1 1, 0 1, 0 0), CURVESTRING(0
 * 0, 1 1, 0 1, 0.5 1, 0 0))
 */
static void
lwcurvepoly_to_wkt_sb(const LWCURVEPOLY * cpoly, stringbuffer_t * sb, int precision, uint8_t variant)
{
	...
	for (i = 0; i < cpoly->nrings; i++) {
		...
		case COMPOUNDTYPE:
			/*
			 * And compoundcurve subgeoms *do* get type
			 * identifiers
			 */
...
/*
 * Multi-curves provide type information for their curved sub-geometries but
 * not their linear sub-geometries. MULTICURVE((0 0, 1 1), CURVESTRING(0 0, 1
 * 1, 2 2))
 */
static void
 ...
 * but not their linear sub-geometries. MULTISURFACE(((0 0, 1 1, 1 0, 0 0)),
 * CURVEPOLYGON(CURVESTRING(0 0, 1 1, 2 2, 0 1, 0 0)))
 */
static void
...
/*
 * Geometry collections provide type information for all their curved
 * sub-geometries but not their linear sub-geometries.
...
lwgeom_to_wkt(const LWGEOM * geom, uint8_t variant, int precision, size_t * size_out)
{
	...
	/*
	 * Extended mode starts with an "SRID=" section for geoms that have
	 * one
	 */
