...
SFCGAL_type_to_lwgeom_type(sfcgal_geometry_type_t type)
{
	...
	case SFCGAL_TYPE_MULTISOLID:
		return COLLECTIONTYPE;	/* Nota: PolyhedralSurface closed
					 * inside aim is to use true solid
					 * type as soon as available in OGC
					 * SFS */
...
/*
 * Return a PostGIS pointarray from a simple SFCGAL geometry: POINT,
 * LINESTRING or TRIANGLE
 *
 ...
 *
 * Used for simple LWGEOM geometry POINT, LINESTRING, TRIANGLE and POLYGON
 * rings
 */
	...
	case SFCGAL_TYPE_GEOMETRYCOLLECTION:
		{
			...
			if (ngeoms) {
				if (ngeoms == nsolids)
					...
					lwnotice("SFCGAL2LWGEOM: SOLID in heterogeneous collection will be treated as a POLYHEDRALSURFACETYPE");
			}
			return rgeom;
		}
	...
	case POLYHEDRALSURFACETYPE:
		{
			...
			/*
			 * We treat polyhedral surface as the only exterior
			 * shell, since we can't distinguish exterior from
			 * interior shells ...
			 */
...
/*
 * No Operation SFCGAL function, used (only) for cunit tests Take a PostGIS
 * geometry, send it to SFCGAL and return it unchanged (in theory)
 */
LWGEOM	       *
