...
/*
 * Ensures the given lat and lon are in the "normal" range: -90 to +90 for
 * lat, -180 to +180 for lon.
 */
static void
lwprint_normalize_latlon(double *lat, double *lon)
{
	/*
	 * First remove all the truly excessive trips around the world via up
	 * or down.
	 */
	while (*lat > 270) {
		*lat -= 360;
	...
	/*
	 * Now see if latitude is past the top or bottom of the world. Past
	 * 90  or -90 puts us on the other side of the earth, so wrap
	 * latitude and add 180 to longitude to reflect that.
	 */
	if (*lat > 90) {
		*lat = 180 - *lat;
		*lon += 180;
	...
	/*
	 * Now make sure lon is in the normal range.  Wrapping longitude has
	 * no effect on latitude.
	 */
	while (*lon > 180) {
		*lon -= 360;
	}
 ...
 * Symbols are specified since N/S or E/W are the only differences when
 * printing lat vs. lon.  They are only used if the "C" (compass dir) token
 * appears in the format string. NOTE: Format string and symbols are required
 * to be in UTF-8.
 */
static char    *
...
{
	/*
	 * 3 numbers, 1 sign or compass dir, and 5 possible strings (degree
	 * signs, spaces, misc text, etc) between or around them.
	 */
#define NUM_PIECES 9
#define WORK_SIZE 1024
	...
	/*
	 * Initialize the working strs to blank.  We may not populate all of
	 * them, and this allows us to concat them all at the end without
	 * worrying about how many we actually needed.
	 */
	...
	if (0 == format_length) {
		...
		/*
		 * Sanity check, we don't want to overwrite an entire piece
		 * of work and no one should need a 1K-sized format string
		 * anyway.
		 */
	...
	for (index = 0; index < format_length; index++) {
		...
		case 'D':
			if (reading_deg) {
				/*
				 * If we're reading degrees, add another
				 * digit.
				 */
		...
		case 'M':
			if (reading_min) {
				/*
				 * If we're reading minutes, add another
				 * digit.
				 */
		...
		case 'S':
			if (reading_sec) {
				/*
				 * If we're reading seconds, add another
				 * digit.
				 */
		...
		case 'C':
			/*
			 * We're done reading anything else we might have
			 * been reading.
			 */
			if (reading_deg || reading_min || reading_sec) {
				/*
				 * We were reading something, that means this
				 * is the next piece.
				 */
				reading_deg = 0;
				reading_min = 0;
		...
		case '.':
			/*
			 * If we're reading deg, min, or sec, we want a
			...
			if (reading_deg) {
				...
				/*
				 * Not reading anything, just pass through
				 * the '.'
				 */
		...
		default:
			/*
			 * Any other char is just passed through unchanged.
			 * But it does mean we are done reading D, M, or S.
			 */
			if (reading_deg || reading_min || reading_sec) {
				/*
				 * We were reading something, that means this
			...
			/*
			 * Check if this is a multi-byte UTF-8 character.  If
			 * so go ahead and read the rest of the bytes as
			 * well.
			 */
			...
			/*
			 * Copy all the character's bytes into the current
			 * piece.
			 */
	...
	if (sec_digits > 0) {
		...
		if (sec_piece >= 0) {
			/*
			 * See if the formatted seconds round up to 60. If
			 * so, increment minutes and reset seconds.
			 */
	...
	/*
	 * Handle the compass direction.  If not using compass dir, display
	 * degrees as a positive/negative number.
	 */
	if (compass_dir_piece >= 0) {
	...
	/*
	 * Allocate space for the result.  Leave plenty of room for excess
	 * digits, negative sign, etc.
	 */
	result = (char *)lwalloc(format_length + WORK_SIZE);
	/*
	 * Append all the pieces together. There may be less than 9, but in
	 * that case the rest will be blank.
	 */
 ...
 * Format string may be null or 0-length, in which case a default format will
 * be used. NOTE: Format string is required to be in UTF-8. NOTE2: returned
 * string is lwalloc'ed, caller is responsible to lwfree it up
 */
static char    *
...
/*
 * Print the X (lon) and Y (lat) of the given point in DMS form using the
 * specified format. First normalizes the values so they will display as -90
 * to 90 and -180 to 180. Format string may be null or 0-length, in which
 * case a default format will be used. NOTE: Format string is required to be
 * in UTF-8. NOTE2: returned string is lwalloc'ed, caller is responsible to
 * lwfree it up
 */
char	       *
...
/*
 * Removes trailing zeros and dot for a %f formatted number. Modifies input.
 */
static void
trim_trailing_zeros(char *str)
{
	...
	ptr = strchr(str, '.');
	if (!ptr)
		return;		/* no dot, no decimal digits */
 ...
 *
 * The actual number of printed decimal digits may be less than the requested
 * ones if out of significant digits.
 *
 * The function will not write more than maxsize bytes, including the
 * terminating NULL. Returns the number of bytes that would have been written
 * if there was enough space (excluding terminating NULL). So a return of
 * ``bufsize'' or more means that the string was truncated and misses a
 * terminating NULL.
 *
 */
int
lwprint_double(double d, int maxdd, char *buf, size_t bufsize)
{
	...
	if (ad < OUT_MAX_DOUBLE) {
		...
		if (maxdd > (OUT_MAX_DOUBLE_PRECISION - ndd))
			maxdd -= ndd;
