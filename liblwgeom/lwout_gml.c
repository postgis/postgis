...
lwgeom_extent_to_gml2(const LWGEOM * geom, const char *srs, int precision, const char *prefix)
{
	...
	/*
	 * if ( ! bbox ) { lwerror("lwgeom_extent_to_gml2: empty geometry
	 * doesn't have a bounding box"); return NULL; }
	 */
...
lwgeom_extent_to_gml3(const LWGEOM * geom, const char *srs, int precision, int opts, const char *prefix)
{
	const		GBOX *bbox = lwgeom_get_bbox(geom);
	/*
	 * if ( ! bbox ) { lwerror("lwgeom_extent_to_gml3: empty geometry
	 * doesn't have a bounding box"); return NULL; }
	 */
...
/*
 * Compute max size required for GML version of this inspected geometry. Will
 * recurse when needed. Don't call this with single-geoms inspected.
 */
static size_t
...
asgml3_point_size(const LWPOINT * point, const char *srs, int precision, int opts, const char *prefix, const char *id){
	...
	if (srs)
		size += strlen(srs) + sizeof(" srsName=..");
	if (id)
		size += strlen(id) + strlen(prefix) + sizeof(" id=..");
	if (IS_DIMS(opts))
...
asgml3_poly_size(const LWPOLY * poly, const char *srs, int precision, int opts, const char *prefix, const char *id){
	...
	if (id)
		size += strlen(id) + strlen(prefix) + sizeof(" id=..");
	if (IS_DIMS(opts))
		size += sizeof(" srsDimension='x'") * poly->nrings;
...
asgml3_triangle_size(const LWTRIANGLE * triangle, const char *srs, int precision, int opts, const char *prefix, const char *id){
	...
	if (srs)
		size += strlen(srs) + sizeof(" srsName=..");
	if (id)
		size += strlen(prefix) + strlen(id) + sizeof(" id=..");
	if (IS_DIMS(opts))
		size += sizeof(" srsDimension='x'");
...
/*
 * In GML3, inside <posList> or <pos>, coordinates are separated by a space
 * separator In GML3 also, lat/lon are reversed for geocentric data
 */
static size_t
