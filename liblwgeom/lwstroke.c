	...
	if (flags & LW_LINEARIZE_FLAG_SYMMETRIC) {
		{
			...
			if (flags & LW_LINEARIZE_FLAG_RETAIN_ANGLE) {
			...
			} else {
				{
					...
					/*
					 * Tweak increment to be regular for
					 * all the arc
					 */
...
/*
 * @param icurve input curve @param tol tolerance, semantic driven by
 * tolerance_type @param tolerance_type see LW_LINEARIZE_TOLERANCE_TYPE
...
lwcircstring_linearize(const LWCIRCSTRING * icurve, double tol,
	...
	for (i = 2; i < icurve->points->npoints; i += 2) {
		...
		if (ret > 0) {
			...
			/*
			 * An error occurred, lwerror should have been called
			 * by now
			 */
...
/*
 * @param icompound input compound curve @param tol tolerance, semantic
 * driven by tolerance_type @param tolerance_type see
 * LW_LINEARIZE_TOLERANCE_TYPE @param flags see flags in lwarc_linearize
 *
 * @return a newly allocated LWLINE
 */
static LWLINE *
...
/*
 * @param icompound input curve polygon @param tol tolerance, semantic driven
 * by tolerance_type @param tolerance_type see LW_LINEARIZE_TOLERANCE_TYPE
 * @param flags see flags in lwarc_linearize
 *
...
pt_continues_arc(const POINT4D * a1, const POINT4D * a2, const POINT4D * a3, const POINT4D * b)
{
	...
	if (diff < EPSILON_SQLMM) {
		...
		/*
		 * Is the point b on the same side of a1/a3 as the mid-point
		 * a2 is?
		 */
		/*
		 * If not, it's in the unbounded part of the circle, so it
		 * continues the arc, return true.
		 */
		if (b_side != a2_side)
			return LW_TRUE;
	}
	...
	while (i < num_edges - 2) {
		...
		for (j = i + 3; j < num_edges + 1; j++) {
			...
			if (pt_continues_arc(&a1, &a2, &a3, &b)) {
				/*
				 * Yes. Mark this edge and the two preceding
				 * it as arc components
				 */
		...
		if (found_arc) {
			 ...
			 * really considered an arc See
			 * http://trac.osgeo.org/postgis/ticket/2420
			 */
			arc_edges = j - 1 - i;
...
lwpolygon_unstroke(const LWPOLY * poly)
{
	...
	if (hascurve == 0) {
		for (i = 0; i < poly->nrings; i++) {
			lwfree(geoms[i]);	/* TODO: should this be
						 * lwgeom_free instead ? */
		}
