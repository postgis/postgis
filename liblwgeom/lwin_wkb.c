...
lwtype_from_wkb_state(wkb_parse_state * s, uint32_t wkb_type)
{
	...
	/*
	 * If any of the higher bits are set, this is probably an extended
	 * type.
	 */
	...
	/*
	 * Strip out just the type number (1-12) from the ISO number (eg
	 * 3001-3012)
	 */
...
static POINTARRAY * ptarray_from_wkb_state(wkb_parse_state * s) {
	...
	/*
	 * If we're in a native endianness, we can just copy the data
	 * directly!
	 */
	if (!s->swap_bytes) {
