...
/*
 * The LWPOINTITERATOR consists of two stacks of items to process: a stack of
 * geometries, and a stack of POINTARRAYs extracted from those geometries.
 * The index "i" refers to the "next" point, which is found at the top of the
 * pointarrays stack.
 *
 * When the pointarrays stack is depleted, we pull a geometry from the
 * geometry stack to replenish it.
 */
...
lwpointiterator_advance(LWPOINTITERATOR * s)
{
	...
	/*
	 * We've reached the end of our current POINTARRAY.  Try to see if
	 * there are any more POINTARRAYS on the stack.
	 */
	...
	/*
	 * We don't have a current POINTARRAY.  Pull a geometry from the
	 * stack, and decompose it into its POINTARRARYs.
	 */
	if (!s->pointarrays) {
		LWGEOM	       *g;
