...
/*
 * static inline double vector_magnitude(const POINT3D* v) { return
...
unit_normal(const POINT3D * P1, const POINT3D * P2, POINT3D * normal)
{
	...
	/*
	 * If edge is really large, calculate a narrower equivalent angle
	 * A1/A3.
	 */
	if (p_dot < 0) {
		vector_sum(P1, P2, &P3);
...
edge_point_side(const GEOGRAPHIC_EDGE * e, const GEOGRAPHIC_POINT * p)
{
	...
	geog2cart(p, &pt);
	/*
	 * We expect the dot product of with normal with any vector in the
	 * plane to be zero
	 */
...
edge_point_in_cone(const GEOGRAPHIC_EDGE * e, const GEOGRAPHIC_POINT * p)
{
	...
	normalize(&vcp);
	/*
	 * The projection of start onto the center defines the minimum
	 * similarity
	 */
	 ...
	 * equal. Unfortunately there are also values of vp_dot_vcp that are
	 * legitimately * very close to but still less than vs_dot_vcp which
	 * we also need to catch. * The tolerance of 10-17 seems to do the
	 * trick on 32-bit and 64-bit architectures, * for the test cases
	 * here. * However, tuning the tolerance value feels like a dangerous
	 * hack. * Fundamentally, the problem is that this test is so
	 * sensitive.
	 */
...
edge_contains_coplanar_point(const GEOGRAPHIC_EDGE * e, const GEOGRAPHIC_POINT * p)
{
	...
	/*
	 * Over the pole, we need normalize latitude and do this calculation
	 * in latitude
	 */
...
*/
static double
sphere_excess(const GEOGRAPHIC_POINT * a, const GEOGRAPHIC_POINT * b, const GEOGRAPHIC_POINT * c)
{
...
edge_contains_point(const GEOGRAPHIC_EDGE * e, const GEOGRAPHIC_POINT * p)
{
	if (edge_point_in_cone(e, p) && edge_point_on_plane(e, p)) {
		/*
		 * if ( edge_contains_coplanar_point(e, p) &&
		 * edge_point_on_plane(e, p) )
		 */
...
*/
int
clairaut_geographic(const GEOGRAPHIC_POINT * start, const GEOGRAPHIC_POINT * end, GEOGRAPHIC_POINT * g_top, GEOGRAPHIC_POINT * g_bottom)
{
	POINT3D		t1, t2;
	GEOGRAPHIC_POINT vN1, vN2;
	LWDEBUG(4, "entering function");
...
edge_intersection(const GEOGRAPHIC_EDGE * e1, const GEOGRAPHIC_EDGE * e2, GEOGRAPHIC_POINT * g)
{
	...
	if (FP_EQUALS(fabs(dot_product(&ea, &eb)), 1.0)) {
		...
		/*
		 * Hack alert, only returning ONE end of the edge right now,
		 * most do better later.
		 */
		/*
		 * Hack alart #2, returning a value of 2 to indicate a
		 * co-linear crossing event.
		 */
...
*/
double
edge_distance_to_edge(const GEOGRAPHIC_EDGE * e1, const GEOGRAPHIC_EDGE * e2, GEOGRAPHIC_POINT * closest1, GEOGRAPHIC_POINT * closest2)
{
	double		d;
...
sphere_project(const GEOGRAPHIC_POINT * r, double distance, double azimuth, GEOGRAPHIC_POINT * n)
{
	...
	/*
	 * If we're going straight up or straight down, we don't need to
...
edge_calculate_gbox_slow(const GEOGRAPHIC_EDGE * e, GBOX * gbox)
{
	...
	/*
	 * Edge is antipodal (one point on each side of the globe), set the
	 * box to contain the whole world and return
	 */
...
gbox_pt_outside(const GBOX * gbox, POINT2D * pt_outside)
{
	...
	while (grow < M_PI) {
		...
		/*
		 * Try a wider growth to push the corners outside the
		 * original box.
		 */
		grow *= 2.0;
	}
...
ptarray_segmentize_sphere_edge_recursive(
				     const POINT3D * p1, const POINT3D * p2,	/* 3-space points we are
										 * interpolating beween */
				     const POINT4D * v1, const POINT4D * v2,	/* real values and z/m
										 * values */
					 double d, double max_seg_length,	/* current segment
										 * length and segment
										 * limit */
					 POINTARRAY * pa)
...
ptarray_segmentize_sphere(const POINTARRAY * pa_in, double max_seg_length){
	...
	for (i = 1; i < pa_in->npoints; i++) {
		...
		if (d > max_seg_length) {
			...
			geog2cart(&g2, &q2);
			/*
			 * 3-d end points, XYZM end point, current edge size,
			 * min edge size
			 */
			ptarray_segmentize_sphere_edge_recursive(&q1, &q2, &p1, &p2, d, max_seg_length, pa_out);
		}
		/*
		 * If we don't segmentize, we need to add first point
		 * manually
		 */
		else {
...
static double
ptarray_distance_spheroid(const POINTARRAY * pa1, const POINTARRAY * pa2, const SPHEROID * s, double tolerance, int check_intersection)
{
	...
	if (pa1->npoints == 1 && pa2->npoints == 1) {
		...
		/* Below tolerance, actual distance isn't of interest */
			...
			return distance;
		/*
		 * Close or greater than tolerance, get the real answer to be
		 * sure
		 */
		else
	...
	if (pa1->npoints == 1 || pa2->npoints == 1) {
		...
		for (i = 1; i < pa_many->npoints; i++) {
			...
			/*
			 * New shortest distance! Record this distance /
			 * location
			 */
			if (d < distance) {
				distance = d;
			...
			if (d < tolerance) {
				/*
				 * Working on a sphere? The answer is
				 * correct, return
				 */
				if (use_sphere) {
					return d;
				...
				/*
				 * Far enough past the tolerance that the
				 * spheroid calculation won't change things
				 */
				else if (d < tolerance * 0.95) {
					return d;
				}
				/*
				 * On a spheroid and near the tolerance?
				 * Confirm that we are *actually* closer than
				 * tolerance
				 */
				else {
					d = spheroid_distance(&g1, &nearest2, s);
					/*
					 * Yes, closer than tolerance,
					 * return!
					 */
					if (d < tolerance)
						return d;
		...
		if (use_sphere)
			return distance;
		/*
		 * On spheroid, calculate final answer based on closest
		 * approach
		 */
		else
	...
	for (i = 1; i < pa1->npoints; i++) {
		...
		for (j = 1; j < pa2->npoints; j++) {
			...
			/*
			 * Copy end to start to allow a new end value in next
			 * iteration
			 */
			e2.start = e2.end;
			B1 = B2;
...
lwgeom_distance_spheroid(const LWGEOM * lwgeom1, const LWGEOM * lwgeom2, const SPHEROID * spheroid, double tolerance)
{
	...
	/*
	 * What's the distance to an empty geometry? We don't know. Return a
	 * negative number so the caller can catch this case.
	 */
	...
	/*
	 * If the boxes aren't disjoint, we have to check for edge
	 * intersections
	 */
	...
	/*
	 * Point/line combinations can all be handled with simple point array
	 * iterations
	 */
	...
	/*
	 * Point/Polygon cases, if point-in-poly, return zero, else return
	 * distance.
	 */
	...
	/*
	 * Line/polygon case, if start point-in-poly, return zero, else
	 * return distance.
	 */
...
lwgeom_covers_lwgeom_sphere(const LWGEOM * lwgeom1, const LWGEOM * lwgeom2)
{
	...
	/*
	 * dim(geom2) > dim(geom1) always returns false (because geom2 is
	 * bigger)
	 */
	...
	/*
	 * If any of the first argument parts covers the second argument,
	 * it's true
	 */
	if (lwtype_is_collection(type1)) {
		uint32_t	i;
	...
	/*
	 * Only if all of the second arguments are covered by the first
	 * argument is the condition true
	 */
	if (lwtype_is_collection(type2)) {
		uint32_t	i;
		LWCOLLECTION   *col = (LWCOLLECTION *) lwgeom2;
...
lwpoly_covers_lwpoly(const LWPOLY * poly1, const LWPOLY * poly2)
{
	...
	for (i = 0; i < poly2->nrings; i++) {
		...
		/*
		 * every other ring is a hole, check if point is inside the
		 * actual polygon
		 */
		if (i % 2 == 0) {
	...
	/*
	 * check for any edge intersections, so nothing is partially outside
	 * of poly1
	 */
	for (i = 0; i < poly2->nrings; i++) {
	...
	/*
	 * no abort condition found, so the poly2 should be completly inside
	 * poly1
	 */
	return LW_TRUE;
}
...
lwline_covers_lwline(const LWLINE * lwline1, const LWLINE * lwline2)
{
	...
	while (i < lwline1->points->npoints - 1 && j < lwline2->points->npoints - 1) {
		...
		/*
		 * we already know, that the last point is on line1, so we're
		 * done
		 */
		if (j == lwline2->points->npoints - 1) {
			...
			/*
			 * point is on current line1 edge, check next point
			 * in line2
			 */
			...
			/*
			 * point is on current line2 edge, check next point
			 * in line1
			 */
...
lwpolygon_calculate_gbox_geodetic(const LWPOLY * poly, GBOX * gbox)
{
	...
	/*
	 * If the box wraps a poly, push that axis to the absolute min/max as
	 * appropriate
	 */
...
ptarray_length_spheroid(const POINTARRAY * pa, const SPHEROID * s)
{
	...
	for (i = 1; i < pa->npoints; i++) {
		...
		/*
		 * B gets incremented in the next loop, so we save the value
		 * here
		 */
		a = b;
		za = zb;
...
point_in_cone(const POINT3D * A1, const POINT3D * A2, const POINT3D * P)
{
	...
	/*
	 * If the point is more similar than the end, the point is in the
	 * cone
	 */
...
ptarray_contains_point_sphere(const POINTARRAY * pa, const POINT2D * pt_outside, const POINT2D * pt_to_test)
{
	...
	for (i = 1; i < pa->npoints; i++) {
		...
		/*
		 * Our test point is on an edge end! Point is "in ring" by
		 * our definition
		 */
		...
		if (inter & PIR_INTERSECTS) {
			/*
			 * If the stabline is touching the edge, that implies
			 * the test point
			 */
			/*
			 * is on the edge, so we're done, the point is in
			 * (on) the ring.
			 */
			...
			/*
			 * It's a touching interaction, disregard all the
			 * left-side ones.
			 */
