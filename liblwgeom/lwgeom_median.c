...
iterate_4d(POINT3D * curr, const POINT4D * points, const uint32_t npoints, const uint32_t max_iter, const double tol){
	...
	for (iter = 0; iter < max_iter; iter++) {
		...
		for (i = 0; i < npoints; i++) {
			/*
			 * we need to use lower epsilon than in FP_IS_ZERO in
			 * the loop for calculation to converge
			 */
		...
		/*
		 * If any of the intermediate points in the calculation is
		 * found in the set of input points, the standard Weiszfeld
		 * method gets stuck with a divide-by-zero.
		 *
		 * To get ourselves out of the hole, we follow an alternate
		 * procedure to get the next iteration, as described in:
		 *
		 * Vardi, Y. and Zhang, C. (2011) "A modified Weiszfeld
		 * algorithm for the Fermat-Weber location problem."  Math.
		 * Program., Ser. A 90: 559-566. DOI 10.1007/s101070100222
		 *
	...
	for (i = 0; i < g->ngeoms; i++) {
		...
		if (!lwgeom_is_empty(subg)) {
			*input_empty = LW_FALSE;
			...
			if (has_m) {
				/*
				 * This limitation on non-negativity can be
				 * lifted by replacing Weiszfeld algorithm
				 * with different one. Possible option
				 * described in:
				 *
				 * Drezner, Zvi & O. Wesolowsky, George.
				 * (1991). The Weber Problem On The Plane
				 * With Some Negative Weights. INFOR.
				 * Information Systems and Operational
				 * Research. 29.
				 * 10.1080/03155986.1991.11732158.
				 */
				if (points[n].m < 0) {
				...
				/*
				 * points with zero weight are not going to
				 * affect calculation, drop them early
				 */
...
lwmpoint_median(const LWMPOINT * g, double tol, uint32_t max_iter, char fail_if_not_converged)
{
	...
	uint32_t	npoints = 0;	/* we need to count this ourselves so
					 * we can exclude empties and
					 * weightless points */
	uint32_t	i;
