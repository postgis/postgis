	...
	for (i = 0; i < g->ngeoms; i++)
	{
		...
		if (!lwgeom_is_empty(subg))
		{
			*input_empty = LW_FALSE;
			...
			{
				lwerror(
				    "Geometric median: getPoint4d_p reported failure on point (POINT(%g %g %g %g), number %d of %d in input).",
				    points[n].x,
				    points[n].y,
				    points[n].z,
				    points[n].m,
			...
			if (has_m)
			{
				...
				{
					lwerror(
					    "Geometric median input contains points with negative weights (POINT(%g %g %g %g), number %d of %d in input). Implementation can't guarantee global minimum convergence.",
					    points[n].x,
					    points[n].y,
					    points[n].z,
					    points[n].m,
