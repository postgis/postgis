...
cmpfunc(const void *a, const void *b)
{
	 ...
	 * If two values is the same we force returning the last point in hte
	 * point array. That way we get the same ordering on diffreent
	 * machines and pllatforms
	 */
	if (v1 == v2)
...
tune_areas(EFFECTIVE_AREAS * ea, int avoid_collaps, int set_area, double trshld)
{
	...
	while (go_on) {
		/*
		 * Get a reference to the point with the currently smallest
		 * effective area
		 */
		current = minheap_pop(&tree, ea->initial_arealist) - ea->initial_arealist;

		/*
		 * We have found the smallest area. That is the resulting
		 * effective area for the "current" point
		 */
		...
		/*
		 * The found smallest area point is now regarded as elimnated
		 * and we have to recalculate the area the adjacent (ignoring
		 * earlier elimnated points) points gives
		 */
		...
		/*
		 * Check if point before current point is the first in the
		 * point array.
		 */
		if (before_current > 0) {
		...
		if (after_current < npoints - 1) {	/* Check if point after
		...
		/*
		 * rearrange the nodes so the eliminated point will be
		 * ingored on the next run
		 */
...
ptarray_calc_areas(EFFECTIVE_AREAS * ea, int avoid_collaps, int set_area, double trshld)
{
	...
	/*
	 * The first and last point shall always have the maximum effective
	 * area. We use float max to not make trouble for bbox
	 */
...
static POINTARRAY * ptarray_set_effective_area(POINTARRAY * inpts, int avoid_collaps, int set_area, double trshld){
	...
	if (set_area) {
		/*
		 * Only return points with an effective area above the
		 * threashold
		 */
