...
lw_arc_side(const POINT2D * A1, const POINT2D * A2, const POINT2D * A3, const POINT2D * Q)
{
	...
	/*
	 * Q is inside the arc boundary, so it's not on the side we might
	 * think from examining only the end points
	 */
	if (d < radius_A && side_Q == side_A2) {
		side_Q *= -1;
	}
...
lwline_crossing_direction(const LWLINE * l1, const LWLINE * l2)
{
	...
	for (i = 1; i < pa2->npoints; i++) {
		...
		for (j = 1; j < pa1->npoints; j++) {
			...
			/*
			 * * Crossing at a co-linearity can be turned handled
			 * by extending * segment to next vertext and seeing
			 * if the end points straddle * the co-linear
			 * segment.
			 */
			if (this_cross == SEG_COLINEAR) {
				LWDEBUG(4, "this_cross == SEG_COLINEAR");
				/*
				 * TODO: Add logic here and in
				 * segment_intersects() continue;
				 */
			}
...
/*
 * * Calculate the geohash, iterating downwards and gaining precision. * From
 * geohash-native.c, (c) 2008 David Troy <dave@roundhousetech.com> * Released
 ...
 * set in them will be the southwest and northeast coordinates of the
 * bounding * box accordingly. A precision less than 0 indicates that the
 * entire length * of the GeoHash should be used.
 */
void
...
lwgeom_geohash_precision(GBOX bbox, GBOX * bounds)
{
	...
	if (minx == maxx && miny == maxy) {
		/*
		 * It's a point. Doubles have 51 bits of precision. * 2 * 51
		 * / 5 == 20
		 */
		return 20;
	}
	...
	/*
	 * Shrink a world bounding box until one of the edges interferes with
	 * the * bounds of our rectangle.
	 */
	while (1) {
		...
		if (lonminadjust || lonmaxadjust) {
			...
			lonmax += lonmaxadjust;
			/*
			 * Each adjustment cycle corresponds to 2 bits of
			 * storage in the * geohash.
			 */
			precision++;
		} else {
			break;
		...
		/*
		 * Only adjust if adjustments are legal (we haven't crossed
		 * any edges).
		 */
...
/*
 * * Return a geohash string for the geometry. <http://geohash.org> * Where
 * the precision is non-positive, calculate a precision based on the * bounds
 * of the feature. Big features have loose precision. * Small features have
 * tight precision.
 */
char	       *
lwgeom_geohash(const LWGEOM * lwgeom, int precision)
{
	...
	/*
	 * Return error if we are being fed something outside our working
	 * bounds
	 */
	...
	/*
	 * * Return the geohash of the center, with a precision determined by
	 * the * extent of the bounds. * Possible change: return the point at
	 * the center of the precision bounds?
	 */
