...
lwline_split_by_line(const LWLINE * lwline_in, const LWGEOM * blade_in){
	 ...
	 * Possible outcomes:
	 *
	 * 1. The lines do not cross or overlap -> Return a collection with
	 * single element 2. The lines cross -> Return a collection of all
	 * elements resulting from the split
	 */
...
lwline_split_by_mpoint(const LWLINE * lwline_in, const LWMPOINT * mp){
	...
	for (i = 0; i < mp->ngeoms; ++i) {
		for (j = 0; j < out->ngeoms; ++j) {
			...
			if (2 == ret) {
				/*
				 * the point splits this line, 2 splits were
				 * added to collection. We'll move the latest
				 * added into the slot of the current one.
				 */
			 ...
			 LWMLINE * v)
{
	 ...
	 * Possible outcomes:
	 *
	 * 1. The point is not on the line or on the boundary -> Leave
	 * collection untouched, return 0 2. The point is on the boundary ->
	 * Leave collection untouched, return 1 3. The point is in the line
	 * -> Push 2 elements on the collection: o start_point - cut_point o
	 * cut_point - last_point -> Return 2
	 */
	...
	/*
	 * We need to project the point on the closest segment, to
	 * interpolate Z and M if needed
	 */
	getPoint4d_p(ipa, seg, &p1);
	getPoint4d_p(ipa, seg + 1, &p2);
	closest_point_on_segment(&pt, &p1, &p2, &pt_projected);
	/*
	 * But X and Y we want the ones of the input point, as on some
	 * architectures the interpolation math moves the coordinates (see
	 * #3422)
	 */
	pt_projected.x = pt.x;
	pt_projected.y = pt.y;
	...
	/*
	 * This is an internal intersection, let's build the two new
	 * pointarrays
	 */
	...
	/*
	 * TODO: replace with a memcpy (if so need to check for duplicated
	 * point) ?
	 */
...
lwpoly_split_by_line(const LWPOLY * lwpoly_in, const LWGEOM * blade_in){
	 ...
	 * Possible outcomes:
	 *
	 * 1. The line does not split the polygon -> Return a collection with
	 * single element 2. The line does split the polygon -> Return a
	 * collection of all elements resulting from the split
	 */
	...
	/*
	 * We should now have all polygons, just skip the ones which are in
	 * holes of the original geometries and return the rest in a
	 * collection
	 */
	...
	for (i = 0; i < n; ++i) {
		...
		if (0 == contains) {
			/*
			 * Original geometry doesn't contain a point in this
			 * ring, must be an hole
			 */
			continue;
		}
