...
segment_locate_along(const POINT4D * p1, const POINT4D * p2, double m, double offset, POINT4D * pn)
{
	...
	if (m1 == m2) {
		/*
		 * Degenerate case: same M on both points. If they are the
		 * same point we just return one of them.
		 */
		if (p4d_same(p1, p2)) {
			*pn = *p1;
			return LW_TRUE;
...
lwmline_locate_along(const LWMLINE * lwmline, double m, double offset){
	...
	for (i = 0; i < lwmline->ngeoms; i++) {
		...
		if (along) {
			...
			/*
			 * Free the containing geometry, but leave the
			 * sub-geometries around
			 */
			along->ngeoms = 0;
			lwmpoint_free(along);
		}
...
*/
int
point_interpolate(const POINT4D * p1, const POINT4D * p2, POINT4D * p, int hasz, int hasm, char ordinate, double interpolation_value)
{
	...
	if (mline->ngeoms == 1) {
		...
		for (i = 0; i < mline->ngeoms; i++) {
			...
			if (col) {
				...
				/*
				 * Shallow free the struct, leaving the geoms
				 * behind.
				 */
				if (col->bbox)
					lwfree(col->bbox);
...
lwline_clip_to_ordinate_range(const LWLINE * line, char ordinate, double from, double to)
{
	...
	for (i = 0; i < pa_in->npoints; i++) {
		...
		if (ordinate_value_p >= from && ordinate_value_p <= to) {
			...
			if (!added_last_point) {
				...
				/*
				 * We didn't add the previous point, so this
				 * is a new segment. Make a new point array.
				 */
				dp = ptarray_construct_empty(hasz, hasm, 32);

				/*
				 * We're transiting into the range so add an
				 * interpolated point at the range boundary.
				 * If we're on a boundary and crossing from
				 * the far side, we also need an interpolated
				 * point.
				 */
				if (i > 0 && (	/* Don't try to interpolate
		...
		/* Is this point inside the ordinate range? No. */
			...
			if (added_last_point == 1) {
				/*
				 * We're transiting out of the range, so add
				 * an interpolated point to the point array
				...
				/*
				 * We're out and the last point was on the
				 * boundary. If the last point was the near
				 * boundary, nothing to do. If it was the far
				 * boundary, we need an interpolated point.
				 */
				if (from != to && (
				...
				/*
				 * We just hopped over the whole range, from
				 * bottom to top, so we need to add *two*
				 * interpolated points!
				 */
				...
				/*
				 * We just hopped over the whole range, from
				 * top to bottom, so we need to add *two*
				 * interpolated points!
				 */
				dp = ptarray_construct(hasz, hasm, 2);
				/* Interpolate upper point. */
			...
			/*
			 * We have an extant point-array, save it out to a
			 * multi-line.
			 */
			if (dp) {
				...
				/*
				 * Only one point, so we have to make an
				 * lwpoint to hold this and set the overall
				 * output type to a generic collection.
				 */
				if (dp->npoints == 1) {
				...
				/*
				 * Pointarray is now owned by lwgeom_out, so
	...
	if (dp && dp->npoints > 0) {
		...
		/*
		 * Pointarray is now owned by lwgeom_out, so drop reference
		 * to it
		 */
		dp = NULL;
	}
...
lwgeom_clip_to_ordinate_range(const LWGEOM * lwin, char ordinate, double from, double to, double offset)
{
	...
	/* Construct a collection to hold our outputs. */
	/*
	 * Things get ugly: GEOS offset drops Z's and M's so we have to drop
	 * ours
	 */
	...
	for (i = 0; i < out_col->ngeoms; i++) {
		...
		if (type == POINTTYPE) {
			...
			/*
			 * lwgeom_offsetcurve(line, offset, quadsegs,
			 * joinstyle (round), mitrelimit)
			 */
 ...
 *
 * Given two vectors (p1-p2 and q1-q2) and a time range (t1-t2) return the
 * time in which a point p is closest to a point q on their respective
 * vectors, and the actual points
 *
 * Here we use algorithm from softsurfer.com that can be found here
 * http://softsurfer.com/Archive/algorithm_0106/algorithm_0106.htm
 *
 * @param p0 start of first segment, will be set to actual closest point of
 * approach on segment. @param p1 end of first segment @param q0 start of
 * second segment, will be set to actual closest point of approach on
 * segment. @param q1 end of second segment @param t0 start of travel time
	      ...
	      double t0, double t1)
{
	...
	/*
	 * lwnotice("FROM %g,%g,%g,%g -- %g,%g,%g,%g", p0->x, p0->y, p0->z,
	 * p0->m, p1->x, p1->y, p1->z, p1->m); lwnotice("  TO %g,%g,%g,%g --
	 * %g,%g,%g,%g", q0->x, q0->y, q0->z, q0->m, q1->x, q1->y, q1->z,
	 * q1->m);
	 */
 ...
 *
 * The function assumes measures are linear so that always a single point is
 * returned for a single measure.
 *
 * @param pa the point array to perform search on @param m the measure to
 * search for @param p the point to write result into @param from the segment
 * number to start from
 *
 * @return the segment number the point was found into or -1 if given measure
 * was out of the known range.
 */
static int
...
lwgeom_tcpa(const LWGEOM * g1, const LWGEOM * g2, double *mindist)
{
	...
	/*
	 * Find overlapping M range WARNING: may be larger than the real one
	 */
	...
	/*
	 * For each consecutive pair of measures, compute time of closest
	 * point approach and actual distance between points at that time
	 */
	mintime = tmin;
	for (i = 1; i < nmvals; ++i) {
		double		t0 = mvals[i - 1];
		...
		/*
		 * lwnotice("Closest points: %g,%g,%g and %g,%g,%g at time
		 * %g", p0.x, p0.y, p0.z, q0.x, q0.y, q0.z, t);
		 */
...
lwgeom_cpa_within(const LWGEOM * g1, const LWGEOM * g2, double maxdist)
{
	...
	assert(gbox1);		/* or the npoints check above would have
				 * failed */
	assert(gbox2);		/* or the npoints check above would have
				 * failed */
