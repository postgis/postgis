 ...
 * Default allocators
 *
 * We include some default allocators that use malloc/free/realloc along with
 * stdout/stderr since this is the most common use case
 *
 */
...
/*
 * Returns a new string which contains a maximum of maxlength characters
 * starting from startpos and finishing at endpos (0-based indexing). If the
 * string is truncated then the first or last characters are replaced by
 * "..." as appropriate.
 *
 * The caller should specify start or end truncation by setting the
 * truncdirection parameter as follows: 0 - start truncation (i.e. characters
 * are removed from the beginning) 1 - end trunctation (i.e. characters are
...
getMachineEndian(void)
{
	...
	return *((char *)&endian_check_int);	/* 0 = big endian | xdr, 1 =
						 * little endian | ndr */
}
