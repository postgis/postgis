...
lw_dist3d_distanceline(const LWGEOM * lw1, const LWGEOM * lw2, int srid, int mode)
{
	...
	if (!lwgeom_has_z(lw1) || !lwgeom_has_z(lw2)) {
		...
		if (!lw_dist2d_comp(lw1, lw2, &thedl2d)) {
			/*
			 * should never get here. all cases ought to be error
			 * handled earlier
			 */
		...
		if (!lwgeom_has_z(lw1)) {
			...
			if (!lw_dist3d_recursive(vertical_line, lw2, &thedl)) {
				/*
				 * should never get here. all cases ought to
				 * be error handled earlier
				 */
...
{
	POINT3DZ	p, projp;	/* projp is "point projected on
					 * plane" */
	PLANE3D		plane;
	...
	/*
	 * Find the plane of the polygon, the "holes" have to be on the same
	 * plane. so we only care about the boudary
	 */
...
lw_dist3d_poly_poly(LWPOLY * poly1, LWPOLY * poly2, DISTPTS3D * dl)
{
	...
	/*
	 * What we do here is to compare the bondary of one polygon with the
	 * other polygon and then take the second boudary comparing with the
	 * first polygon
	 */
	dl->twisted = 1;
	if (!lw_dist3d_ptarray_poly(poly1->rings[0], poly2, &plane, dl))
		return LW_FALSE;
	if (dl->distance == 0.0)	/* Just check if the answer already
					 * is given */
		return LW_TRUE;

	if (!define_plane(poly1->rings[0], &plane))
		return LW_FALSE;
	dl->twisted = -1;	/* because we swithc the order of geometries
				 * we swithch "twisted" to -1 which will give
				 * the right order of points in shortest
				 * line. */
...
lw_dist3d_pt_seg(POINT3DZ * p, POINT3DZ * A, POINT3DZ * B, DISTPTS3D * dl)
{
	...
	/*
	 * This is for finding the 3Dmaxdistance. the maxdistance have to be
	...
	if (r < 0) {		/* If the first vertex A is closest to the
				 * point p */
		return lw_dist3d_pt_pt(p, A, dl);
	}
	if (r > 1) {		/* If the second vertex B is closest to the
				 * point p */
		return lw_dist3d_pt_pt(p, B, dl);
	}

	/*
	 * else if the point p is closer to some point between a and b then
	 * we find that point and send it to lw_dist3d_pt_pt
	 */
	c.x = A->x + r * (B->x - A->x);
	c.y = A->y + r * (B->y - A->y);
	c.z = A->z + r * (B->z - A->z);
...
{
	VECTOR3D	v1, v2, vl;
	double		s1k, s2k;	/* two variables representing where
					 * on Line 1 (s1k) and where on Line
					 * 2 (s2k) a connecting line between
					 * the two lines is perpendicular to
					 * both lines */
	POINT3DZ	p1, p2;
	double		a, b, c, d, e, D;
	...
	/*
	 * Here we use algorithm from softsurfer.com that can be found here
	...
	if (D < 0.000000001) {
		...
		s1k = 0.0;	/* If the lines are paralell we try by using
				 * the startpoint of first segment. If that
				 * gives a projected point on the second line
				 * outside segment 2 it wil be found that s2k
				 * is >1 or <0. */
	...
	/*
	 * Now we check if the projected closest point on the infinite lines
	 * is outside our segments. If so the combinations with start and end
	...
	if (s1k < 0.0 || s1k > 1.0 || s2k < 0.0 || s2k > 1.0) {
		...
		if (!lw_dist3d_pt_pt(&p1, &p2, dl)) {	/* Send the closest
							 * points to point-point
							 * calculation */
			return LW_FALSE;
		}
	}
...
lw_dist3d_pt_poly(POINT3DZ * p, LWPOLY * poly, PLANE3D * plane, POINT3DZ * projp, DISTPTS3D * dl)
{
	...
	if (pt_in_ring_3d(projp, poly->rings[0], plane)) {
		...
		return lw_dist3d_pt_pt(p, projp, dl);	/* If the projected
							 * point is inside the
							 * polygon the shortest
							 * distance is between
							 * that point and the
							 * inputed point */
	} else {
		return lw_dist3d_pt_ptarray(p, poly->rings[0], dl);	/* If the projected
									 * point is outside the
									 * polygon we search for
									 * the closest distance
									 * against the boundarry
									 * instead */
	}
...
lw_dist3d_ptarray_poly(POINTARRAY * pa, LWPOLY * poly, PLANE3D * plane, DISTPTS3D * dl)
{
	...
	s1 = project_point_on_plane(&p1, plane, &projp1);	/* the sign of s1 tells
								 * us on which side of
								 * the plane the point
								 * is. */
	...
	for (i = 1; i < pa->npoints; i++) {
		...
		/*
		 * If s1and s2 has different signs that means they are on
		 * different sides of the plane of the polygon. That means
		 * that the edge between the points crosses the plane and
		 * might intersect with the polygon
		 */
		if ((s1 * s2) <= 0) {
			f = fabs(s1) / (fabs(s1) + fabs(s2));	/* The size of s1 and s2
			...
			/*
			 * get the point where the line segment crosses the
			 * plane
			 */
			...
			intersects = LW_TRUE;	/* We set intersects to true
						 * until the opposite is
						 * proved */

			if (pt_in_ring_3d(&intersectionp, poly->rings[0], plane)) {	/* Inside outer ring */
				for (k = 1; k < poly->nrings; k++) {
					/*
					 * Inside a hole, so no intersection
					 * with the polygon
					 */
	...
	/*
	 * check or pointarray against boundary and inner boundaries of the
	 * polygon
	 */
...
define_plane(POINTARRAY * pa, PLANE3D * pl)
{
	...
	if ((pa->npoints - 1) == 3) {	/* Triangle is special case */
		...
		pointsinslice = (uint32_t) floor((pa->npoints - 1) / 4);	/* divide the pointarray
										 * into 4 slices */
	}
	...
	sumy = 0;
	sumz = 0;
	numberofvectors = floor((pa->npoints - 1) / pointsinslice);	/* the number of vectors
									 * we try can be 3, 4 or
									 * 5 */
...
{
	/*
	 * In our plane definition we have a point on the plane and a normal
	 * vektor (pl.pv), perpendicular to the plane this vector will be
	 * paralell to the line between our inputted point above the plane
	 * and the point we are searching for on the plane. So, we already
	 * have a direction from p to find p0, but we don't know the
	 * distance.
	 */
...
pt_in_ring_3d(const POINT3DZ * p, const POINTARRAY * ring, PLANE3D * plane)
{
	...
	if (fabs(plane->pv.z) >= fabs(plane->pv.x) && fabs(plane->pv.z) >= fabs(plane->pv.y)) {	/* If the z vector of
		...
		for (i = 0; i < ring->npoints - 1; i++) {
			...
			/* a downward crossing */
				...
				if (p->x < v1.x + vt * (v2.x - v1.x)) {
					/*
					 * a valid crossing of y=p.y right of
					 * p.x
					 */
					++cn;
				}
	...
	} else if (fabs(plane->pv.y) >= fabs(plane->pv.x) && fabs(plane->pv.y) >= fabs(plane->pv.z)) {	/* If the y vector of
													 * the normal vector to
													 * the plane is larger
													 * than x and z vector
													 * we project the ring
	...
	} else {		/* Hopefully we only have the cases where x
				 * part of the normal vector is largest left */
		for (i = 0; i < ring->npoints - 1; i++) {
			double		vt;
			getPoint3dz_p(ring, i + 1, &v2);
