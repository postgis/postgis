...
_varint_u64_encode_buf(uint64_t val, uint8_t * buf) {
	...
	while (1) {
		...
		if (q > 0) {
			...
			/* in our byte to 1. */
			/*
			 * Then we put that byte in our buffer and move the
			 * pointer
			 */
			/* forward one step */
			*ptr = 0x80 | grp;
			...
			/*
			 * to read in our input value we leave the most
			 * significant bit unset
			 */
			*ptr = grp;
			ptr++;
			return ptr - buf;
...
varint_u64_decode(const uint8_t * the_start, const uint8_t * the_end, size_t * size){
	...
	while (ptr < the_end) {
		...
		if (nByte & 0x80) {
			/*
			 * We get here when there is more to read in the
			 * input varInt
			 */
			/*
			 * Here we take the least significant 7 bits of the
			 * read
			 */
			/*
			 * byte and put it in the most significant place in
			 * the result variable.
			 */
			nVal |= ((uint64_t) (nByte & 0x7f)) << nShift;
			/*
			 * move the "cursor" of the input buffer step (8
			 * bits)
			 */
			ptr++;
