...
wkt_parser_ptarray_add_coord(POINTARRAY * pa, POINT p)
{
	...
	/*
	 * While parsing the point arrays, XYM and XMZ points are both
	 * treated as XYZ
	 */
	pt.x = p.x;
	pt.y = p.y;
	...
	ptarray_append_point(pa, &pt, LW_TRUE);	/* Allow duplicate points in
						 * array */
	return pa;
}
...
wkt_parser_polygon_finalize(LWGEOM * poly, char *dimensionality)
{
	...
	if (flagdims > 2) {
		...
		/*
		 * Harmonize the flags in the sub-components with the wkt
		 * flags
		 */
...
wkt_parser_curvepolygon_finalize(LWGEOM * poly, char *dimensionality)
{
	...
	if (flagdims > 2) {
		/*
		 * If the number of dimensions are not consistent, we have a
		 * problem.
		 */
		if (flagdims != FLAGS_NDIMS(poly->flags)) {
			lwgeom_free(poly);
...
lwgeom_parser_result_free(LWGEOM_PARSER_RESULT * parser_result)
{
	...
	/*
	 * We don't free parser_result->message because it is a const *char
	 */
}
