...
/*
 * Support qsort of nodes for collection/multi types so nodes are in "spatial
 * adjacent" order prior to merging.
 */
static int
	...
	case RECT_NODE_SEG_CIRCULAR:
		{
			...
			if (seg_side == arc_side) {
				...
				/*
				 * Segment is horizontal, do we cross first
				 * point?
				 */
				if (p1->y == p3->y) {
					return 1;
...
/*
 * Only pass the head of a ring. Either a LinearRing from a polygon or a
 * CompoundCurve from a CurvePolygon. Takes a horizontal line through the
 * ring, and adds up the crossing directions. An "inside" point will be on
 * the same side ("both left" or "both right") of the edges the line crosses,
 * so it will have a non-zero return sum. An "outside" point will be on both
 * sides, and will have a zero return sum.
 */
static int
...
/*
 * Only pass in the head of an "area" type. Polygon or CurvePolygon. Sums up
 * containment of exterior (+1) and interior (-1) rings, so that zero is
 * uncontained, +1 is contained and negative is an error (multiply contained
 * by interior rings?)
 */
static int
rect_tree_area_contains_point(RECT_NODE * node, const POINT2D * pt)
{
	...
	/* See if the ring encloses the point */
		...
		/*
		 * External rings return positive containment, interior ones
		 * negative,
		 */
		/*
		 * so that a point-in-hole case nets out to zero (contained
		 * by both
		 */
...
/*
 * Pass in arbitrary tree, get back true if point is contained or on
...
/*
 * For area types, doing intersects and distance, we will need to do a
 * point-in-poly test first to find the full-contained case where an
 * intersection exists without any edges actually intersecting.
 */
static int
...
/*
 * We expect the incoming nodes to be in a spatially coherent order. For
 * incoming nodes derived from point arrays, the very fact that they are a
 * vertex list implies a reasonable ordering: points nearby in the list will
 * be nearby in space. For incoming nodes from higher level structures
 * (collections, etc) the caller should sort the nodes using a z-order first,
 * so that this merge step results in a spatially coherent structure.
 */
static RECT_NODE *
...
rect_tree_from_lwcurvepoly(const LWGEOM * lwgeom){
	...
	for (i = 0; i < lwcol->nrings; i++) {
		...
		if (node) {
			/*
			 * In the case of arc circle, it's possible for a
			 * ring to consist of a single closed edge. That will
			 * arrive as a leaf node. We need to wrap that node
			 * in an internal node with an appropriate ring type
			 * so all the other code can try and make sense of
			 * it.
			 */
			if (node->type == RECT_NODE_LEAF_TYPE) {
...
rect_tree_from_lwcollection(const LWGEOM * lwgeom){
	...
	for (i = 0; i < lwcol->ngeoms; i++) {
		...
		if (node) {
			/*
			 * Curvepolygons are collections where the
			 * sub-geometries
			 */
			/*
			 * are the rings, and will need to doint
			 * point-in-poly
			 */
			/*
			 * tests in order to do intersects and distance
			 * calculations
			 */
			/* correctly */
...
/*
 * Get an actual coordinate point from a tree to use for point-in-polygon
 * testing.
 */
...
/*
 * Work down to leaf nodes, until we find a pair of leaf nodes that
 * intersect. Prune branches that do not intersect.
 */
static int
rect_tree_intersects_tree_recursive(RECT_NODE * n1, RECT_NODE * n2)
{
	...
	if (rect_node_intersects(n1, n2)) {
		...
		/*
		 * We can only test for a true intersection if the nodes are
		 * both leaf nodes
		 */
...
{
	/*
	 * It is possible for an area to intersect another object without any
	 * edges intersecting, if the object is fully contained. If that is
	 * so, then any point in the object will be contained, so we do a
	 * quick point-in-poly test first for those cases
	 */
	...
	/*
	 * Not contained, so intersection can only happen if edges actually
	 * intersect.
	 */
...
/*
 * For slightly more speed when doing distance comparisons, where we only
 * care if d1 < d2 and not what the actual value of d1 or d2 is, we use the
 * squared distance and avoid the sqrt()
 */
...
/*
 * The closest any two objects in two nodes can be is the smallest distance
...
/*
 * The furthest apart the objects in two nodes can be is if they are at
 * opposite corners of the bbox that contains both nodes
 */
static inline double
...
/*
 * Leaf nodes represent individual edges from the original shape. As such,
 * they can be either points (if original was a (multi)point) two-point
 * straight edges (for linear types), or three-point circular arcs (for
 * curvilinear types). The distance calculations for each possible
 * combination of primitive edges is different, so there's a big case switch
 * in here to match up the right combination of inputs to the right distance
 * calculation.
 */
static double
...
/*
 * (If necessary), sort the children of each node in order of their distance
 * from the enter of the other node.
 */
static void
...
rect_tree_distance_tree_recursive(RECT_NODE * n1, RECT_NODE * n2, RECT_TREE_DISTANCE_STATE * state)
{
	...
	/*
	 * If your minimum is greater than anyone's maximum, you can't hold
	 * the winner
	 */
	...
	/*
	 * If your maximum is a new low, we'll use that as our new global
	 * tolerance
	 */
