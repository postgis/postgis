...
/*
 * static void free_homogenizebuffer(HomogenizeBuffer *buffer) { int i; for (
 * i = 0; i < NUMTYPES; i++ ) { if ( buffer->buf[i] ) {
 ...
 * typed collection. * Otherwise, return a generic collection, with the
 * subtypes in minimal * typed collections.
 */
static void
	...
	case TINTYPE:
		{
			...
			/*
			 * Strip single-entry multi-geometries down to
			 * singletons
			 */
			if (col->ngeoms == 1) {
