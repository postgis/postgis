...
lw_dist2d_distanceline(const LWGEOM * lw1, const LWGEOM * lw2, int srid, int mode)
{
	...
	if (!lw_dist2d_comp(lw1, lw2, &thedl)) {
		/*
		 * should never get here. all cases ought to be error handled
		 * earlier
		 */
	...
	/*
	 * if thedl.distance is unchanged there where only empty geometries
	 * input
	 */
...
lw_dist2d_recursive(const LWGEOM * lwg1, const LWGEOM * lwg2, DISTPTS * dl)
{
	...
	for (i = 0; i < n1; i++) {
		...
		for (j = 0; j < n2; j++) {
			...
			/*
			 * If one of geometries is empty, return. True here
			 * only means continue searching. False would have
			 * stoped the process
			 */
			...
			(g2->type == LINETYPE || g2->type == POLYGONTYPE)) {
				...
				if (dl->distance <= dl->tolerance && dl->mode == DIST_MIN)
					return LW_TRUE;	/* just a check if  the
							 * answer is already
							 * given */
			}
		}
...
lw_dist2d_point_poly(LWPOINT * point, LWPOLY * poly, DISTPTS * dl)
{
	...
	/*
	 * Inside the outer ring. Scan though each of the inner rings looking
	 * to see if its inside.  If not, distance==0. Otherwise, distance =
	 * pt to ring distance
	 */
...
lw_dist2d_line_poly(LWLINE * line, LWPOLY * poly, DISTPTS * dl)
{
	...
	for (i = 1; i < poly->nrings; i++) {
		if (!lw_dist2d_ptarray_ptarray(line->points, poly->rings[i], dl))
			return LW_FALSE;
	...
	/*
	 * No intersection, have to check if a point is inside polygon
	 */
	pt = getPoint2d_cp(line->points, 0);

	/*
	 * Outside outer ring, so min distance to a ring is the actual min
	 * distance
	 *
	 * if ( ! pt_in_ring_2d(&pt, poly->rings[0]) ) { return ; }
	 */

	/*
	 * Its in the outer ring. Have to check if its inside a hole
	 */
	for (i = 1; i < poly->nrings; i++) {
		if (ptarray_contains_point(poly->rings[i], pt) != LW_OUTSIDE) {
			/*
			 * Its inside a hole, then the actual distance is the
			 * min ring distance
			 */
			return LW_TRUE;
		}
...
lw_dist2d_poly_poly(LWPOLY * poly1, LWPOLY * poly2, DISTPTS * dl)
{
	...
	/*
	 * 2	check if poly1 has first point outside poly2 and vice versa, if
	 * so, just check outer rings here it would be possible to handle the
	 * information about wich one is inside wich one and only search for
	 * the smaller ones in the bigger ones holes.
	 */
	...
	/*
	 * 3	check if first point of poly2 is in a hole of poly1. If so check
	 * outer ring of poly2 against that hole of poly1
	 */
	pt = getPoint2d_cp(poly2->rings[0], 0);
	...
	/*
	 * 4	check if first point of poly1 is in a hole of poly2. If so check
	 * outer ring of poly1 against that hole of poly2
	 */
	pt = getPoint2d_cp(poly1->rings[0], 0);
	...
	/*
	 * 5	If we have come all the way here we know that the first point of
	 * one of them is inside the other ones outer ring and not in holes
	 * so we check wich one is inside.
	 */
	pt = getPoint2d_cp(poly1->rings[0], 0);
	if (ptarray_contains_point(poly2->rings[0], pt) != LW_OUTSIDE) {
		dl->distance = 0.0;
...
lw_dist2d_pt_ptarray(const POINT2D * p, POINTARRAY * pa, DISTPTS * dl)
{
	...
	for (t = 1; t < pa->npoints; t++) {
		...
		if (dl->distance <= dl->tolerance && dl->mode == DIST_MIN)
			return LW_TRUE;	/* just a check if  the answer is
					 * already given */
		start = end;
	}
...
lw_dist2d_ptarray_ptarray(POINTARRAY * l1, POINTARRAY * l2, DISTPTS * dl)
{
	...
	if (dl->mode == DIST_MAX) {	/* If we are searching for
					 * maxdistance we go straight to
					 * point-point calculation since the
					 * maxdistance have to be between two
					 * vertexes */
		for (t = 0; t < l1->npoints; t++) {	/* for each segment in
							 * L1 */
			start = getPoint2d_cp(l1, t);
			for (u = 0; u < l2->npoints; u++) {	/* for each segment in
								 * L2 */
		...
		for (t = 1; t < l1->npoints; t++) {	/* for each segment in
							 * L1 */
			end = getPoint2d_cp(l1, t);
			start2 = getPoint2d_cp(l2, 0);
			for (u = 1; u < l2->npoints; u++) {	/* for each segment in
								 * L2 */
...
lw_dist2d_ptarray_ptarrayarc(const POINTARRAY * pa, const POINTARRAY * pb, DISTPTS * dl)
{
	...
	if (dl->mode == DIST_MAX) {
		...
		A1 = getPoint2d_cp(pa, 0);
		for (t = 1; t < pa->npoints; t++) {	/* For each segment in
							 * pa */
			A2 = getPoint2d_cp(pa, t);
			...
			for (u = 1; u < pb->npoints; u += 2) {	/* For each arc in pb */
				...
				/*
				 * If we've found a distance within
...
lw_dist2d_ptarrayarc_ptarrayarc(const POINTARRAY * pa, const POINTARRAY * pb, DISTPTS * dl)
{
	...
	if (dl->mode == DIST_MAX) {
		...
		A1 = getPoint2d_cp(pa, 0);
		for (t = 1; t < pa->npoints; t += 2) {	/* For each segment in
							 * pa */
			A2 = getPoint2d_cp(pa, t);
			A3 = getPoint2d_cp(pa, t + 1);
			B1 = getPoint2d_cp(pb, 0);
...
lw_dist2d_seg_arc(const POINT2D * A1, const POINT2D * A2, const POINT2D * B1, const POINT2D * B2, const POINT2D * B3, DISTPTS * dl)
{
	...
	if (dist_C_D < radius_C) {
		...
		POINT2D		E, F;	/* points of interection of edge A
					 * and circle(B) */
		...
		/*
		 * If E is within A and within B then it's an interesction
		 * point
		 */
		...
		/*
		 * If F is within A and within B then it's an interesction
		 * point
		 */
	...
	/* Line misses circle. */
	/*
	 * If closest point to A on circle is within B, then that's the
	 * closest
	 */
...
lw_dist2d_pt_arc(const POINT2D * P, const POINT2D * A1, const POINT2D * A2, const POINT2D * A3, DISTPTS * dl)
{
	...
	POINT2D		X;	/* point circle(A) where line from C to P
				 * crosses */
	...
	if (p2d_same(A1, A3) || lw_pt_in_arc(&X, A1, A2, A3)) {
		...
		/*
		 * Distance is the minimum of the distances to the arc end
		 * points
		 */
		  ...
		  DISTPTS * dl)
{
	...
	int		pt_in_arc_A, pt_in_arc_B;	/* Test whether
							 * potential
	...
	/* the line joining CA to CB. */
		...
		/*
		 * If the nearest points are both within the arcs, that's our
		 * answer
		 */
	...
	/*
	 * Circles cross at two points, are either of those points in both
	 * arcs?
	 */
	/* http://paulbourke.net/geometry/2circle/ */
	else if (d < (radius_A + radius_B)) {
		POINT2D		E, F;	/* Points where circle(A) and
					 * circle(B) cross */
		...
		/*
		 * Start from D and project h units perpendicular to CA-D to
		 * get E
		 */
			     ...
			     const POINT2D * CENTER, DISTPTS * dl)
{
	...
	if (radius_A == radius_B) {
		/*
		 * Check if B1 or B3 are in the same side as A2 in the A1-A3
		 * arc
		 */
		...
		/*
		 * Check if A1 or A3 are in the same side as B2 in the B1-B3
		 * arc
		 */
...
lw_dist2d_seg_seg(const POINT2D * A, const POINT2D * B, const POINT2D * C, const POINT2D * D, DISTPTS * dl)
{
	 ...
	 *
	 * Solving the above for r and s yields (Ay-Cy)(Dx-Cx)-(Ax-Cx)(Dy-Cy)
	 * r = ----------------------------- (eqn 1)
	 * (Bx-Ax)(Dy-Cy)-(By-Ay)(Dx-Cx)
	 *
	 * (Ay-Cy)(Bx-Ax)-(Ax-Cx)(By-Ay) s = -----------------------------
	 * (eqn 2) (Bx-Ax)(Dy-Cy)-(By-Ay)(Dx-Cx) Let P be the position vector
	 * of the intersection point, then P=A+r(B-A) or Px=Ax+r(Bx-Ax)
	 * Py=Ay+r(By-Ay) By examining the values of r & s, you can also
	 * determine some other limiting conditions: If 0<=r<=1 & 0<=s<=1,
	 * intersection exists r<0 or r>1 or s<0 or s>1 line segments do not
	 * intersect If the denominator in eqn 1 is zero, AB & CD are
	 * parallel If the numerator in eqn 1 is also zero, AB & CD are
	 * collinear.
	 *
	 */
	...
	if ((r_bot == 0) || (s_bot == 0)) {
		if ((lw_dist2d_pt_seg(A, C, D, dl)) && (lw_dist2d_pt_seg(B, C, D, dl))) {
			dl->twisted = ((dl->twisted) * (-1));	/* here we change the
								 * order of inputted
								 * geometrys and that we
								 * notice by changing
								 * sign on dl->twisted */
			return ((lw_dist2d_pt_seg(C, A, B, dl)) && (lw_dist2d_pt_seg(D, A, B, dl)));	/* if all is successful
													 * we return true */
		} else {
			return LW_FALSE;	/* if any of the calls to
						 * lw_dist2d_pt_seg goes
						 * wrong we return false */
		}
	}
	...
	if (((r < 0) || (r > 1) || (s < 0) || (s > 1)) || (dl->mode == DIST_MAX)) {
		...
		if (dl->mode == DIST_MIN) {	/* If there is intersection
						 * we identify the
						 * intersection point and
						 * return it but only if we
						 * are looking for
						 * mindistance */
			POINT2D		theP;
...
{
	/*
	 * here we define two lists to hold our calculated "z"-values and the
	 * order number in the geometry
	 */
	...
	POINT2D		c1, c2;
	const		POINT2D *theP;
	float		min1X, max1X, max1Y, min1Y, min2X, max2X, max2Y,
			min2Y;
	int		t;
	int		n1 = l1->npoints;
	int		n2 = l2->npoints;
	...
	min2Y = box2->ymin;
	/*
	 * we want the center of the bboxes, and calculate the slope between
	 * the centerpoints
	 */
	...
	/*
	 * Here we calculate where the line perpendicular to the
	 * center-center line crosses the axes for each vertex if the
	 * center-center line is vertical the perpendicular line will be
	 * horizontal and we find it's crossing the Y-axes with z = y-kx
	 */
	...
	/*
	 * if the center-center line is horizontal the perpendicular line
	 * will be vertical. To eliminate problems with deviding by zero we
	 * are here mirroring the coordinate-system and we find it's crossing
	 * the X-axes with z = x-(1/k)y
	 */
	else {			/* West or East */
		k = -deltaY / deltaX;
		for (t = 0; t < n1; t++) {	/* for each segment in L1 */
...
lw_dist2d_pre_seg_seg(POINTARRAY * l1, POINTARRAY * l2, LISTSTRUCT * list1, LISTSTRUCT * list2, double k, DISTPTS * dl)
{
	...
	twist = dl->twisted;	/* to keep the incomming order between
				 * iterations */
	for (i = (n1 - 1); i >= 0; --i) {
		/*
		 * we break this iteration when we have checked every point
		 * closer to our perpendicular "checkline" than our shortest
		 * found distance
		 */
		if (((list2[0].themeasure - list1[i].themeasure)) > maxmeasure)
			break;
		for (r = -1; r <= 1; r += 2) {	/* because we are not
						 * iterating in the original
						 * pointorder we have to
						 * check the segment before
						 * and after every point */
			pnr1 = list1[i].pnr;
			...
			for (u = 0; u < n2; ++u) {
				...
				p4 = getPoint2d_cp(l2, pnr4);
				dl->twisted = twist;	/* we reset the "twist"
							 * for each iteration */
				if (!lw_dist2d_selected_seg_seg(p1, p2, p3, p4, dl))
					return LW_FALSE;

				maxmeasure = sqrt(dl->distance * dl->distance + (dl->distance * dl->distance * k * k));	/* here we "translate"
															 * the found mindistance
...
lw_dist2d_selected_seg_seg(const POINT2D * A, const POINT2D * B, const POINT2D * C, const POINT2D * D, DISTPTS * dl)
{
	...
	if ((lw_dist2d_pt_seg(A, C, D, dl)) && (lw_dist2d_pt_seg(B, C, D, dl))) {
		...
		return LW_FALSE;	/* if any of the calls to
					 * lw_dist2d_pt_seg goes wrong we
					 * return false */
	}
}
...
lw_dist2d_pt_seg(const POINT2D * p, const POINT2D * A, const POINT2D * B, DISTPTS * dl)
{
	...
	/*
	 * otherwise, we use comp.graphics.algorithms Frequently Asked
	 * Questions method
	 *
	 * (1)        AC dot AB r = --------- ||AB||^2 r has the following
	 * meaning: r=0 P = A r=1 P = B r<0 P is on the backward extension of
	 * AB r>1 P is on the forward extension of AB 0<r<1 P is interior to
	 * AB
	 */
	...
	/*
	 * This is for finding the maxdistance. the maxdistance have to be
	 * between two vertexes, compared to mindistance which can be between
	 * tvo vertexes vertex.
	 */
	if (dl->mode == DIST_MAX) {
		if (r >= 0.5) {
	...
	if (r < 0) {		/* If p projected on the line is outside
				 * point A */
		return lw_dist2d_pt_pt(p, A, dl);
	}
	if (r >= 1) {		/* If p projected on the line is outside
				 * point B or on point B */
		return lw_dist2d_pt_pt(p, B, dl);
	}

	/*
	 * If the point p is on the segment this is a more robust way to find
	 * out that
	 */
	 ...
	 * then we find that "point on segment" and send it to
	 * lw_dist2d_pt_pt
	 */
	c.x = A->x + r * (B->x - A->x);
	c.y = A->y + r * (B->y - A->y);
...
lw_dist2d_pt_pt(const POINT2D * thep1, const POINT2D * thep2, DISTPTS * dl)
{
	...
	if (((dl->distance - dist) * (dl->mode)) > 0) {	/* multiplication with
							 * mode to handle
							 * mindistance (mode=1)
							 * and maxdistance (mode
							 * = (-1) */
		dl->distance = dist;

		if (dl->twisted > 0) {	/* To get the points in right order.
					 * twisted is updated between 1 and
					 * (-1) every time the order is
...
distance2d_pt_seg(const POINT2D * p, const POINT2D * A, const POINT2D * B)
{
	 ...
	 * Questions method
	 *
	 * (1)     	      AC dot AB r = --------- ||AB||^2 r has the
	 * following meaning: r=0 P = A r=1 P = B r<0 P is on the backward
...
azimuth_pt_pt(const POINT2D * A, const POINT2D * B, double *d)
{
	...
	if (A->x < B->x) {
		...
		} else {	/* ( A->y > B->y )  - equality case handled
				 * above */
			*d = atan(fabs(A->y - B->y) / fabs(A->x - B->x))
				+ (M_PI / 2);
		}
	} else {		/* ( A->x > B->x ) - equality case handled
				 * above */
		if (A->y > B->y) {
			*d = atan(fabs(A->x - B->x) / fabs(A->y - B->y))
				+ M_PI;
