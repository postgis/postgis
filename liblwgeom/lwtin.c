...
/*
 * We supposed that the geometry is valid we could have wrong result if not
 */
int
lwtin_is_closed(const LWTIN * tin)
{
	...
	for (i = 0, carc = 0; i < tin->ngeoms; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			for (found = 0, k = 0; k < carc; k++) {
				...
				if ((arcs[k].ax == pa.x && arcs[k].ay == pa.y &&
				 ...
				 arcs[k].by == pb.y && arcs[k].bz == pb.z &&
					...
					/*
					 * Look like an invalid TIN anyway
					 * not a closed one
					 */
					if (arcs[k].cnt > 2) {
						lwfree(arcs);
			...
			if (!found) {
				...
				/*
				 * Look like an invalid TIN anyway not a
				 * closed one
				 */
				if (carc > narcs) {
					lwfree(arcs);
	...
	/*
	 * A TIN is closed if each edge is shared by exactly 2 faces
	 */
	for (k = 0; k < carc; k++) {
		if (arcs[k].cnt != 2) {
			lwfree(arcs);
