...
/*
 * Construct a new LWCIRCSTRING.  points will *NOT* be copied use
 * SRID=SRID_UNKNOWN for unknown SRID (will have 8bit type's S = 0)
 */
LWCIRCSTRING   *
lwcircstring_construct(int srid, GBOX * bbox, POINTARRAY * points)
{
	...
	/*
	 * The first arc requires three points.  Each additional arc requires
	 * two more points.  Thus the minimum point count is three, and the
	 * count must be odd.
	 */
...
/*
 * Construct a LWCIRCSTRING from an array of LWPOINTs LWCIRCSTRING dimensions
 * are large enough to host all input dimensions.
 */
LWCIRCSTRING   *
