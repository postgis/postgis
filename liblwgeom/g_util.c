...
/*
 * Type array. Note that the order of this array is important in that any
 * typename in the list must *NOT* occur within an entry before it. Otherwise
 * if we search for "POINT" at the top of the list we would also match
 * MULTIPOINT, for example.
 */
...
/*
 * We use a very simple upper case mapper here, because the system toupper()
 * function is locale dependent and may have trouble mapping lower case
 * strings to the upper case ones we expect (see, the "Turkisk I",
 * http://www.i18nguy.com/unicode/turkish-i18n.html) We could also count on
 * PgSQL sending us *lower* case inputs, as it seems to do that regardless of
 * the case the user provides for the type arguments.
 */
