...
/*
 * Return Nth vertex in GEOSGeometry as a POINT. May return NULL if the
 * geometry has NO vertexex.
 */
...
LWGEOM_GEOS_getPointN(const GEOSGeometry * g_in, uint32_t n)
{
	...
	if (!seq_in)
		return NULL;
	if (!GEOSCoordSeq_getSize(seq_in, &sz))
		return NULL;
	if (!sz)
		return NULL;

	if (!GEOSCoordSeq_getDimensions(seq_in, &dims))
		return NULL;
	...
	if (!GEOSCoordSeq_getX(seq_in, n, &val))
		return NULL;
	if (!GEOSCoordSeq_setX(seq_out, n, val))
		return NULL;
	if (!GEOSCoordSeq_getY(seq_in, n, &val))
		return NULL;
	if (!GEOSCoordSeq_setY(seq_out, n, val))
		return NULL;
	if (dims > 2) {
		if (!GEOSCoordSeq_getZ(seq_in, n, &val))
			return NULL;
		if (!GEOSCoordSeq_setZ(seq_out, n, val))
			return NULL;
	}
 ...
 * May return the input untouched (if already valid). May return geometries
 * of lower dimension (on collapses)
 */
static LWGEOM *
...
/*
 * Close the point array, if not already closed in 2d. Returns the input if
 * already closed in 2d, or a newly constructed POINTARRAY. TODO: move in
 * ptarray.c
 */
...
/*
 * Make sure all rings are closed and have > 3 points. May return the input
 * untouched.
 */
LWGEOM	       *
...
LWGEOM_GEOS_nodeLines(const GEOSGeometry * lines){
	...
	/*
	 * Union with first geometry point, obtaining full noding and
	 * dissolving of duplicated repeated points
	 *
...
/*
 * We expect initGEOS being called already. Will return NULL on error (expect
 * error handler being called by then)
 *
 */
static GEOSGeometry *
LWGEOM_GEOS_makeValidPolygon(const GEOSGeometry * gin){
	...
	if (NULL == geos_cut_edges) {
		...
		/*
		 * NOTE: the noding process may drop lines collapsing to
		 * points. We want to retrive any of those
		 */
		GEOSGeometry   *pi;
		GEOSGeometry   *po;
	...
	/*
	 * See if an area can be build with the remaining edges and if it
	 * can, symdifference with the original area. Iterate this until no
	 * more polygons can be created with left-over edges.
	 */
	while (GEOSGetNumGeometries(geos_cut_edges)) {
		...
		/*
		 * Save the new ring boundaries first (to compute further cut
		 * edges later)
		 */
		new_area_bound = GEOSBoundary(new_area);
		if (!new_area_bound) {
			/*
			 * We did check for empty area already so this must
			 * be some other error
			 */
		...
		/*
		 * Now let's re-set geos_cut_edges with what's left from the
		 * original boundary. ASSUMPTION: only the previous cut-edges
		 * can be left, so we don't need to reconsider the whole
		 * original boundaries
		 *
	...
	for (i = 0; i < ngeoms; ++i) {
		...
		if (GEOSGeomTypeId(vg) == GEOS_POINT)
			...
			for (j = 0; j < nsubgeoms; ++j) {
				...
				/*
				 * NOTE: ownership of the cloned geoms will
				 * be taken by final collection
				 */
				lines[nlines++] = GEOSGeom_clone(gc);
			}
		} else {
			/*
			 * NOTE: return from GEOSGeomType will leak but we
			 * really don't expect this to happen
			 */
...
LWGEOM_GEOS_makeValid(const GEOSGeometry * gin){
	...
	case GEOS_POINT:
		/*
		 * points are always valid, but we might have invalid
		 * ordinate values
		 */
	 ...
	 */
	{
		...
		/*
		 * TODO: handle some errors here... Lack of exceptions is
		 * annoying indeed, I'm getting old --strk;
		 */
...
lwgeom_make_valid(LWGEOM * lwgeom_in)
{
	...
	/*
	 * Step 1 : try to convert to GEOS, if impossible, clean that up
	 * first otherwise (adding only duplicates of existing points)
	 */
	...
	if (lwgeom_is_collection(lwgeom_in) && !lwgeom_is_collection(lwgeom_out)) {
		 ...
		 * lwgeom_in or otherwise we couldn't have a collection and a
		 * non-collection
		 */
