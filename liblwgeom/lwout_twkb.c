...
*/
static int
ptarray_to_twkb_buf(const POINTARRAY * pa, TWKB_GLOBALS * globals, TWKB_STATE * ts, int register_npoints, uint32_t minpoints)
{
	...
	/*
	 * If npoints is more than 127 it is unpredictable how many bytes
	 * npoints will need
	 */
	/*
	 * Then we have to store the deltas in a temp buffer to later add
	 * them after npoints
	 */
	...
	if (pa->npoints > 127) {
		/*
		 * Independent buffer to hold the coordinates, so we can put
		 * the npoints
		 */
		/*
		 * into the stream once we know how many points we actually
		 * have
		 */
		...
		if (register_npoints) {
			/*
			 * We do not store a pointer to the place where we
			 * want the npoints value
			 */
			/*
			 * Instead we store how far from the beginning of the
			 * buffer we want the value
			 */
			/*
			 * That is because we otherwise will get in trouble
			 * if the buffer is reallocated
			 */
			npoints_offset = b_p->writecursor - b_p->buf_start;

			/*
			 * We just move the cursor 1 step to make room for
			 * npoints byte
			 */
			/*
			 * We use the function append_byte even if we have no
			 * value yet,
			 */
			/*
			 * since that gives us the check for big enough
			 * buffer and moves the cursor
			 */
	...
	for (i = 0; i < pa->npoints; i++) {
		...
		for (j = 0; j < ndims; j++) {
			/*
			 * To get the relative coordinate we don't get the
			 * distance
			 */
			/*
			 * from the last point but instead the distance from
			 * our
			 */
			/*
			 * last accumulated point. This is important to not
			 * build up an
			 */
	...
	if (pa->npoints > 127) {
		...
		/*
		 * If we didn't use a temp buffer, we just write that npoints
		 * value
		 */
		/* to where it belongs */
		if (register_npoints)
...
lwmulti_to_twkb_buf(const LWCOLLECTION * col, TWKB_GLOBALS * globals, TWKB_STATE * ts)
{
	...
	if (ts->idlist) {
		for (i = 0; i < col->ngeoms; i++) {
			/*
			 * Skip empty points in multipoints, we can't
			 * represent them
			 */
...
lwgeom_to_twkb_buf(const LWGEOM * geom, TWKB_GLOBALS * globals, TWKB_STATE * ts)
{
	...
	case POLYGONTYPE:
		...
		/*
		 * All these Collection types have 'ngeoms' and 'geoms'
		 * elements
		 */
	case MULTIPOINTTYPE:
	case MULTILINETYPE:
...
lwgeom_write_to_buffer(const LWGEOM * geom, TWKB_GLOBALS * globals, TWKB_STATE * parent_state)
{
	...
	for (i = 0; i < MAX_N_DIMS; i++) {
		...
		/*
		 * Reset acumulated delta values to get absolute values on
		 * next point
		 */
	...
	/*
	 * Zig-zag the precision value before encoding it since it is a
	 * signed value
	 */
	...
	/*
	 * If we have a header_buf, we know that this function is called
	 * inside a collection
	 */
