...
lwcollection_construct_empty(uint8_t type, int srid, char hasz, char hasm)
{
	...
	ret->srid = srid;
	ret->ngeoms = 0;
	ret->maxgeoms = 1;	/* Allocate room for sub-members, just in
				 * case. */
...
lwcollection_same(const LWCOLLECTION * c1, const LWCOLLECTION * c2)
{
	 ...
	 *
	 * hit = lwalloc(sizeof(uint32_t)*c1->ngeoms); memset(hit, 0,
	 * sizeof(uint32_t)*c1->ngeoms);
	 *
	 * for (i=0; i<c1->ngeoms; i++) { char found=0; for (j=0;
	 * j<c2->ngeoms; j++) { if ( hit[j] ) continue; if (
	 * lwgeom_same(c1->geoms[i], c2->geoms[j]) ) { hit[j] = 1; found=1;
	 * break; } } if ( ! found ) return LW_FALSE; }
	 */
	...
	for (i = 0; i < col->ngeoms; i++) {
		...
		if (lwtype_is_collection(subtype)) {
			...
			for (j = 0; j < tmpcol->ngeoms; j++) {
				/*
				 * We've over-run our buffer, double the
				 * memory segment
				 */
				if (geomlistlen == geomlistsize) {
					geomlistsize *= 2;
