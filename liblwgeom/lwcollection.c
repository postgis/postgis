...
LWCOLLECTION*
lwcollection_force_dims(const LWCOLLECTION *col, int hasz, int hasm, double defaultz, double defaultm)
{
	LWCOLLECTION *colout;
	...
	else
	{
		...
		for( i = 0; i < col->ngeoms; i++ )
		{
			geoms[i] = lwgeom_force_dims(col->geoms[i], hasz, hasm, defaultz, defaultm);
		}
