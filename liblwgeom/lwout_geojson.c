...
static char *asgeojson_psurface(const LWPSURFACE *psurface, char *srs, GBOX *bbox, int precision);
...
lwgeom_to_geojson(const LWGEOM *geom, char *srs, int precision, int has_bbox)
{
	...
	case POLYHEDRALSURFACETYPE:
		return asgeojson_psurface((LWPSURFACE*)geom, srs, bbox, precision);
	default:
...
static size_t
asgeojson_psurface_size(const LWPSURFACE *psurface, char *srs, GBOX *bbox, int precision)
{
	LWPOLY *poly;
	int size;
	int i, j;
	...
	if (srs) size += asgeojson_srs_size(srs);
	if (bbox) size += asgeojson_bbox_size(FLAGS_GET_Z(psurface->flags), precision);
	size += sizeof("'coordinates':[]}");

	for (i=0; i < psurface->ngeoms; i++)
	{
		poly = psurface->geoms[i];
...
static size_t
asgeojson_psurface_buf(const LWPSURFACE *psurface, char *srs, char *output, GBOX *bbox, int precision)
{
	LWPOLY *poly;
	int i, j;
	char *ptr=output;
	...
	if (srs) ptr += asgeojson_srs_buf(ptr, srs);
	if (bbox) ptr += asgeojson_bbox_buf(ptr, bbox, FLAGS_GET_Z(psurface->flags), precision);
	ptr += sprintf(ptr, "\"coordinates\":[");
	for (i=0; i<psurface->ngeoms; i++)
	{
		if (i) ptr += sprintf(ptr, ",");
...
static char *
asgeojson_psurface(const LWPSURFACE *psurface, char *srs, GBOX *bbox, int precision)
{
	char *output;
	int size;

	size = asgeojson_psurface_size(psurface, srs, bbox, precision);
	output = lwalloc(size);
	asgeojson_psurface_buf(psurface, srs, output, bbox, precision);
