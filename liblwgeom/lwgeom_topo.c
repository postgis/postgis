  ...
  if ( numfaceedges )
  {
    ...
    for ( i=0; i<numfaceedges; ++i )
    {
      ...
      for ( j=0; j<num_signed_edge_ids; ++j )
      {
        ...
        if ( seid == e->edge_id )
        {
          /* TODO: remove entry from signed_edge_ids ? */
      ...
      /* TODO: We need to check only a single point
      ...
      /* TODO: create a GEOS point from the interior point */
      LWPOINT *epgeom = lwpoint_make2d(0, ep.x, ep.y);
      ...
      /* TODO: can be optimized by computing this on our side rather
       *       than on GEOS (saves conversion of big edges) */
      /* TODO: check that bounding box shortcut is taken, or use
      ...
      if ( contains == 2 )
      {
        ...
        for ( i=0; i<numedges; ++i ) {
          lwline_release(ring_edges[i].geom);
        }
        lwfree(ring_edges);
