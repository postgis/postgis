...
void
lwt_BackendIfaceRegisterCallbacks(LWT_BE_IFACE * iface,
 ...
 *
 * @param myself the id of an edge to skip, if any (for ChangeEdgeGeom). Can
 * use 0 for none.
 *
 * Return -1 on cross or error, 0 if everything is fine. Note that before
 * returning -1, lwerror is invoked...
 */
static int
...
lwt_AddIsoEdge(LWT_TOPOLOGY * topo, LWT_ELEMID startNode,
	...
	/*
	 * NOT IN THE SPECS: A closed edge is never isolated (as it forms a
	 * face)
	 */
	...
	/*
	 * Check for: existence of nodes nodes faces match Extract: nodes
	 * face id nodes geoms
	 */
	num_nodes = 2;
	...
	for (i = 0; i < num_nodes; ++i) {
		...
		if (!skipISOChecks) {
			if (n->node_id == startNode) {
				/*
				 * l) Check that start point of acurve match
				 * start node geoms.
				 */
				...
				/*
				 * m) Check that end point of acurve match
				 * end node geoms.
				 */
	 ...
	 *
	 * the nodes anode and anothernode are no more isolated because now
...
lwt_ModEdgeSplit(LWT_TOPOLOGY * topo, LWT_ELEMID edge,
	...
	if (!split_col)
		return -1;	/* should have raised an exception */
...
typedef struct edgeend_t {
	...
	LWT_ELEMID	cwFace;
	/*
	 * Signed identifier of next counterclockwise edge
	 * (+outgoing,-incoming)
	 */
	LWT_ELEMID	nextCCW;
...
/*
 * Get first distinct vertex from endpoint @param pa the pointarray to seek
 * points in @param ref the point we want to search a distinct one @param
 * from vertex index to start from (will really start from "from"+dir) @param
 * dir  1 to go forward -1 to go backward @return 0 if edge is collapsed (no
 * distinct points)
 */
static int
...
/*
 * Return non-zero on failure (lwerror is invoked in that case) Possible
 * failures: -1 no two distinct vertices exist -2 azimuth computation failed
 * for first edge end
 */
static int
 ...
 * around a node, starting from the given azimuth, and take note of the face
 * on the both sides.
 *
 * @param topo the topology to act upon @param node the identifier of the
 * node to analyze @param data input (myaz) / output (nextCW, nextCCW)
 * parameter @param other edgeend, if also incident to given node (closed
 * edge). @param myedge_id identifier of the edge id that data->myaz belongs
 * to @return number of incident edges found
 *
 */
static int
		       ...
		       edgeend * other, int myedge_id)
{
	...
	if (other) {
		...
		minaz = maxaz = azdif;
		/*
		 * TODO: set nextCW/nextCCW/cwFace/ccwFace to
		 * other->something ?
		 */
...
/*
 * Get a point internal to the line and write it into the "ip" parameter
 *
...
_lwt_GetInteriorEdgePoint(const LWLINE * edge, POINT2D * ip)
{
	...
	if (pa->npoints < 2)
		return 0;	/* empty or structurally collapsed */
	...
	if (p2d_same(&fp, &lp))
		return 0;	/* no distinct points in edge */
...
_lwt_MakeRingShell(LWT_TOPOLOGY * topo, LWT_ELEMID * signed_edge_ids, int num_signed_edge_ids){
	...
	/*
	 * Should now build a polygon with those edges, in the order given by
	 * GetRingEdges.
	 */
	POINTARRAY     *pa = NULL;
	for (i = 0; i < num_signed_edge_ids; ++i) {
	...
	/*
	 * NOTE: the ring may very well have collapsed components, which
	 * would make it topologically invalid
	 */
 ...
 *
 * @param topo the topology to act upon @param sedge edge id and walking side
 * and direction (forward,left:positive backward,right:negative) @param face
 * the face in which the edge identifier is known to be @param mbr_only do
 * not create a new face but update MBR of the current
 *
 * @return: -1: if mbr_only was requested 0: if the edge does not form a ring
 * -1: if it is impossible to create a face on the requested side ( new face
 * on the side is the universe ) -2: error >0 : id of newly added face
 */
static LWT_ELEMID
_lwt_AddFaceSplit(LWT_TOPOLOGY * topo,
	...
	if (face == 0) {
		...
		if (!isccw) {
			...
			lwfree(signed_edge_ids);
			/*
			 * Face on the left side of this ring is the universe
			 * face. Next call (for the other side) should create
			 * the split face
			 */
		...
		if (isccw) {
			{
				...
				updface.mbr = (GBOX *) shellbox;	/* const cast, we won't
									 * free it, later */
	...
	if (face != 0 && !isccw) {
		...
		newface.mbr = (GBOX *) shellbox;	/* const cast, we won't
							 * free it, later */
	}
	...
	if (oldface) {
		newface.mbr = NULL;	/* it is a reference to oldface
					 * mbr... */
	...
	if (face != 0 && !isccw) {	/* ring is clockwise in a real face */
		/*
		 * face shrinked, must update all non-contained edges and
		 * nodes
		 */
	...
	if (numfaceedges) {
		...
		for (i = 0; i < numfaceedges; ++i) {
			...
			/*
			 * (2.1) skip edges whose ID is in the list of
			 * boundary edges ?
			 */
			...
			/*
			 * We need to check only a single point (to avoid
			 * collapsed elements of the shell polygon giving
			 * false positive). The point but must not be an
			 * endpoint.
			 */
			if (!_lwt_GetInteriorEdgePoint(e->geom, &ep)) {
				...
				lwfree(signed_edge_ids);
				lwpoly_free(shell);
				lwfree(forward_edges);	/* contents owned by
							 * edges */
				lwfree(backward_edges);	/* contents owned by
							 * edges */
				_lwt_release_edges(edges, numfaceedges);
			...
			/*
			 * IDEA: can be optimized by computing this on our
			 * side rather than on GEOS (saves conversion of big
			 * edges)
			 */
			/*
			 * IDEA: check that bounding box shortcut is taken,
			 * or use shellbox to do it here
			 */
			...
			/*
			 * (2.2) skip edges (NOT, if newface_outside)
			 * contained in shell
			 */
			if (newface_outside) {
				if (contains) {
...
_lwt_AddEdge(LWT_TOPOLOGY * topo,
	...
	if (!_lwt_FirstDistinctVertex2D(pa, &p2, pa->npoints - 1, -1, &pn)) {
		lwgeom_free(cleangeom);
		/*
		 * This should never happen as we checked the edge while
		 * computing first edgend
		 */
	...
	/*
	 * Check endpoints existance, match with Curve geometry and get face
	 * information (if any)
	 */
	...
	/*
	 * If we don't have faces setup by now we must have encountered a
	 * malformed topology (no containing_face on isolated nodes, no
	...
	/*
	 * Link prev_left to us (if it's not us already)
	 */
	...
	/*
	 * Link prev_right to us (if it's not us already)
	 */
	...
	/*
	 * NOT IN THE SPECS... set containing_face = null for start_node and
	 * end_node if they where isolated
	 *
	 */
	...
	if (modFace > -1) {
		...
		/*
		 * IDEA: avoid building edge ring if input is closed, which
		 * means we know in advance it splits a face
		 */
		...
		if (modFace) {
			...
			if (newface < 0) {
				...
				/*
				 * must be forming a maximal ring in
				 * universal face
				 */
...
_lwt_FaceByEdges(LWT_TOPOLOGY * topo, LWT_ISO_EDGE * edges, int numfaceedges){
	...
	for (i = 0; i < numfaceedges; ++i) {
		/*
		 * NOTE: skipping edges with same face on both sides,
		 * although correct, results in a failure to build faces from
		 * invalid topologies as expected by legacy tests. TODO:
		 * update legacy tests expectances/unleash this skipping ?
		 */
...
/*
 * Find which edge from the "edges" set defines the next portion of the given
 * "ring".
 *
 * The edge might be either forward or backward.
 *
 * @param ring The ring to find definition of. It is assumed it does not
 * contain duplicated vertices. @param from offset of the ring point to start
 * looking from @param edges array of edges to search into @param numedges
 * number of edges in the edges array
 *
 * @return index of the edge defining the next ring portion or -1 if no edge
 * was found to be part of the ring
 */
static int
		      ...
		      const LWT_ISO_EDGE * edges, int numedges)
{
	...
	for (i = 0; i < numedges; ++i) {
		...
		if (p2d_same(&p1, &p2)) {
			...
			/*
			 * first point matches, let's check next non-equal
			 * one
			 */
			for (j = 1; j < epa->npoints; ++j) {
				...
				if (p2d_same(&p1, &p2))
					continue;
				/*
				 * we assume there are no duplicated points
				 * in ring
				 */
				...
				break;	/* we want to check a single
					 * non-equal next vertex */
			}
		...
		if (!match) {
			...
			if (p2d_same(&p1, &p2)) {
				LWDEBUGF(1, "Last point of edge %" LWTFMT_ELEMID
					 " matches ring vertex %d", isoe->edge_id, from);
				/*
				 * last point matches, let's check next
				 * non-equal one
				 */
				for (j = 2; j <= epa->npoints; j++) {
					...
					/*
					 * we won't check duplicated edge
					 * points
					 */
					if (p2d_same(&p1, &p2))
						continue;
					/*
					 * we assume there are no duplicated
					 * points in ring
					 */
					getPoint2d_p(ring, from + 1, &pt);
					LWDEBUGF(1, "Ring's point %d is %g,%g",
...
/*
 * Reverse values in array between "from" (inclusive) and "to" (exclusive)
 * indexes
 */
static void
...
/*
 * Rotate values in array between "from" (inclusive) and "to" (exclusive)
 * indexes, so that "rotidx" is the new value at "from"
 */
static void
...
lwt_GetFaceEdges(LWT_TOPOLOGY * topo, LWT_ELEMID face_id, LWT_ELEMID * *out)
{
	...
	if (!face) {
		/*
		 * _lwt_FaceByEdges should have already invoked lwerror in
		 * this case
		 */
		_lwt_release_edges(edges, numfaceedges);
		return -1;
	...
	for (i = 0; i < facepoly->nrings; ++i) {
		...
		/*
		 * now "scroll" the list of edges so that the one with
		 * smaller absolute edge_id is first
		 */
...
_lwt_EdgeMotionArea(LWLINE * geom, int isclosed){
	...
	if (isclosed) {
		...
		/*
		 * make valid, in case the edge self-intersects on its
		 * first-last vertex segment
		 */
...
lwt_ChangeEdgeGeom(LWT_TOPOLOGY * topo, LWT_ELEMID edge_id, LWLINE * geom)
{
	...
	/*
	 * Not in the specs: if the edge is closed, check we didn't change
	 * winding ! (should be part of isomorphism checking)
	 */
	...
	/*
	 * Not in the specs: Check topological isomorphism
	 */
	...
	//3. if any
node beside endnodes are found:
		if (numnodes > (1 + isclosed ? 0 : 1)) {
			{
				GEOSGeometry   *oarea, *narea;
				const		GEOSPreparedGeometry *oareap,
					       *nareap;
				...
				//3.2.bail out if any
					node is in one and not the other
				...
				for (i = 0; i < numnodes; ++i) {
					...
					if (ocont != ncont) {
						...
						lwfree(wkt);	/* would not necessarely
								 * reach this point */
						return -1;
					}
				}
	...
	/*
	 * Check edge adjacency before TODO: can be optimized to gather
	 * azimuths of all edge ends once
	 */
	...
	/*
	 * -- Update faces MBR of left and right faces -- TODO: think about
	 * ways to optimize this part, like see if --       the old edge
	 * geometry partecipated in the definition --       of the current
	 * MBR (for shrinking) or the new edge MBR --       would be larger
	 * than the old face MBR... --
	 */
	...
	if (!gbox_same(oldbox, newbox)) {
		{
			...
			/* no need to update twice the same face.. */
				...
				faces[facestoupdate++].mbr = nface2->bbox;	/* ownership left to
										 * nface */
			}
...
lwt_RemoveIsoNode(LWT_TOPOLOGY * topo, LWT_ELEMID nid)
{
	...
	/*
	 * TODO: notify to caller about node being removed ? See
	 * https://trac.osgeo.org/postgis/ticket/3231
	 */
...
lwt_RemIsoEdge(LWT_TOPOLOGY * topo, LWT_ELEMID id)
{
	...
	/*
	 * TODO: notify to caller about edge being removed ? See
	 * https://trac.osgeo.org/postgis/ticket/3248
	 */
 ...
 *
 * @param of old face id (never 0 as you cannot remove face 0) @param nf new
 * face id @return 0 on success, -1 on backend error
 */
static int
 ...
 * previously occupied by the removed edge if modFace is 1, identifier of the
 * created face (0 if none) if modFace is 0.
 */
static LWT_ELEMID
_lwt_RemEdge(LWT_TOPOLOGY * topo, LWT_ELEMID edge_id, int modFace){
	...
	int		fnode_edges = 0;	/* number of edges on the
						 * first node (excluded the
						 * one being removed ) */
	int		lnode_edges = 0;	/* number of edges on the
						 * last node (excluded the
						 * one being removed ) */
	...
	/*
	 * Id of face that will take up all the space previously taken by
	 * left and right faces of the edge
	 */
	...
	if (edge->face_left == edge->face_right) {
		...
		if (edge->face_left == 0 || edge->face_right == 0) {
			...
			/*
			 * update mbr of floodface as union of mbr of both
			 * faces
			 */
			...
			gbox_merge(box2, box1);	/* box1 is now the union of
						 * the two */
			newface.mbr = box1;
			if (modFace) {
		...
		/*
		 * Update face references for edges and nodes still
		 * referencing the removed face(s)
		 */
	...
	/*
	 * If any of the edge nodes remained isolated, set containing_face =
	 * floodface
	 */
	if (!fnode_edges) {
...
_lwt_HealEdges(LWT_TOPOLOGY * topo, LWT_ELEMID eid1, LWT_ELEMID eid2,
	...
	if (commonnode != -1) {
		...
		for (i = 0; i < num_node_edges; ++i) {
			int		r;
			if (node_edges[i].edge_id == eid1)
				continue;
			if (node_edges[i].edge_id == eid2)
				continue;
			commonnode = -1;
	...
	/*
	 * -- Update next_left_edge/next_right_edge for -- any edge having
	 * them still pointing at the edge being removed -- (eid2 only when
	 * modEdge, or both otherwise) -- -- NOTE: -- e#freenode is 1 when
	 * edge# end node was the common node -- and -1 otherwise. This gives
	 * the sign of possibly found references -- to its "free" (non
	 * connected to other edge) endnode. -- e2sign is -1 if edge1
	 * direction is opposite to edge2 direction, -- or 1 otherwise. --
	 */
	...
	if (!modEdge) {
		/*
		 * update edges connected to e1's boundary from their end
		 * node
		 */
		...
		/*
		 * update edges connected to e1's boundary from their start
		 * node
		 */
	...
	/*
	 * -- -- NOT IN THE SPECS: -- Drop composition rows involving second
	 * -- edge, as the first edge took its space, -- and all affected
	 * TopoGeom have been previously checked -- for being composed by
	 * both edges.
	 */
...
lwt_GetFaceByPoint(LWT_TOPOLOGY * topo, LWPOINT * pt, double tol){
	...
	/*
	 * Not in a face, may be in universe or on edge, let's check for
	 * distance
	 */
	/*
...
/*
 * Return the smallest delta that can perturbate the given value
 */
...
/*
 * Return the smallest delta that can perturbate the given point static
 * inline double _lwt_minTolerancePoint2d( const POINT2D* p ) { double max =
 * FP_ABS(p->x); if ( max < FP_ABS(p->y) ) max = FP_ABS(p->y); return
 * _lwt_minToleranceDouble(max); }
 */

/*
 * Return the smallest delta that can perturbate the maximum absolute value
...
_lwt_minTolerance(LWGEOM * g)
{
	...
	if (max < FP_ABS(gbox->xmax))
		max = FP_ABS(gbox->xmax);
	if (max < FP_ABS(gbox->ymin))
		max = FP_ABS(gbox->ymin);
	if (max < FP_ABS(gbox->ymax))
		max = FP_ABS(gbox->ymax);
...
/*
 * @param findFace if non-zero the code will determine which face contains
 * the given point (unless it is known to be NOT isolated) @param moved if
 * not-null will be set to 0 if the point was added w/out any snapping or 1
 * otherwise.
 */
static LWT_ELEMID
_lwt_AddPoint(LWT_TOPOLOGY * topo, LWPOINT * point, double tol, int
	...
	/*
	 * -- 1. Check if any existing node is closer than the given
	 * precision --    and if so pick the closest TODO: use WithinBox2D
	 */
	...
	if (num) {
		...
		for (i = 0; i < num; ++i) {
			 ...
			 */
			/*
			 * must be closer than tolerated, unless distance is
			 * zero
			 */
			if (dist && dist >= tol)
				continue;
	...
	/*
	 * -- 2. Check if any existing edge falls within tolerance --    and
	 * if so split it by a point projected on it TODO: use WithinBox2D
	 */
	...
	if (num) {
		...
		for (i = 0; i < num; ++i) {
			...
			if (moved)
				*moved = lwgeom_same(prj, pt) ? 0 : 1;
			if (lwgeom_has_z(pt)) {
				{
					/*
					 * -- This is a workaround for
					 * ClosestPoint lack of Z support: --
			...
			if (!contains) {
				{
					...
					/*
					 * In order to reduce the robustness
					 * issues, we'll pick an edge that
					 * contains the projected point, if
					 * possible
					 */
					if (i + 1 < num) {
					...
					/*
					 * -- The tolerance must be big
					 * enough for snapping to happen --
					 * and small enough to snap only to
					 * the projected point. --
					 * Unfortunately ST_Distance returns
					 * 0 because it also uses -- a
					 * projected point internally, so we
					 * need another way.
					 */
					...
					/*
					 * -- Snapping currently snaps the
					 * first point below tolerance -- so
					 * may possibly move first point. See
					 * ticket #1631
					 */
					...
					if (-1 == lwt_ChangeEdgeGeom(topo, edge_id, snapline)) {
						/*
						 * TODO: should have invoked
						 * lwerror already, leaking
						 * memory
						 */
						lwgeom_free(prj);
						lwgeom_free(snapedge);
			...
			if (-1 == id) {
				/*
				 * TODO: should have invoked lwerror already,
				 * leaking memory
				 */
				lwgeom_free(prj);
				_lwt_release_edges(edges, num);
			...
			/*
			 * TODO: decimate the two new edges with the given
			 * tolerance ?
			 *
			 * the edge identifiers to decimate would be: edge_id
			 * and "id" The problem here is that decimation of
			 * existing edges may introduce intersections or
			 * topological inconsistencies, for example:
			 *
			 * - A node may end up falling on the other side of
			 * the edge - The decimated edge might intersect
		...
		if (-1 == id) {
			/*
			 * should have invoked lwerror already, leaking
			 * memory
			 */
...
/*
 * Return identifier of an equal edge, 0 if none or -1 on error (and lwerror
...
/*
 * Add a pre-noded pre-split line edge. Used by lwt_AddLine Return edge id, 0
 * if none added (empty edge), -1 on error
 *
 * @param handleFaceSplit if non-zero the code will check if the newly added
 * edge would split a face and if so would create new faces accordingly.
 * Otherwise it will set left_face and right_face to null (-1)
 */
static LWT_ELEMID
_lwt_AddLineEdge(LWT_TOPOLOGY * topo, LWLINE * edge, double tol,
	...
	nid[0] = _lwt_AddPoint(topo, start_point,
			  _lwt_minTolerance(lwpoint_as_lwgeom(start_point)),
			       handleFaceSplit, &mm);
	lwpoint_free(start_point);	/* too late if lwt_AddPoint calls
					 * lwerror */
	if (nid[0] == -1)
		return -1;	/* lwerror should have been called */
	moved += mm;
	...
	nid[1] = _lwt_AddPoint(topo, end_point,
			    _lwt_minTolerance(lwpoint_as_lwgeom(end_point)),
			       handleFaceSplit, &mm);
	moved += mm;
	lwpoint_free(end_point);	/* too late if lwt_AddPoint calls
					 * lwerror */
	if (nid[1] == -1)
		return -1;	/* lwerror should have been called */

	/*
	 * -- Added endpoints may have drifted due to tolerance, so -- we
	 * need to re-snap the edge to the new nodes before adding it
	 */
	if (moved) {
		...
		end_point = NULL;
		for (i = 0; i < nn; ++i) {
			if (node[i].node_id == nid[0])
				start_point = node[i].geom;
			if (node[i].node_id == nid[1])
				end_point = node[i].geom;
		}
		if (!start_point || !end_point) {
			if (nn)
				_lwt_release_nodes(node, nn);
		...
		if (col) {
			{
				...
				/*
				 * Check if the so-snapped edge collapsed
				 * (see #1650)
				 */
				if (col->ngeoms == 0) {
	...
	if (id == -1) {
		if (tmp)
			lwgeom_free(tmp);	/* probably too late, due to
						 * internal lwerror */
		return -1;
	}
	if (id) {
		if (tmp)
			lwgeom_free(tmp);	/* possibly takes "edge" down
						 * with it */
		return id;
	}
	...
	/*
	 * Remove consecutive vertices below given tolerance on edge addition
	 */
	if (tol) {
		{
			...
			LWDEBUGF(1, "_lwt_GetEqualEdge returned %" LWTFMT_ELEMID, id);
			if (id == -1) {
				lwgeom_free(tmp);	/* probably too late,
							 * due to internal
							 * lwerror */
				return -1;
			}
			if (id) {
				lwgeom_free(tmp);	/* takes "edge" down
							 * with it */
				return id;
			}
		}
	...
	if (id == -1) {
		lwgeom_free(tmp);	/* probably too late, due to internal
					 * lwerror */
		return -1;
	}
...
/*
 * Simulate split-loop as it was implemented in pl/pgsql version of
...
_lwt_AddLine(LWT_TOPOLOGY * topo, LWLINE * line, double tol, int *nedges,
	...
	*nedges = -1;		/* error condition, by default */
	...
	if (tol) {
		{
			...
			tmp = lwline_as_lwgeom(clean);	/* NOTE: might collapse
							 * to non-simple */
	...
	if (tmp != (LWGEOM *) line)
		lwgeom_free(tmp);
	if (!noded)
		return NULL;	/* should have called lwerror already */
	...
	if (numedges) {
		{
			...
			for (i = 0; i < numedges; ++i) {
				...
				/*
				 * must be closer than tolerated, unless
				 * distance is zero
				 */
				if (dist && dist >= tol)
					continue;
	...
	/*
	 * 2.1. Node with existing nodes within tol TODO: check if we should
	 * be only considering _isolated_ nodes!
	 */
	...
	if (numnodes) {
		{
			...
			/*
			 * -- re-node to account for ST_Snap introduced
			 * self-intersections -- See
			 * http://trac.osgeo.org/postgis/ticket/1714 -- TODO:
			 * consider running UnaryUnion once after all noding
			 */
	 ...
	 * needed) and then check all edges for existing already ( so to save
	 * a DB scan for each edge to be added )
	 */
...
lwt_AddPolygon(LWT_TOPOLOGY * topo, LWPOLY * poly, double tol, int *nfaces)
{
	...
	*nfaces = -1;		/* error condition, by default */
	...
	/*
	 * -- Find faces covered by input polygon -- NOTE: potential snapping
	 * changed polygon edges
	 */
	...
	if (nfacesinbox) {
		...
		for (j = 0; j < nfacesinbox; ++j) {
			...
			/*
			 * check if a point on this face surface is covered
			 * by our polygon
			 */
			...
			/*
			 * check if a point on this face's surface is covered
			 * by our polygon
			 */
			...
			if (!covers) {
				continue;	/* we're not composed by this
						 * face */
			}
	...
	/*
	 * possibly 0 if non face's surface point was found to be covered by
	 * input polygon
	 */
	*nfaces = num;
...
typedef struct LWT_EDGERING_T {
	/*
	 * Signed edge identifiers positive ones are walked in their
	 * direction, negative ones in the opposite direction
	 */
...
/*
 * WARNING: use of 'j' is intentional, not to clash with 'i' used in
 * LWT_EDGERING_CLEAN
 */
...
/*
 * Identifier for a placeholder face that will be used to mark hole rings
 */
 ...
 *
 * Edge identifiers are signed, those with negative identifier need to be
 * updated their right_face, those with positive identifier need to be
 * updated their left_face.
 *
 * @param face identifier of the face bound by the ring @return 0 on success,
 * -1 on error
 */
static int
...
_lwt_EdgeRingSignedArea(LWT_EDGERING_POINT_ITERATOR * it)
{
	...
	if (!_lwt_EdgeRingIterator_next(it, &P1))
		return 0.0;
	if (!_lwt_EdgeRingIterator_next(it, &P2))
		return 0.0;
...
_lwt_EdgeRingCrossingCount(const POINT2D * p, LWT_EDGERING_POINT_ITERATOR * it)
{
	...
	if (!_lwt_EdgeRingIterator_next(it, &v1))
		return cn;
	v0 = v1;
 ...
 *
 * Create and register face to shell (CCW) walks, register arbitrary negative
 * face_id to CW rings.
 *
 * Push CCW rings to shells, CW rings to holes.
 *
 * The ownership of the "geom" and "ids" members of the LWT_EDGERING pushed
 * to the given LWT_EDGERING_ARRAYS are transferred to caller.
 *
 * @param side 1 for left side, -1 for right side
 *
 ...
 *
 * @param registered id of registered face. It will be a negative number for
 * holes or isolated edge strips (still registered in the face table, but
 * only temporary).
 *
			    ...
			    LWT_ELEMID * registered)
{
	...
	if (isccw) {
		...
		*registered = newface.face_id;
		...
		/*
		 * update ring edges set new face_id on resp. side to
		 * *registered
		 */
...
_lwt_FindFaceContainingRing(LWT_TOPOLOGY * topo, LWT_EDGERING * ring,
	...
	for (i = 0; i < candidates.size; ++i) {
		...
		/*
		 * Skip test if a containing shell was already found and this
		 * shell's bbox is not contained in the other
		 */
		...
		if (contains) {
			/*
			 * Continue until all shells are tested, as we want
			 * to use the one with the smallest bounding box
			 */
			/*
			 * IDEA: sort shells by bbox size, stopping on first
			 * match
			 */
...
/*
 * @return -1 on error (and report error), 1 if faces beside the universal
 * one exist 0 otherwise
 */
static int
_lwt_CheckFacesExist(LWT_TOPOLOGY * topo)
{
	...
	if (faces)
		_lwt_release_faces(faces, nelems);
	return nelems;
}
...
{
	/*
	 * Fetch all edges Sort edges by edge_id Mark all edges' left and
	 * right face as -1 Iteratively: Fetch next edge with left or right
	 * face == -1 For each side with face == -1: Find ring on its side If
	 * ring is CCW: create a new face, assign to the ring edges'
	 * appropriate side If ring is CW (face needs to be same of
	 * external): assign a negative face_id the ring edges' appropriate
	 * side Now for each edge with a negative face_id on the side: Find
	 * containing face (mbr cache and all) Update with id of containing
	 * face
	 */
	...
	/*
	 * Check if Topology already contains some Face (ignoring the
	 * Universal Face)
	 */
