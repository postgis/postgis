...
LWPOLY*
lwpoly_force_dims(const LWPOLY *poly, int hasz, int hasm, double defaultz, double defaultm)
{
	LWPOLY *polyout;
	...
	else
	{
		...
		for( i = 0; i < poly->nrings; i++ )
		{
			rings[i] = ptarray_force_dims(poly->rings[i], hasz, hasm, defaultz, defaultm);
		}
