...
lwgeom_force_clockwise(LWGEOM * lwgeom)
{
	...
	case TRIANGLETYPE:
		...
		/*
		 * Not handle POLYHEDRALSURFACE and TIN as they are supposed
		 * to be well oriented
		 */
...
lwgeom_as_multi(const LWGEOM * lwgeom)
{
	...
	if (lwgeom_is_empty(lwgeom)) {
		...
		/*
		 * Sub-geometries are not allowed to have bboxes or SRIDs,
		 * move the bbox to the collection
		 */
		box = ogeoms[0]->bbox;
		ogeoms[0]->bbox = NULL;
...
lwgeom_as_curve(const LWGEOM * lwgeom)
{
	...
	/*
	 * int hasz = FLAGS_GET_Z(lwgeom->flags); int hasm =
	 * FLAGS_GET_M(lwgeom->flags); int srid = lwgeom->srid;
	 */
...
lwgeom_same(const LWGEOM * lwgeom1, const LWGEOM * lwgeom2)
{
	...
	if (lwgeom1->bbox && lwgeom2->bbox) {
		/*
		 * lwnotice("bbox1:%p, bbox2:%p", lwgeom1->bbox,
		 * lwgeom2->bbox);
		 */
...
{
	lwgeom->srid = SRID_UNKNOWN;	/* TODO: To be changed to
					 * SRID_UNKNOWN */
}
...
lwgeom_affine(LWGEOM * geom, const AFFINE * affine)
{
	...
	switch (type) {
		/*
		 * Take advantage of fact tht pt/ln/circ/tri have same memory
		 * structure
		 */
	case POINTTYPE:
	case LINETYPE:
...
lwgeom_subdivide_recursive(const LWGEOM * geom, uint32_t maxvertices, uint32_t depth, LWCOLLECTION * col, const GBOX * clip)
{
	...
	if (lwgeom_is_collection(geom) && geom->type != MULTIPOINTTYPE) {
		...
		for (i = 0; i < incol->ngeoms; i++) {
			/*
			 * Don't increment depth yet, since we aren't
			 * actually subdividing geomtries yet
			 */
