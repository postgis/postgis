...
/*
 * When clustering lists with NULL elements, they will get this as their
 * cluster number. (All the other clusters will be non-negative)
 */
#define KMEANS_NULL_CLUSTER -1

/*
 * If the algorithm doesn't converge within this number of iterations, it
 * will return with a failure error code.
 */
...
update_r(POINT2D * *objs, int *clusters, uint32_t n, POINT2D * *centers, uint32_t k)
{
	...
	for (i = 0; i < n; i++) {
		...
		/*
		 * Don't try to cluster NULL objects, just add them to the
		 * "unclusterable cluster"
		 */
		if (!obj) {
...
kmeans(POINT2D * *objs, int *clusters, uint32_t n, POINT2D * *centers, uint32_t k)
{
	...
	/*
	 * Previous cluster state array. At this time, r doesn't mean
	...
	for (i = 0; i < KMEANS_MAX_ITERATIONS && !converged; i++) {
		...
		/*
		 * if all the cluster numbers are unchanged, we are at a
		 * stable solution
		 */
...
lwgeom_cluster_2d_kmeans(const LWGEOM * *geoms, uint32_t n, uint32_t k)
{
	...
	/*
	 * An array of objects to be analyzed. All NULL values will be
	 * returned in the  KMEANS_NULL_CLUSTER.
	 */
	POINT2D	      **objs;
	...
	/*
	 * The vector of cluster means. We have to allocate a chunk of memory
	 * for these because we'll be mutating them in the kmeans algorithm
	 */
	...
	/*
	 * Array of sums of distances to a point from accepted cluster
	 * centers
	 */
	...
	for (i = 1; i < k; i++) {
		...
		for (j = 0; j < n; j++) {
			/*
			 * empty objs and accepted clusters are already
			 * marked with distance = -1
			 */
			if (distances[j] < 0)
				continue;
			...
			/*
			 * greedily take a point that's farthest from any of
			 * accepted clusters
			 */
