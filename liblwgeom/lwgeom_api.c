...
/*
 * Returns the float thats very close to the input, but >=. handles the funny
 * differences in float4 and float8 reps.
 */
inline float
...
/*
 * Copies a point from the point array into the parameter point will set
 * point's z=NO_Z_VALUE if pa is 2d will set point's m=NO_M_VALUE if pa is 3d
 * or 2d
 *
...
/*
 * Copies a point from the point array into the parameter point will set
 * point's z=NO_Z_VALUE  if pa is 2d will set point's m=NO_M_VALUE  if pa is
 * 3d or 2d
 *
...
/*
 * Copy a point from the point array into the parameter point will set
 * point's z=NO_Z_VALUE if pa is 2d NOTE: point is a real POINT3DZ *not* a
 * pointer
 */
POINT3DZ
...
/*
 * Copy a point from the point array into the parameter point will set
 * point's z=NO_Z_VALUE if pa is 2d
 *
...
getPoint3dz_p(const POINTARRAY * pa, uint32_t n, POINT3DZ * op)
{
	...
	/*
	 * if input POINTARRAY has the Z, it is always at third position so
	 * make a single copy
	 */
	...
	/*
	 * Otherwise copy the 2d part and initialize Z to NO_Z_VALUE
	 */
	else {
		memcpy(op, ptr, sizeof(POINT2D));
		op->z = NO_Z_VALUE;
...
/*
 * Copy a point from the point array into the parameter point will set
 * point's m=NO_Z_VALUE if pa has no M
 *
...
getPoint3dm_p(const POINTARRAY * pa, uint32_t n, POINT3DM * op)
{
	...
	/*
	 * if input POINTARRAY has the M and NO Z, we can issue a single
	 * memcpy
	 */
	if (zmflag == 1) {
		memcpy(op, ptr, sizeof(POINT3DM));
	...
	/*
	 * Otherwise copy the 2d part and initialize M to NO_M_VALUE
	 */
	memcpy(op, ptr, sizeof(POINT2D));

	/*
	 * Then, if input has Z skip it and copy next double, otherwise
...
/*
 * Copy a point from the point array into the parameter point z value (if
 * present) is not returned.
 *
...
/*
 * set point N to the given value NOTE that the pointarray can be of any
 * dimension, the appropriate ordinate values will be extracted from it
 *
 */
void
