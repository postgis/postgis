...
/*
 * Utility struct to keep GEOSSTRtree and associated structures to be freed
 * after use
 */
struct STRTree {
	GEOSSTRtree    *tree;
	...
	for (p = 0; p < num_geoms; p++) {
		...
		for (i = 0; i < cxt.num_items_found; i++) {
			...
			if (p != q && UF_find(uf, p) != UF_find(uf, q)) {
				...
				/*
				 * Don't build prepared a geometry around a
				 * Point or MultiPoint - there are some
				 * problems in the implementation, and it's
				 * not clear there would be a performance
				 * benefit in any case.  (See #3433)
				 */
...
/*
 * Union p's cluster with q's cluster, if q is not a border point of another
 * cluster. Applicable to DBSCAN with minpoints > 1.
 */
static void
union_if_available(UNIONFIND * uf, uint32_t p, uint32_t q, char *is_in_core, char *in_a_cluster)
{
	if (in_a_cluster[q]) {
		/*
		 * Can we merge p's cluster with q's cluster?  We can do this
		 * only if both p and q are considered _core_ points of their
		 * respective clusters.
		 */
		if (is_in_core[q]) {
			UF_union(uf, p, q);
...
/*
 * An optimized DBSCAN union for the case where min_points == 1. If
 * min_points == 1, then we don't care how many neighbors we find; we can
 * union clusters on the fly, as as we go through the distance calculations.
 * This potentially allows us to avoid some distance computations altogether.
 */
static int
...
union_dbscan_general(LWGEOM * *geoms, uint32_t num_geoms, UNIONFIND * uf, double eps, uint32_t min_points, char **in_a_cluster_ret)
{
	...
	for (p = 0; p < num_geoms; p++) {
		...
		/*
		 * We didn't find enough points to do anything, even if they
		 * are all within eps.
		 */
		...
		for (i = 0; i < cxt.num_items_found; i++) {
			...
			if (num_neighbors >= min_points) {
				/*
				 * If we've already identified p as a core
				 * point, and it's already in the same
				 * cluster in q, then there's nothing to
				 * learn by computing the distance.
				 */
				if (UF_find(uf, p) == UF_find(uf, q))
					continue;

				/*
				 * Similarly, if q is already identifed as a
				 * border point of another cluster, there's
				 * no point figuring out what the distance
				 * is.
				 */
			...
			if (mindist <= eps) {
				/*
				 * If we haven't hit min_points yet, we don't
				 * know if we can union p and q. Just set q
				 * aside for now.
				 */
				if (num_neighbors < min_points) {
					...
					/*
					 * If we just hit min_points, we can
					 * now union all of the neighbor
					 * geometries we've been saving.
					 */
					if (num_neighbors == min_points) {
						uint32_t	j;
					...
					/*
					 * If we're above min_points, no need
					 * to store our neighbors, just go
					 * ahead and union them now.  This
					 * may allow us to cut out some
...
combine_geometries(UNIONFIND * uf, void **geoms, uint32_t num_geoms, void ***clusterGeoms, uint32_t * num_clusters, char is_lwgeom)
{
	...
	/*
	 * Combine components of each cluster into their own
	 * GeometryCollection
	 */
	...
	for (i = 0, j = 0, k = 0; i < num_geoms; i++) {
		...
		if ((i == num_geoms - 1) ||
			...
			if (k >= uf->num_clusters) {
				/*
				 * Should not get here - it means that we
				 * have more clusters than uf->num_clusters
				 * thinks we should.
				 */
				return LW_FAILURE;
			}
