...
/*
 * Construct a new LWLINE.  points will *NOT* be copied use SRID=SRID_UNKNOWN
 * for unknown SRID (will have 8bit type's S = 0)
 */
LWLINE	       *
...
/*
 * Construct a LWLINE from an array of point and line geometries LWLINE
 * dimensions are large enough to host all input dimensions.
 */
LWLINE	       *
lwline_from_lwgeom_array(int srid, uint32_t ngeoms, LWGEOM * *geoms)
{
	...
	for (i = 0; i < ngeoms; i++) {
		...
		if (g->type == POINTTYPE) {
			...
			/*
			 * Append the new line points, de-duplicating against
			 * the previous points. Duplicated points internal to
			 * the linestring are untouched.
			 */
...
/*
 * Construct a LWLINE from an array of LWPOINTs LWLINE dimensions are large
 * enough to host all input dimensions.
 */
LWLINE	       *
lwline_from_ptarray(int srid, uint32_t npoints, LWPOINT * *points)
{
	...
	for (i = 0; i < npoints; i++) {
		...
		if (FLAGS_GET_Z(points[i]->flags))
			hasz = LW_TRUE;
		if (FLAGS_GET_M(points[i]->flags))
			hasm = LW_TRUE;
		if (hasz && hasm)
			break;	/* Nothing more to learn! */
	}
...
lwline_interpolate_points(const LWLINE * line, double length_fraction, char repeat)
{
	...
	for (i = 0; i < ipa->npoints - 1 && points_found < points_to_interpolate; i++) {
		...
		/*
		 * If our target distance is before the total length we've
		 * seen so far. create a new point some distance down the
		 * current segment.
		 */
