...
DefineGeom(pxr::UsdStageRefPtr stage, const pxr::SdfPath &path, int srid, const char *type_name, int has_z, int has_m)
{
	...
	SetCustomAttribute(prim, pxr::TfToken(TOKEN_POSTGIS_SRID), pxr::SdfValueTypeNames->Int, srid);
	SetCustomAttribute(
	    prim, pxr::TfToken(TOKEN_POSTGIS_TYPE_NAME), pxr::SdfValueTypeNames->String, std::string(type_name));
	SetCustomAttribute(prim, pxr::TfToken(TOKEN_POSTGIS_HAS_Z), pxr::SdfValueTypeNames->Int, has_z);
	SetCustomAttribute(prim, pxr::TfToken(TOKEN_POSTGIS_HAS_M), pxr::SdfValueTypeNames->Int, has_m);

	return geometry;
}

static void
ReadPointArray(pxr::VtVec3fArray &points, pxr::VtVec4dArray &xyzm, const POINTARRAY *pa)
{
...
Writer::WritePoint(const LWPOINT *p)
{
	...
	/* set constant width */
	auto widths_attr = geometry.CreateWidthsAttr();
	widths_attr.Set(pxr::VtFloatArray(1, m_width));
	geometry.SetWidthsInterpolation(pxr::UsdGeomTokens->constant);
...
Writer::WritePolygon(const LWPOLY *p)
{
	...
	ReadPointArray(points, xyzm, boundary_pa);
...
Writer::Writer(const std::string &root_name, const std::string &geom_name, float width)
    : m_root_name(root_name), m_geom_name(geom_name), m_width(width)
{
