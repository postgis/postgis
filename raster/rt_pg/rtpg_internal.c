...
/*
 * reverse string search function from http://stackoverflow.com/a/1634398
 */
char	       *
...
rtpg_getSR(int srid)
{
	...
	/*
	 * SELECT CASE WHEN (upper(auth_name) = 'EPSG' OR upper(auth_name) =
	 * 'EPSGA') AND length(COALESCE(auth_srid::text, '')) > 0 THEN
	 * upper(auth_name) || ':' || auth_srid WHEN
	 * length(COALESCE(auth_name, '') || COALESCE(auth_srid::text, '')) >
	 * 0 THEN COALESCE(auth_name, '') || COALESCE(auth_srid::text, '')
	 * ELSE '' END, proj4text, srtext FROM spatial_ref_sys WHERE srid = X
	 * LIMIT 1
	 */
