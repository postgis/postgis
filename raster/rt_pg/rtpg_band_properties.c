...
Datum
RASTER_getBandPixelType(PG_FUNCTION_ARGS)
{
...
Datum
RASTER_getBandPixelTypeName(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_raster	raster = NULL;
	rt_band		band = NULL;
	...
	/*
	 * We don't need to check for NULL pointer, because if out of memory,
	 * palloc exit via elog(ERROR). It never returns NULL.
	 */
...
Datum
RASTER_getBandNoDataValue(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_raster	raster = NULL;
	rt_band		band = NULL;
...
Datum
RASTER_bandIsNoData(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_raster	raster = NULL;
	rt_band		band = NULL;
...
Datum
RASTER_getBandPath(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_raster	raster = NULL;
	rt_band		band = NULL;
...
Datum
RASTER_bandmetadata(PG_FUNCTION_ARGS)
{
	...
	if (call_cntr < max_calls) {
		...
		if (bmd2[call_cntr].bandpath)
			pfree(bmd2[call_cntr].bandpath);
...
Datum
RASTER_setBandNoDataValue(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
...
Datum
RASTER_setBandIsNoData(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
	rt_raster	raster = NULL;
...
Datum
RASTER_setBandPath(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
	rt_raster	raster = NULL;
