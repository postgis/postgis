...
Datum
RASTER_lib_version(PG_FUNCTION_ARGS)
{
	char		ver[64];
	text	       *result;
...
Datum
RASTER_lib_build_date(PG_FUNCTION_ARGS)
{
...
Datum
RASTER_gdal_version(PG_FUNCTION_ARGS)
{
...
Datum
RASTER_minPossibleValue(PG_FUNCTION_ARGS)
{
	...
	/*
	 * correct pixsize of unsigned pixel types example: for PT_8BUI, the
	 * value is CHAR_MIN but if char is signed, the value returned is
	 * -127 instead of 0.
	 */
	switch (pixtype) {
	case PT_1BB:
...
Datum
RASTER_memsize(PG_FUNCTION_ARGS)
{
