...
struct rtpg_nmapalgebra_arg_t {
	...
	uint8_t	       *ownsdata;	/* is the raster self owned or just a
					 * pointer to another raster */
	int	       *nband;	/* source raster's band index, 0-based */
	uint8_t	       *hasband;	/* does source raster have band at
					 * index nband? */
...
static int
rtpg_nmapalgebra_rastbandarg_process(rtpg_nmapalgebra_arg arg, ArrayType * array, int *allnull, int *allempty, int *noband)
{
	Oid		etype;
	Datum	       *e;
	bool	       *nulls;
...
Datum
RASTER_nMapAlgebra(PG_FUNCTION_ARGS)
{
	rtpg_nmapalgebra_arg arg = NULL;
	rt_iterator	itrset;
	ArrayType      *maskArray;
	...
	if (!PG_ARGISNULL(1) || get_fn_expr_argtype(fcinfo->flinfo, 1) == REGPROCEDUREOID) {
		...
		/*
		 * TODO: consider adding checks of the userfunction
		 * parameters should be able to use get_fn_expr_argtype() of
		 * fmgr.c
		 */
...
Datum
RASTER_nMapAlgebraExpr(PG_FUNCTION_ARGS)
{
	 ...
	 * process expressions
	 *
	 * exprpos elements are: 1 - expression => spi_plan[0] 4 -
	 * nodata1expr => spi_plan[1] 5 - nodata2expr => spi_plan[2]
	 */
	for (i = 0; i < arg->callback.exprcount; i++) {
		char	       *expr = NULL;
		char	       *tmp = NULL;
		...
		if (arg->callback.expr[i].spi_argcount) {
			...
			for (j = 0, k = 0; j < argkwcount; j++) {
				if (arg->callback.expr[i].spi_argpos[j] < 1)
					continue;
	...
	/*
	 * switch to prior memory context to ensure memory allocated in
	 * correct context
	 */
...
rtpg_union_noarg(rtpg_union_arg arg, rt_raster raster)
{
	...
	for (; i < arg->numband; i++) {
		...
		/*
		 * add new working rt_raster but only if working raster
		 * already exists
		 */
...
Datum
RASTER_union_transfn(PG_FUNCTION_ARGS)
{
	...
	if (nargs > 2) {
		...
		if (!PG_ARGISNULL(2)) {
			...
			case TEXTOID:{
					...
					/*
					 * bandarg set. only possible after
					 * the first call to function
					 */
					if (iwr->bandarg) {
						/*
						 * only reallocate if new
						 * bands need to be added
						 */
						if (numband > idx) {
						 ...
						 * step happening below
						 */
						else
							idx = iwr->numband;
					}
					/*
					 * bandarg not set, first call to
					 * function
					 */
					else {
					...
					/*
					 * set initial values for bands that
					 * are "new"
					 */
		...
		for (i = 0; i < iwr->numband; i++) {
			...
			/*
			 * add new working rt_raster but only if working
	...
	for (i = 0; i < iwr->numband; i++) {
		...
		for (j = 0; j < iwr->bandarg[i].numraster; j++) {
			...
			/*
			 * if rasters AND bands are present, use copy
			 * approach
			 */
			if (!isempty[0] && !isempty[1] && hasband[0] && hasband[1]) {
				...
				/*
				 * with iterator raster, copy data to output
				 * raster
				 */
...
Datum
RASTER_union_finalfn(PG_FUNCTION_ARGS)
{
	rtpg_union_arg	iwr;
	rt_raster _rtn = NULL;
	rt_raster _raster = NULL;
...
RASTER_clip(PG_FUNCTION_ARGS)
{
	...
	/*
	 * intersection is empty AND extent type is INTERSECTION, return
	 * empty
	 */
...
RASTER_reclass(PG_FUNCTION_ARGS)
{
	...
	/*
	 * process each element of reclassarg each element is the index of
	 * the band to process, the set of reclass ranges and the output
	 * band's pixeltype
	 */
	for (i = 0; i < n; i++) {
		if (nulls[i])
		...
		for (a = 0, j = 0; a < comma_n; a++) {
			...
			for (b = 0; b < colon_n; b++) {
				...
				for (c = 0; c < dash_n; c++) {
					...
					/*
					 * strsplit removes dash (a.k.a.
					 * negative sign), compare now to
					 * restore
					 */
					if (c < 1)
...
Datum
RASTER_colorMap(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rtpg_colormap_arg arg = NULL;
	char	       *junk = NULL;
	...
	if (PG_ARGISNULL(2)) {
		...
		for (i = 0; i < arg->nentry; i++) {
			...
			for (j = 0; j < arg->nelement; j++) {
				...
				/*
				 * first element is ALWAYS a band value,
				 * percentage OR "nv" string
				 */
				if (j == 0) {
					char	       *percent = NULL;
...
Datum
RASTER_mapAlgebraExpr(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
	rt_raster	raster = NULL;
	...
	if (!PG_ARGISNULL(3)) {
		...
		/*
		 * pfree(expression); expression = NULL;
		 */
	}
	...
	if (initexpr != NULL && strstr(initexpr, "[rast") == NULL) {
		...
		/*
		 * Return the new raster as it will be before computing pixel
		 * by pixel
		 */
...
Datum
RASTER_mapAlgebraFct(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
	rt_raster	raster = NULL;
	...
	for (x = 0; x < width; x++) {
		for (y = 0; y < height; y++) {
			...
			if (ret == ES_NONE) {
				...
				/*
				 * Add pixel positions if callback has proper
				 * # of args
				 */
...
Datum
RASTER_mapAlgebraFctNgb(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
	rt_raster	raster = NULL;
	...
	rt_band		band = NULL;
	rt_band		newband = NULL;
	int		x, y, nband, width, height, ngbwidth, ngbheight,
			winwidth, winheight, u, v, nIndex, nNullItems;
	double		r, rpix;
	double		newnodatavalue = 0.0;
	double		newinitialvalue = 0.0;
	double		newval = 0.0;
	...
	int		intReplace = 0;
	float		fltReplace = 0;
	bool		valuereplace = false, pixelreplace, nNodataOnly = true,
			nNullSkip = false;
	...
	if (strcmp(strFromText, "VALUE") == 0)
		...
		/*
		 * if the text is not "IGNORE" or "NULL", it may be a
		 * numerical value
		 */
		...
		/*
		 * this setting means that the neighborhood should be skipped
	...
	/*
	 * The dimensions of the neighborhood array, for creating a
	...
	/*
	 * Get information about the type of item in the multi-dimensional
	 * array (float8).
	 */
	get_typlenbyvalalign(FLOAT8OID, &typlen, &typbyval, &typalign);
	...
	for (x = 0 + ngbwidth; x < width - ngbwidth; x++) {
		for (y = 0 + ngbheight; y < height - ngbheight; y++) {
			/*
			 * populate an array with the pixel values in the
			 * neighborhood
			 */
			nIndex = 0;
			nNullItems = 0;
			...
			for (u = x - ngbwidth; u <= x + ngbwidth; u++) {
				for (v = y - ngbheight; v <= y + ngbheight; v++) {
					...
					if (ret == ES_NONE) {
						...
						/*
						 * Fill this will NULL if we
						 * can't read the raster
						 * pixel.
						 */
						neighborData[nIndex] = PointerGetDatum(NULL);
						neighborNulls[nIndex] = true;
			...
			/**
	                 ...
	                 * nodata value has already been set by the first optimization
	                 **/
			if (!(nNodataOnly ||	/* neighborhood only contains
						 * NODATA -- OR -- */
			      (nNullSkip && nNullItems > 0) ||	/* neighborhood should
								 * skip any NODATA
								 * cells, and a NODATA
								 * cell was detected --
								 * OR -- */
			      (valuereplace && nNullItems > 0))) {	/* neighborhood should
									 * replace NODATA cells
									 * with the central
									 * pixel value, and a
									 * NODATA cell was
									 * detected */
				...
				/*
				 * Assign the neighbor matrix as the first
...
Datum
RASTER_mapAlgebra2(PG_FUNCTION_ARGS)
{
	 ...
	 * Returning NULL"); for (k = 0; k < set_count; k++) { if (_rast[k]
	 * != NULL) rt_raster_destroy(_rast[k]); if (pgrastpos[k] != -1)
	 * PG_FREE_IF_COPY(pgrast[k], pgrastpos[k]); } PG_RETURN_NULL(); }
	 */
	...
	/*
	 * determine who called this function Arg 4 will either be text or
	 * regprocedure
	 */
	...
	case TEXTOID:{
			 ...
			 * process expressions
			 *
			 * spi_exprpos elements are: 4 - expression =>
			 * spi_plan[0] 7 - nodata1expr => spi_plan[1] 8 -
			 * nodata2expr => spi_plan[2]
			 */
	...
	case REGPROCEDUREOID:{
			...
			if (!PG_ARGISNULL(4)) {
				...
				/*
				 * TODO: consider adding checks of the
				 * userfunction parameters should be able to
				 * use get_fn_expr_argtype() of fmgr.c
				 */
