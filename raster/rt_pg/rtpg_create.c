...
Datum
RASTER_makeEmpty(PG_FUNCTION_ARGS)
{
	uint16		width = 0, height = 0;
	double		ipx = 0, ipy = 0, scalex = 0, scaley = 0, skewx = 0,
			skewy = 0;
	...
	if (raster == NULL)
		PG_RETURN_NULL();	/* error was supposedly printed
					 * already */
...
Datum
RASTER_addBand(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	...
	/*
	 * process each element of addbandargset each element is the index of
	 * where to add the new band, new band's pixeltype, the new band's
	 * initial value and the new band's NODATA value if NOT NULL
	 */
	for (i = 0; i < n; i++) {
		if (nulls[i])
			continue;
...
Datum
RASTER_addBandRasterArray(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
...
Datum
RASTER_addBandOutDB(PG_FUNCTION_ARGS)
{
	rt_pgraster    *pgraster = NULL;
	rt_pgraster    *pgrtn = NULL;
...
Datum
RASTER_copyBand(PG_FUNCTION_ARGS)
{
...
RASTER_tile(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		/*
		 * if (!numbands) { elog(NOTICE, "Raster provided has no
		 * bands"); rt_raster_destroy(arg1->raster.raster);
		 * pfree(arg1); PG_FREE_IF_COPY(pgraster, 0);
		 * MemoryContextSwitchTo(oldcontext);
		 * SRF_RETURN_DONE(funcctx); }
		 */
	...
	if (call_cntr < max_calls) {
		...
		/*
		 * override user-defined if edge tile (only possible if
		 * padding is false
		 */
		if (ex || ey) {
			/* right edge */
