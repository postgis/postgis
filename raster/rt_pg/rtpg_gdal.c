...
Datum RASTER_isVisible(PG_FUNCTION_ARGS)
{
	...
	src_srid = clamp_srid(rt_raster_get_srid(raster));
	if (gserialized_get_srid(gser_a) != src_srid ||
	    gserialized_get_srid(gser_b) != src_srid) {
		rt_raster_destroy(raster);
		PG_FREE_IF_COPY(pgraster, 0);
	...
	if (!isfinite(pa.x) || !isfinite(pa.y) ||
	    (gserialized_has_z(gser_a) && !isfinite(pa.z)) ||
	    !isfinite(pb.x) || !isfinite(pb.y) ||
	    (gserialized_has_z(gser_b) && !isfinite(pb.z))) {
		lwgeom_free(geom_a);
		lwgeom_free(geom_b);
		rt_raster_destroy(raster);
		PG_FREE_IF_COPY(pgraster, 0);
	...
	snprintf(dst_filename, sizeof(dst_filename), "/vsimem/postgis_isvisible_%d_%llu.tif",
		MyProcPid, (unsigned long long)dst_file_seq++);

	dst_ds = GDALViewshedGenerate(
		src_band,
		"GTiff",
		dst_filename,
		NULL,
	...
	if (GDALGetGeoTransform(dst_ds, dst_gt) != CE_None ||
	    GDALInvGeoTransform(dst_gt, dst_igt) == 0) {
		GDALClose(dst_ds);
		VSIUnlink(dst_filename);
		GDALClose(src_ds);
		if (src_drv != NULL && destroy_src_drv) {
	...
	if (!isfinite(dst_x) || !isfinite(dst_y) ||
	    dst_x < 0.0 || dst_y < 0.0 ||
	    dst_x >= GDALGetRasterXSize(dst_ds) ||
