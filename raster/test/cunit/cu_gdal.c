...
test_gdal_polygonize()
{
	...
	/*
	 * for (i = 0; i < nPols; i++) { wkt = lwgeom_to_text((const LWGEOM
	 * *) gv[i].geom); printf("(i, val, geom) = (%d, %f, %s)\n", i,
	 * gv[i].val, wkt); rtdealloc(wkt); }
	 */
...
test_raster_to_gdal()
{
	...
	/*
	 * FILE *fh = NULL; fh = fopen("/tmp/out.tif", "w"); fwrite(gdal,
	 * sizeof(uint8_t), gdalSize, fh); fclose(fh);
	 */
