...
\! rm -f /tmp/postgis_rt_bytea_binary.copy

\copy (SELECT id, rast FROM rt_bytea_test ORDER BY id) TO '/tmp/postgis_rt_bytea_binary.copy' WITH (FORMAT binary)
...
\copy rt_bytea_binary_copy FROM '/tmp/postgis_rt_bytea_binary.copy' WITH (FORMAT binary)
