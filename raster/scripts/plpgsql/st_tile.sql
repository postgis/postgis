...
--                 ST_MinPossibleValue(ST_BandPixelType(rast, band)) is used.
--
...
-- FROM srtm_22_03;
----------------------------------------------------------------------------------------------------------------------
DROP FUNCTION ST_Tile(rast raster, width integer, height integer, padwithnodata boolean, nodatavalue double precision);
...
-- Old test for the extent of the tile grid
SELECT ST_AsBinary((gv).geom) geom,  (gv).val
FROM (SELECT ST_PixelAsPolygons(ST_Rescale(ST_TestRaster(0, 0, 1), 0.003, 0.004)) gv) foo;
