...
/*
 * Set given number of bits of the given byte, starting from given bitOffset
 * (from the first) to the given value.
 *
 * Examples: char ch; ch=0; setBits(&ch, 1, 1, 0) -> ch==8 ch=0; setBits(&ch,
 * 3, 2, 1) -> ch==96 (0x60)
 *
...
setBits(char *ch, double val, int bits, int bitOffset)
{
	...
	/*
	 * Shift ival so the significant bits start at the first bit
	 */
...
/*
 * unused up to now void write_uint8(uint8_t** from, uint8_t v) { assert(NULL
 * != from);
 *
 * (*from)++ = v; }
 */
...
/*
 * unused up to now void write_int8(uint8_t** from, int8_t v) { assert(NULL
 * != from);
 *
 * (*from)++ = v; }
 */
...
/*
 * unused up to now void write_int16(uint8_t** to, uint8_t littleEndian,
 * int16_t v) { assert(NULL != to);
 *
 * if ( littleEndian ) { (*to)[0] = v & 0x00FF; (*to)[1] = v >> 8; } else {
 * (*to)[1] = v & 0x00FF; (*to)[0] = v >> 8; } to += 2; }
 */
...
/*
 * unused up to now void write_uint32(uint8_t** to, uint8_t littleEndian,
 * uint32_t v) { assert(NULL != to);
 *
 * if ( littleEndian ) { (*to)[0] = v & 0x000000FF; (*to)[1] = ( v &
 * 0x0000FF00 ) >> 8; (*to)[2] = ( v & 0x00FF0000 ) >> 16; (*to)[3] = ( v &
 * 0xFF000000 ) >> 24; } else { (*to)[3] = v & 0x000000FF; (*to)[2] = ( v &
 * 0x0000FF00 ) >> 8; (*to)[1] = ( v & 0x00FF0000 ) >> 16; (*to)[0] = ( v &
 * 0xFF000000 ) >> 24; } to += 4; }
 */
...
/*
 * unused up to now void write_int32(uint8_t** to, uint8_t littleEndian,
 * int32_t v) { assert(NULL != to);
 *
 * if ( littleEndian ) { (*to)[0] = v & 0x000000FF; (*to)[1] = ( v &
 * 0x0000FF00 ) >> 8; (*to)[2] = ( v & 0x00FF0000 ) >> 16; (*to)[3] = ( v &
 * 0xFF000000 ) >> 24; } else { (*to)[3] = v & 0x000000FF; (*to)[2] = ( v &
...
/*
 * unused up to now void write_float32(uint8_t** from, uint8_t littleEndian,
 * float f) { union { float f; uint32_t i; } u;
 *
 * u.f = f; write_uint32(from, littleEndian, u.i); }
 */
...
/*
 * unused up to now void write_float64(uint8_t** to, uint8_t littleEndian,
 * double v) { union { double d; uint64_t i; } u;
 *
 * assert(NULL != to);
 *
 * u.d = v;
 *
 * if ( littleEndian ) { (*to)[0] =   u.i & 0x00000000000000FFULL; (*to)[1] =
 * ( u.i & 0x000000000000FF00ULL ) >> 8; (*to)[2] = ( u.i &
 * 0x0000000000FF0000ULL ) >> 16; (*to)[3] = ( u.i & 0x00000000FF000000ULL )
 * >> 24; (*to)[4] = ( u.i & 0x000000FF00000000ULL ) >> 32; (*to)[5] = ( u.i
 * & 0x0000FF0000000000ULL ) >> 40; (*to)[6] = ( u.i & 0x00FF000000000000ULL
 * ) >> 48; (*to)[7] = ( u.i & 0xFF00000000000000ULL ) >> 56; } else {
 * (*to)[7] =   u.i & 0x00000000000000FFULL; (*to)[6] = ( u.i &
 * 0x000000000000FF00ULL ) >> 8; (*to)[5] = ( u.i & 0x0000000000FF0000ULL )
 * >> 16; (*to)[4] = ( u.i & 0x00000000FF000000ULL ) >> 24; (*to)[3] = ( u.i
 * & 0x000000FF00000000ULL ) >> 32; (*to)[2] = ( u.i & 0x0000FF0000000000ULL
 * ) >> 40; (*to)[1] = ( u.i & 0x00FF000000000000ULL ) >> 48; (*to)[0] = (
 * u.i & 0xFF00000000000000ULL ) >> 56; } to += 8; }
 */
...
rt_raster_serialized_size(rt_raster raster) {
	...
	for (i = 0; i < raster->numBands; ++i) {
		...
		/*
		 * XXX jorgearevalo: bug here. If the size is actually
		 * 8-bytes aligned, this line will add 8 bytes trailing
		 * padding, and it's not necessary
		 */
		/* size += 8 - (size % 8); */
		if (size % 8)
			size += 8 - (size % 8);
...
rt_raster_serialize(rt_raster raster)
{
	...
	/* Set size */
	/*
	 * NOTE: Value of rt_raster.size may be updated in returned object,
	 * for instance, by rt_pg layer to store value calculated by
	 * SET_VARSIZE.
	 */
	raster->size = size;
...
rt_raster_deserialize(void *serialized, int header_only){
	...
	/*
	 * NOTE: Value of rt_raster.size may be different than actual size of
	 * raster data being read. See note on SET_VARSIZE in
	 * rt_raster_serialize function above.
	 */
