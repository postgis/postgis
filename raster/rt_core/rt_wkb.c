...
rt_band_from_wkb(
	...
	if (band->offline) {
...

		{
			...
			*ptr += sz + 1;
			...
			/*
			 * TODO: How could we know if the offline band is a
			 * nodata band?
			 */
			/* trust in the force */
			/* band->isnodata = FALSE; */
	...
	*ptr += sz;
	...
	/*
	 * Should now flip values if > 8bit and littleEndian !=
...
rt_raster_from_wkb(const uint8_t * wkb, uint32_t wkbsize){
	...
	if (!rast->numBands) {
		/*
		 * Here ptr should have been left to right after last used
		 * byte
		 */
		if (ptr < wkbend) {
...
rt_raster_from_hexwkb(const char *hexwkb, uint32_t hexwkbsize){
	...
	rtdealloc(wkb);		/* as long as rt_raster_from_wkb copies
				 * memory */
