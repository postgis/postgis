 ...
 *
 * Copyright (C) 2013 Bborie Park <dustymugs@gmail.com> Copyright (C)
	    ...
	    double *i_mag, double *j_mag, double *theta_i, double *theta_ij)
{
	...
	*j_mag = sqrt(xskew * xskew + yscale * yscale);
	...
	/*
	 * Rotation ======== Two steps: 1] calculate the magnitude of the
	 * angle between the x axis and the i basis vector. 2] Calculate the
	 * sign of theta_i based on the angle between the y axis and the i
	 * basis vector.
	 */
	*theta_i = acos(xscale / (*i_mag));	/* magnitude */
	 ...
	 * =================================== Two steps: 1] calculate the
	 * magnitude of the angle between the j basis vector and the i basis
	 * vector. 2] Calculate the sign of theta_ij based on the angle
	 * between the perpendicular of the i basis vector and the j basis
	 * vector.
	 */
...
{
	double		o11, o12, o21, o22;	/* calculated geotransform
						 * coefficients */
	int		success;
...
rt_raster_compute_skewed_raster(
	...
	/*
	 * skew not provided or skew is zero, return raster of correct dim
	...
	for (i = 0; i < 2; i++) {
		...
		do {
			...
			/*
			 * check the four corners that they are covered along
			 * the specific axis pixel column should be >= 0
			 */
			for (j = 0; j < 4; j++) {
				switch (j) {
					/* upper-left */
...
)
{
	...
	rtn_ds = GDALCreateCopy(
				rtn_drv,
				"/vsimem/out.dat",	/* should be fine
							 * assuming this is in a
							 * process */
				src_ds,
				FALSE,	/* should copy be strictly
					 * equivelent? */
...
rt_raster_gdal_drivers(uint32_t * drv_count, uint8_t can_write) {
	...
	for (i = 0, j = 0; i < count; i++) {
		...
		/*
		 * Starting with GDAL 2.0, vector drivers can also be
		 * returned
		 */
		...
		rtn[j].can_read = 1;
		/*
		 * we require CreateCopy and VirtualIO support to write to
		 * GDAL
		 */
...
rt_raster_to_gdal_mem(
	...
	*rtn_drv = drv;
	...
	for (i = 0; i < count; i++) {
		...
		/*
		 * For all pixel types other than PT_8BSI, set pointer to
		 * start of data
		 */
		if (pt != PT_8BSI) {
	...
	for (i = 1; i <= numBands; i++) {
		...
		for (iYBlock = 0; iYBlock < nYBlocks; iYBlock++) {
			for (iXBlock = 0; iXBlock < nXBlocks; iXBlock++) {
				...
				/*
				 * if block width is same as raster width,
				 * shortcut
				 */
...
rt_raster_gdal_rasterize(
	...
	if (
		...
		/*
		 * for now, force scale to be in left-right, top-down
		 * orientation
		 */
	...
	if (NULL != skew_x) {
		...
		/*
		 * negative scale-x affects skew for now, force skew to be in
	...
	if (NULL != skew_y) {
		...
		/*
		 * positive scale-y affects skew for now, force skew to be in
		 * left-right, top-down orientation
		 */
		if (
		    NULL != scale_y &&
	...
	/*
	 * if geometry is a point, a linestring or set of either and bounds
	 * not set, increase extent by a pixel to avoid missing points on
	 * border
	 *
	 * a whole pixel is used instead of half-pixel due to backward
	 * compatibility with GDAL 1.6, 1.7 and 1.8.  1.9+ works fine with
	 * half-pixel.
	 */
	...
	if ((
		...
		/*
		 * see if geometry is properly contained by extent all parts
		 * of geometry lies within extent
		 */
