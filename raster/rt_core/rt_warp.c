...
)
{
	...
	/*
	 * special case when src_srs and dst_srs is NULL and raster's
	 * geotransform matrix is default
	 */
	if (
		...
		/*
		 * substitute spatial info (lack of) with a real one
		 * EPSG:32731 (WGS84/UTM zone 31s)
		 */
		if (
	...
	/*
	 * don't use suggested dimensions as use of suggested scales on
	 * suggested extent will result in suggested dimensions
	 */
	_dim[0] = 0;
	_dim[1] = 0;
	...
	if (hasnodata) {
		...
		for (i = 0; i < numBands; i++) {
			...
			if (!rt_band_get_hasnodata_flag(band)) {
				/*
				 * based on line 1004 of gdalwarp.cpp the
				 * problem is that there is a chance that
				 * this number is a legitimate value
				 */
	...
	if (subspatial) {
		...
		/*
		 * NOTE: the times 10 is because it was divided by 10 in a
		 * section above, I'm not sure the above division was needed
		 */
		gt[1] = _scale[0] * 10;
		gt[5] = -1 * _scale[1] * 10;
