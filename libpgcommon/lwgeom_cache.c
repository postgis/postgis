...
/*
 * Generic statement caching infrastructure. We cache the following kinds of
 * objects:
 *
 * geometries-with-trees PreparedGeometry, RTree, CIRC_TREE, RECT_TREE
 * srids-with-projections projPJ
 *
 * Each GenericCache* has a type, and after that some data. Similar to
 * generic LWGEOM*. Test that the type number is what you expect before
 * casting and de-referencing struct members.
 */
typedef struct {
	int		type;
	char		data[1];
...
/*
 * Although there are only two basic kinds of cache entries, the actual trees
 * stored in the geometries-with-trees pattern are quite diverse, and they
 * might be used in combination, so we have one slot for each tree type as
 * well as a slot for projections.
 */
typedef struct {
