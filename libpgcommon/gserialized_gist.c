...
/*
 * Convert a double-based GBOX into a float-based GIDX, ensuring the float
...
gserialized_set_gidx(GSERIALIZED * g, GIDX * gidx)
{
	...
	/*
	 * Serialized has no box. We need to allocate enough space for the
	 * old data plus the box, and leave a gap in the memory segment to
	 * write the new values into.
	 */
	else {
		...
		memcpy(g_out, g, 8);
		/*
		 * Copy the body of g into place after leaving space for the
		 * box
		 */
		ptr = g_out->data;
		ptr += box_size;
...
gserialized_datum_get_gidx_p(Datum gsdatum, GIDX * gidx)
{
	 ...
	 * the 32 bytes * of floats necessary to hold the 8 floats of the
	 * largest XYZM index * bounding box, so 40 bytes.
	 */
	gpart = (GSERIALIZED *) PG_DETOAST_DATUM_SLICE(gsdatum, 0, 40);
...
/*
 * * Peak into a geography to find the bounding box. If the * box is there,
 * copy it out and return it. If not, calculate the box from the * full
 * geography and return the box based on that. If no box is available, *
 * return LW_FAILURE, otherwise LW_SUCCESS.
 */
int
