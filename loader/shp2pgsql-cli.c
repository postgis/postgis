	...
	while ((c = pgis_getopt(argc, argv, "-acdeg:ikm:nps:t:wDGIN:ST:W:X:")) != EOF) {
		...
		case 's':
			if (pgis_optarg) {
				...
				/*
				 * With -s, user must specify TO_SRID or
				 * FROM_SRID:TO_SRID
				 */
		...
		/*
		 * Once we have parsed the arguments, make sure certain
		...
		/*
		 * Determine the shapefile name from the next argument, if no
		 * shape file, exit.
		 */
		if (pgis_optind < argc) {
		...
		/*
		 * Determine the table and schema names from the next
		 * argument
		 */
		if (pgis_optind < argc) {
		...
		/*
		 * If the table parameter is not provided, use the shape file
		 * name as a proxy value. Strip out the .shp and the leading
		 * path information first.
		 */
		...
		/*
		 * If we are not in "prepare" mode, go ahead and write out
		 * the data.
		 */
