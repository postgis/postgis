	...
	if (map != NULL) {
		if (map->size) {
			for (i = 0; i < map->size; i++) {
				if (map->pgfieldnames[i])
					free(map->pgfieldnames[i]);
				if (map->dbffieldnames[i])
...
colmap_read(const char *filename, colmap * map, char *errbuf, size_t errbuflen)
{
	...
	/*
	 * Read column map file and load the colmap_dbffieldnames and
	 * colmap_pgfieldnames arrays
	 */
	...
	while (fgets(linebuffer, 1024, fptr) != NULL) {
		/*
		 * Split into two separate strings: pgfieldname and
		 * dbffieldname
		 */
		...
		/*
		 * Allocate memory and copy the string ensuring it is
		 * terminated
		 */
 ...
 * (e.g. 1256) or as "LDID/1234" from the DBF hreader. We want to look up the
 * equivalent iconv encoding string so we can use iconv to transcode the data
 * into UTF8
 */
char	       *
...
/*
 * In the case where data is coming out of the database in some wierd
 * encoding we want to look up the appropriate code page entry to feed to
 * DBFCreateEx
 *
