		...
		else
		{
			if (srtext[0] == ' ')
			{
				snprintf(
				    state->message,
				    SHPDUMPERMSGLEN,
				    _("WARNING: Cannot determine spatial reference (empty table or unknown spatial ref). No prj file will be generated."));
				PQclear(res);
				free(query);
				return SHPDUMPERWARN;
			}
...
getTableInfo(SHPDUMPERSTATE *state)
{
	...
	if (state->geo_col_name)
	{
		...
		if (state->schema)
		{
			...
			sprintf(
			    query,
			    "SELECT count(\"%s\"), max(ST_zmflag(\"%s\"::geometry)), geometrytype(\"%s\"::geometry) FROM \"%s\".\"%s\" GROUP BY geometrytype(\"%s\"::geometry)",
			    state->geo_col_name,
			    state->geo_col_name,
			    state->geo_col_name,
			    state->schema,
		...
		else
		{
			...
			sprintf(
			    query,
			    "SELECT count(\"%s\"), max(ST_zmflag(\"%s\"::geometry)), geometrytype(\"%s\"::geometry) FROM \"%s\" GROUP BY geometrytype(\"%s\"::geometry)",
			    state->geo_col_name,
			    state->geo_col_name,
			    state->geo_col_name,
			    state->table,
