...
static int ShpDumperUseUserQuerySource(SHPDUMPERSTATE *state);
...
static int
ShpDumperUseUserQuerySource(SHPDUMPERSTATE *state)
{
	return state->config->usrquery && !state->usrquery_materialized;
}
...
ShpDumperGetDataSource(SHPDUMPERSTATE *state, const char *alias)
{
	...
	if (ShpDumperUseUserQuerySource(state))
	{
	...
	if (state->geo_col_name)
	{
		if (ShpDumperUseUserQuerySource(state))
		{
			...
			if (!source || !quoted_geo_col_name)
			{
				...
				snprintf(state->message,
					 SHPDUMPERMSGLEN,
					 _("ERROR: Out of memory building table metadata query"));
				return SHPDUMPERERR;
			}
...
ShpDumperCreate(SHPDUMPERCONFIG *config)
{
	...
	state->usrquery_materialized = LW_FALSE;
	state->message[0] = '\0';
...
ShpDumperOpenTable(SHPDUMPERSTATE *state)
{
	...
	/* If possible, preserve historic single-evaluation semantics by
	 * materializing user queries. Fall back to a derived query source only
	 * when the session lacks TEMP privilege. */
	if (state->config->usrquery)
	{
		int fallback_to_query_source = LW_FALSE;

		state->table = core_asprintf("__pgsql2shp%lu_tmp_table", (long)getpid());
		stringbuffer_init(&sb);
		stringbuffer_aprintf(&sb, "CREATE TEMP TABLE \"%s\" AS %s", state->table, state->config->usrquery);
		res = PQexec(state->conn, stringbuffer_getstring(&sb));
		stringbuffer_release(&sb);

		if (res && PQresultStatus(res) == PGRES_COMMAND_OK)
		{
			state->usrquery_materialized = LW_TRUE;
			PQclear(res);
			res = NULL;
		}
		else
		{
			...
			if (sqlstate && !strcmp(sqlstate, "42501"))
				fallback_to_query_source = LW_TRUE;

			if (res)
			{
				if (!fallback_to_query_source)
				{
					snprintf(state->message,
						 SHPDUMPERMSGLEN,
	 ...
	 */
	fieldtuples = ShpDumperUseUserQuerySource(state) ? PQnfields(res) : PQntuples(res);
	...
	{
		stringbuffer_aprintf(&sb, " FROM \"%s\".\"%s\"", state->schema, state->table);
	}
	else
	{
		stringbuffer_aprintf(&sb, " FROM \"%s\"", state->table);
	}
