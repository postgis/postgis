...
/*
 * Shapefile (dbf) field name are at most 10chars + 1 NULL. Postgresql field
 * names are at most 63 bytes + 1 NULL.
 */
...
typedef struct shp_loader_config {
	...
	/*
	 * whether to do a single transaction or run each statement on its
	 * own
	 */
	int		usetransaction;
...
typedef struct shp_loader_state {
	...
	/*
	 * String containing colume name list in the form "(col1, col2, col3
	 * ... , colN)"
	 */
	char	       *col_names;

	/*
	 * String containing the PostGIS geometry type, e.g. POINT, POLYGON
	 * etc.
	 */
	char	       *pgtype;
	...
	/*
	 * SRID of the shape file.  If not reprojecting, will be the same as
	 * to_srid.
	 */
	int		from_srid;

	/*
	 * SRID of the table.  If not reprojecting, will be the same as
	 * from_srid.
	 */
	int		to_srid;

	/*
	 * geometry/geography column name to use.  Will be set to the default
	 * if the config did not specify a column name.
	 */
	char	       *geo_col;
