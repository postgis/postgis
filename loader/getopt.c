 ...
 * UNIFORUM was slightly different from the current System V Release 2 manual
 * page.  The difference apparently involved a note about the famous rules 5
 * and 6, recommending using white space between an option and its first
 * argument, and not grouping options that have arguments. Getopt itself is
 * currently lenient about both of these things.  White space is allowed, but
 * not mandatory, and the last option in a group can have an argument.  That
