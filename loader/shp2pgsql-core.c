...
/*
 * Return allocated string containing UTF8 string converted from encoding
 * fromcode
 */
static int
utf8(const char *fromcode, char *inputbuf, char **outputbuf)
{
	...
	outbytesleft = inbytesleft * 3 + 1;	/* UTF8 string can be 3 times
						 * larger */
	/* then local string */
	*outputbuf = (char *)malloc(outbytesleft);
	...
	if (iconv(cd, &inputbuf, &inbytesleft, &outputptr, &outbytesleft) == (size_t) - 1) {
		...
		if (iconv(cd, &inputbuf, &inbytesleft, &outputptr, &outbytesleft) == -1) {
			...
			if (iconv(cd, &inputbuf, &inbytesleft, &outputptr, &outbytesleft) == -1) {
				/*
				 * Still no. Throw away the buffer and
				 * return.
				 */
				free(*outputbuf);
				iconv_close(cd);
...
escape_copy_string(char *str)
{
	 ...
	 * tab, backslash, cr, lf
	 *
	 * 1. find # of escaped characters 2. make new string
	 *
	 */
	...
	/*
	 * Count how many characters we need to escape so we know the size of
	 * the string we need to return
	 */
	while (*ptr) {
		if (*ptr == '\t' || *ptr == '\\' || *ptr == '\n' || *ptr == '\r')
			toescape++;
	...
	/*
	 * If we don't have to escape anything, simply return the input
	 * pointer
	 */
	if (toescape == 0)
		return str;
...
escape_insert_string(char *str)
{
	 ...
	 *
	 * 1. find # of characters 2. make new string
	 */
...
GeneratePointGeometry(SHPLOADERSTATE * state, SHPObject * obj, char **geometry, int force_multi)
{
	...
	/*
	 * If we have more than 1 vertex then we are working on a MULTIPOINT
	 * and so generate a MULTIPOINT rather than a POINT
	 */
...
GenerateLineStringGeometry(SHPLOADERSTATE * state, SHPObject * obj, char **geometry)
{
	...
	/*
	 * If using MULTILINESTRINGs then generate the serialized collection,
	 * otherwise just a single LINESTRING
	 */
	...
	for (i = 0; i < n - 1; i++) {	/* edge from V[i] to V[i+1] */
		if (((V[i].y <= P.y) && (V[i + 1].y > P.y))	/* an upward crossing */
			...
			if (P.x < V[i].x + vt * (V[i + 1].x - V[i].x))	/* P.x < intersect */
				++cn;	/* a valid crossing of y=P.y right of
					 * P.x */
		}
	}
...
FindPolygons(SHPObject * obj, Ring * **Out)
{
	...
	for (pi = 0; pi < in_index; pi++) {
		 ...
		 * will assign the little polygon's hole to the little
		 * polygon w/o a lot of extra fancy containment logic here
		 */
	...
	*Out = Outer;
	/*
	 * Only free the containing Inner array, not the ring elements,
	 * because the rings are now owned by the linked lists in the Outer
	 * array elements.
	 */
	free(Inner);
...
GeneratePolygonGeometry(SHPLOADERSTATE * state, SHPObject * obj, char **geometry)
{
	...
	if (state->config->simple_geometries == 1 && polygon_total != 1) {	/* We write Non-MULTI
										 * geometries, but have
										 * several parts: */
	...
	for (pi = 0; pi < polygon_total; pi++) {
		...
		/*
		 * Firstly count through the total number of rings in this
		 * polygon
		 */
		ring_total = 0;
		polyring = Outer[pi];
		...
		/*
		 * Cycle through each ring within the polygon, starting with
		 * the outer
		 */
		polyring = Outer[pi];
		...
		while (polyring) {
			/*
			 * Create a POINTARRAY containing the points making
			 * up the ring
			 */
			...
			for (vi = 0; vi < polyring->n; vi++) {
				/*
				 * Build up a point array of all the points
				 * in this ring
				 */
			...
			/*
			 * Copy the POINTARRAY pointer so we can use the
			 * LWPOLY constructor
			 */
	...
	/*
	 * If using MULTIPOLYGONS then generate the serialized collection,
	 * otherwise just a single POLYGON
	 */
	if (state->config->simple_geometries == 0) {
...
ShpLoaderOpenShape(SHPLOADERSTATE * state)
{
	...
	if (strcmp(state->config->encoding, ENCODING_DEFAULT) == 0) {
		...
		if (state->hDBFHandle->pszCodePage) {
			/*
			 * And we figured out what iconv encoding it maps to,
			 * so use it!
			 */
	...
	if (state->config->readshape == 1) {
		...
		if (state->config->null_policy == POLICY_NULL_ABORT) {
			/*
			 * If we abort on null items, scan the entire file
			 * for NULLs
			 */
		...
		/*
		 * If in simple geometry mode, alter names for CREATE TABLE
		 * by skipping MULTI
		 */
	...
	/*
	 * Generate a string of comma separated column names of the form
	 * "(col1, col2 ... colN)" for the SQL insertion string
	 */
	...
	for (j = 0; j < state->num_fields; j++) {
		...
		if (state->config->encoding) {
			...
			/*
			 * If a column map file has been passed in, use this
			 * to create the postgresql field name from the dbf
			 * column name
			 */
		...
		/*
		 * Make field names lowercase unless asked to keep
		 * identifiers case.
		 */
		...
		/*
		 * Escape names starting with the escape char (_), those
		 * named 'gid' or after pgsql reserved attribute names
		 */
		if (name[0] == '_' ||
		...
		/*
		 * Now generate the PostgreSQL type name string and width
		 * based upon the shapefile type
		 */
		...
		if (state->config->readshape == 1 || j < (state->num_fields - 1)) {
			/*
			 * Don't include last comma if its the last field and
			 * no geometry field will follow
			 */
			strcat(state->col_names, "\",");
		} else {
			strcat(state->col_names, "\"");
...
/*
 * Return a pointer to an allocated string containing the header for the
 * specified loader state
 */
int
ShpLoaderGetSQLHeader(SHPLOADERSTATE * state, char **strheader)
{
	...
	/*
	 * Create the stringbuffer containing the header; we use this API as
	 * it's easier for handling string resizing during append
	 */
	...
	if (state->config->opt != 'a') {
		/*
		 * Create a table for inserting the shapes into with
		 * appropriate columns and types
		 */
		if (state->config->schema) {
		...
		/*
		 * Generate the field types based upon the shapefile
		 * information
		 */
		for (j = 0; j < state->num_fields; j++) {
			...
			if (!strcmp("numeric", state->pgfieldtypes[j])) {
				/*
				 * Doubles we just allow PostgreSQL to
				 * auto-detect the size
				 */
		...
		/*
		 * Add the geography column directly to the table definition,
		 * we don't need to do an AddGeometryColumn() call.
		 */
		if (state->config->readshape == 1 && state->config->geography) {
			char	       *dimschar;
		...
		/*
		 * Create the primary key.  This is done separately because
		 * the index for the PK needs to be in the correct
		 * tablespace.
		 */

		/*
		 * TODO: Currently PostgreSQL does not allow specifying an
		 * index to use for a PK (so you get a default one called
		 * table_pkey) and it does not provide a way to create a PK
		 * index in a specific tablespace.  So as a hacky solution we
		 * create the PK, then move the index to the correct
		 * tablespace.  Eventually this should be: CREATE INDEX
		 * table_pkey on table(gid) TABLESPACE tblspc; ALTER TABLE
		 * table ADD PRIMARY KEY (gid) USING INDEX table_pkey; A
		 * patch has apparently been submitted to PostgreSQL to
		 * enable this syntax, see this thread:
		...
		if (state->config->idxtablespace != NULL) {
			...
			/*
			 * WARNING: We're assuming the default "table_pkey"
			 * name for the primary key index.  PostgreSQL may
			 * use "table_pkey1" or similar in the case of a name
			 * conflict, so you may need to edit the produced SQL
			 * in this rare case.
			 */
		...
		if (state->config->readshape == 1 && (!state->config->geography)) {
			/*
			 * If they didn't specify a target SRID, see if they
			 * specified a source SRID.
			 */
	...
	/*
	 * Copy the string buffer into a new string, destroying the string
	 * buffer
	 */
...
ShpLoaderGenerateSQLRowStatement(SHPLOADERSTATE * state, int item, char **strrecord)
{
	...
	for (i = 0; i < DBFGetFieldCount(state->hDBFHandle); i++) {
		...
		if (DBFIsAttributeNULL(state->hDBFHandle, item, i)) {
			...
			case FTDouble:
				...
				/*
				 * If the value is an empty string, change to
				 * 0
				 */
				if (val[0] == '\0') {
					val[0] = '0';
				...
				/*
				 * If the value ends with just ".", remove
				 * the dot
				 */
				if (val[strlen(val) - 1] == '.')
					val[strlen(val) - 1] = '\0';
			...
			/*
			 * Escape attribute correctly according to dump
			 * format
			 */
			if (state->config->dump_format) {
		...
		/*
		 * Only put in delimeter if not last field or a shape will
		 * follow
		 */
	...
	if (state->config->readshape == 1) {
		...
		if (obj->nVertices == 0) {
			...
			/*
			 * The default returns out of the function, so res
			...
			/*
			 * Now generate the geometry string according to the
			...
			if (!state->config->dump_format) {
				...
				if (state->to_srid != state->from_srid) {
					/*
					 * We need to add an explicit cast to
					 * geography/geometry to ensure that
					 * PostgreSQL doesn't get confused
					 * with the ST_Transform() raster
					 * function.
					 */
	...
	*strrecord = ret;
	...
	/*
	 * If any warnings occurred, set the returned message string and
	 * warning status
	 */
