...
for EXTNAME in $extensions_to_install; do
	...
	if test "$EXTNAME" = "address_standardizer"; then #repeat for address_standardizer_data_us
		...
		cp extensions/$EXTNAME/sql/${EXTNAME}_data_us--TEMPLATED--TO--ANY.sql ${PGPATHEDB}/share/extension/${EXTNAME}_data_us--$POSTGIS_MICRO_VER--ANY.sql;
	fi
	...
	for OLD_VERSION in $UPGRADEABLE_VERSIONS; do \
		if [ "$OLD_VERSION" > "2.5" ] || [ "$OLD_VERSION" in $WIN_RELEASED_VERSIONS ]; then
			...
			if test "$EXTNAME" = "address_standardizer"; then
				cp extensions/$EXTNAME/sql/${EXTNAME}_data_us--TEMPLATED--TO--ANY.sql  ${PGPATHEDB}/share/extension/${EXTNAME}_data_us--$OLD_VERSION--${POSTGIS_MINOR_MAX_VER}.sql; \
			fi
		fi
	done
done
