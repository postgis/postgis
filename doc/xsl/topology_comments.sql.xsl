...
<xsl:when test="parent::db:section[contains(@xml:id,'Types')]">
COMMENT ON TYPE topology.<xsl:value-of select="db:refnamediv/db:refname" /> IS 'postgis type: <xsl:value-of select='$comment' />';
</xsl:when>
<xsl:when test="parent::db:section[contains(@xml:id,'Domain')]">
COMMENT ON DOMAIN topology.<xsl:value-of select="db:refnamediv/db:refname" /> IS 'postgis domain: <xsl:value-of select='$comment' />';
</xsl:when>
		</xsl:choose>
...
<xsl:choose><xsl:when test="contains(db:parameter,'OUT')"></xsl:when><xsl:when test="contains(db:type,'topoelement set')">topoelement</xsl:when><xsl:when test="contains(db:type,'topogeometry set')">topogeometry</xsl:when><xsl:otherwise><xsl:value-of select="db:type" /></xsl:otherwise></xsl:choose><xsl:if test="position()&lt;last() and not(contains(db:parameter,'OUT')) and not(contains(following-sibling::paramdef[1],'OUT'))"><xsl:text>, </xsl:text></xsl:if></xsl:when>
