...
| `UsersWikiBatchLoadShapefilesForWindowsUsingShp2pgsql` | Retired from this repository import after moving the useful Windows batch-loop pattern into the `shp2pgsql` section of the user manual. |
| `UsersWikiBufferCoast` | Retired from this repository import after moving the useful buffer/unbuffer coastline simplification pattern into the `ST_Buffer` manual examples. |
...
| `WKTRaster_Documentation01` | Retired from this repository import after checking the current raster manual. The old beta introduction, loader notes, raster type overview, catalog notes, in-db/out-db discussion, and placeholder function-reference sections are covered or superseded by `doc/using_raster_dataman.xml`, `doc/reference_raster.xml`, `raster/rt_pg/rtpostgis.sql.in`, and the current `raster2pgsql` loader documentation. |
